...
- 저장된 회원 대화를 Spring의 명시적 session-end 또는 AI 내부 10분 inactivity timeout으로 확정해 프로필을 갱신한다.
...
AI idle scheduler ── pg-profile 세션 활동 조회 ───────────> 공통 profile finalizer
...
- Spring I-20은 `logout`·`newConversation`만 `/events/session-end`로 보낸다.
- AI는 수락된 회원 발화를 저장할 때 pg-profile의 `last_activity_at`을 DB 서버 시각으로 갱신하고, 기본 60초마다 10분 이상 비활성인 세션을 bounded batch로 처리한다.
- 두 경로는 같은 finalizer와 `session-end:{userId}:{sessionId}` claim으로 직렬화한다. Spring I-20만 키를 영구 완료하고 idle은 checkpoint 성공 뒤 claim을 해제한다. AI가 자기 HTTP 엔드포인트를 호출하지 않는다.
- 탭 닫기 전용 신호/API는 없다. timeout 처리 중·후 같은 sessionId로 돌아와도 새 발화가 activity를 재개하며, scheduler는 라이브 스트림 슬롯을 점유하지 않는다.

로컬 기본값은 `PROFILE_SESSION_IDLE_TIMEOUT_S=600`, `PROFILE_IDLE_SWEEP_INTERVAL_S=60`이다. timeout 통합 테스트는 시간을 실제로 기다리지 않고 clock/config를 주입한다.

### 2.4 Spring 상품 검색 응답 envelope 호환
...
### 2.5 Spring 최소 검색 응답에서 price를 optional로 변경
...
### 2.6 sample_100 사전 임베딩 적재 도구 추가
