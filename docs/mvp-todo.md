...
- [ ] `get_seller_aggregates` I-6 연결 (§4.4) 🔴 C-13
- [ ] 통계 질의 파싱 → 집계 → `token` 산문 응답
- [ ] `get_product_detail` I-7 연결 (§4.5) 🔴 C-14
- [ ] draft 흐름 — 개정안 → SSE `draft{changes[]}` emit
- [ ] 발화 ≠ 동의 가드 (반영은 FE S-3 PATCH)
...
- [ ] PostgresStore 네임스페이스 (profile/facts/episodes)
...
- [ ] checkpointer 대화 저장 (user 수신 즉시 / assistant 완료 후)
...
| C-13 | I-6 (판매자 집계) | 판매자 통계 |
| C-14 | I-7 (상세 읽기) | 판매자 draft |
