...
| ↳ **I-1 검색만 재시도 1회** (v0.19.2) | **3s × 2 = 6s** (config `SPRING_MAX_RETRIES`) | **BE 관측 포인트** — 한 턴에 같은 검색 요청이 최대 2번 온다. 재시도 대상은 **타임아웃·연결 오류·응답 중단(서버가 응답 도중 연결 종료)·5xx·일시 4xx(408·429)** 이며 **그 밖의 4xx 는 즉시 실패**한다(다시 불러도 같은 결과). 408·429 만 4xx 중 예외인 이유는 요청 자체가 유효하고 서버·인프라의 **일시 상태**일 뿐이기 때문이다(계약에 명시된 코드는 아니나 프록시·게이트웨이가 낼 수 있다). `Retry-After` 는 존중하지 않는다 — backoff 가 없어 즉시 재호출하며, 상한 1회로 증폭을 묶는다. I-1은 GET·멱등이라 안전하고, **비멱등 호출(I-2 담기 등)에는 재시도를 걸지 않는다.** 재시도 총량(기본 6s)은 구매자 전체 상한(30s)·first-token 상한(10s)보다 작고 기동 시 검증하지만, 이는 **필요조건**이다. `may_auto_relax` 턴(#113)은 `conditions` 를 검색 뒤로 미뤄 첫 이벤트 앞에 I-1이 본 검색+자동 완화 probe로 **최대 2회 직렬**(12s)이다. **실측(#277, 2026-08-04)**: 미루지 않는 턴 p50 0.40s — 검색이 1차 타임아웃(3.0s)을 쓴 턴도 0.42s로 같았고, 미룬 정상 턴 0.76~1.08s, 검색 재시도 1회 6.80s였다. 두 호출이 모두 재시도를 쓰고 재시도 응답이 상한 직전(2.9s)에 오면 10s를 넘어 **이벤트 0건인 채 504(8/8)**였지만 재시도 뒤 즉답이면 7.09s로 상한 안이었다. #113 전에는 조건 칩 뒤 in-stream `error(SEARCH_FAILED)`로 끝났다. 수치는 LLM head 제외(#151 baseline head p95 ≈3.0s를 더해 읽으며, 그러면 재시도 1회 턴이 상한에 닿는다). `relaxation_max_rounds=0`이면 미루기 자체가 꺼져 이 경로가 사라진다 |
...
| 버전 | 날짜 | 변경 |
|---|---|---|
| v0.20.2 | 2026-08-04 | **[#277] §2.9(c)의 I-1 재시도 예산 근거를 실 HTTP 경계 측정으로 정정했다.** `conditions`를 미루지 않는 턴은 p50 0.40s였고 검색이 1차 타임아웃(3.0s)을 쓴 턴도 0.42s, 미룬 정상 턴은 0.76~1.08s, 검색 재시도 1회는 6.80s였다. 수치는 LLM head 제외(#151 baseline head p95 ≈3.0s를 더해 읽으며, 그러면 재시도 1회 턴이 상한에 닿는다). #113의 `may_auto_relax` 턴은 첫 이벤트 앞에 본 검색과 자동 완화 probe가 최대 2회 직렬이라 단일 I-1 기준 6s 검증만 안전하다. 두 호출이 모두 재시도를 쓰고 재시도 응답이 상한 직전(2.9s)에 온 실최악은 이벤트 0건·504가 8/8 재현됐지만 재시도 뒤 즉답이면 7.09s·200이었다. `relaxation_max_rounds=0`이면 미루기가 꺼진다. **와이어 계약(엔드포인트·SSE 이벤트·필드·오류 코드)은 불변**이고, 튜너블과 검증식은 바꾸지 않아 직렬 예산 재설계를 후속 과제로 남겼다. |
| v0.20.1 | 2026-08-04 | **[#278] §4.6 `categoryName` 해석을 정본 2026-08-03 개정(잎 이름 정확 일치)으로 동기화하고 구 "대분류명이 기본" 문구를 폐기했다.** 와이어 계약은 불변이며 코드는 잎 매핑(#115/#217)으로 이미 정합하다. §3.1 v0.15.26 등재 계약인 `conditionActions` 수신을 구현했다(계약 자체 불변). |
