...
- **AI 구현 상태**: config `progress_events_enabled` **기본 `true`**(#396, 2026-08-06 FE 구현 완료로 해제 — 계약 등재 v0.21.0·운영 기동 가드 제거)로 실제 와이어에 나가며, stage 어휘·다회 emit은 v0.27.0부터 실제로 나가고 #406(v0.32.4)이 `retrying`을 더해 8종이 됐다. `retrying`은 `spring_max_retries > 0`일 때만 나가며 기본값 1에서는 실제 와이어에 나간다. `spring_max_retries=0` 배포에서는 나가지 않는다. **[갱신 v0.32.5, #306]** 발신 조건은 **턴 유형과 무관하다** — v0.32.4까지는 `conditions`를 미룬 턴이 재시도 자체를 하지 않아 `retrying`도 나가지 않았으나, #306이 그 억제를 제거해 미룬 턴에서도 나간다(그 턴에서는 `conditions` **앞**에 온다 — 위 "상대 순서" 규약대로 `conditions` 자체는 여전히 검색·자동 완화 뒤다). 되돌리려면 `PROGRESS_EVENTS_ENABLED=false` 와 `SPRING_MAX_RETRIES=0` 을 함께 설정한다.
