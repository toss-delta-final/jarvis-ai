...
- **AI 구현 상태**: config `progress_events_enabled` **기본 `true`**(#396, 2026-08-06 FE 구현 완료로 해제 — 계약 등재 v0.21.0·운영 기동 가드 제거)로 실제 와이어에 나가며, stage 어휘·다회 emit은 v0.27.0부터 실제로 나가고 #406(v0.32.4)이 `retrying`을 더해 8종이 됐다. `retrying`은 `spring_max_retries > 0`일 때만 나가며 기본값 1에서는 실제 와이어에 나간다. `spring_max_retries=0` 배포에서는 나가지 않는다. 되돌리려면 `PROGRESS_EVENTS_ENABLED=false` 와 `SPRING_MAX_RETRIES=0` 을 함께 설정한다.
