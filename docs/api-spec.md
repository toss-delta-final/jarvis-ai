...
**(1) `progress`** — 진행 단계 알림 (0회 이상). **[확정 v0.21.0, 이슈 #289, 정본 합의·등재 2026-08-05 / 다회 emit·어휘 확장 v0.27.0, 이슈 #396, 정본 개정 2026-08-06 / retrying 추가 v0.32.2, 이슈 #406]**
...
- **AI 구현 상태**: config `progress_events_enabled` **기본 `true`**(#396, 2026-08-06 FE 구현 완료로 해제 — 계약 등재 v0.21.0·운영 기동 가드 제거)로 실제 와이어에 나가며, stage 어휘·다회 emit은 v0.27.0부터 실제로 나가고 #406(v0.32.2)이 `retrying`을 더해 8종이 됐다. `retrying`은 `spring_max_retries > 0`일 때만 나가며 현재 기본값은 #394 한시 조치로 `0`이라 실제 와이어에는 나가지 않는다. 되돌리려면 `PROGRESS_EVENTS_ENABLED=false`.
...
| C-20 | **[신규 v0.22.0 · 개정 v0.26.0] 개인화 그래프 인증 레인 = Spring proxy (조회 포함)** | **조회(§3.8)·변경(§3.9) 모두** FE→Spring→AI internal(`X-Internal-Token`)이다. v0.22.0은 조회만 FE→AI 직접(`chat:stream` 티켓 재사용)으로 두었으나, **마이페이지에는 채팅 세션이 없어 티켓을 발급받을 수 없다**(CH-1b는 `sessionId` 필수) — 재사용할 자산이 없어 전제가 무너졌다. 전용 `profile:read`/`profile:write` 티켓 신설안은 **여전히 기각** — `scope`가 exact `chat:stream`으로 하드 고정된 검증 경로를 `/chat`·`/seller/chat`이 함께 지나가므로 채팅 인증에 회귀 위험을 만드는 대가를 치른다(§2.3 a) | §3.8·§3.9 / 이슈 #149·#322·#499 | 🟢 **(1)(2)(3) 해소(2026-08-08, `jarvis-backend#132`)** — (1) Spring이 `M-11`·`M-12`·`M-13`·`M-15`·`M-16` **5개를 호스팅**한다(`M-14`는 폐기라 제외), (2) *"회원 id는 로그인 세션에서만 도출한다. 경로·쿼리·본문 어디서도 받지 않는다"*, (3) *"사용자 AT는 AI로 넘기지 않는다"*. 확약 대상 경로는 **5개**다(v0.26.0의 6개에서 I-35 폐기로 감소). 🔴 **잔여 (4)**: 비로그인 게스트에게 §3.8을 노출하지 않는다는 확약 — 노출해야 한다면 `guest.id`가 UUID 문자열이라 BIGINT `{userId}`로 표현할 수 없어 계약을 다시 열어야 한다 |
| C-21 | **[신규 v0.22.0 · 갱신 v0.26.0] `If-Match`/`graphVersion` 통과 규약** | 요청은 `If-Match` 헤더(정규), 성공 응답은 본문 `graphVersion`(정규) + §3.8 `ETag`(편의 사본), **`409` 응답은 `error.detail.graphVersion`**(v0.26.0 위치 정정 — 구 계약은 봉투 밖 형제). 값은 **불투명**이며 파싱·순서 비교 금지 | §2.5·§3.8·§3.9 / 이슈 #149·#322·#499 | 🟢 **(1)(2) 해소(2026-08-08, `jarvis-backend#132`)** — (1) *"If-Match 무변형 전달(재따옴표·정규화 금지)"*, (2) *"`PROFILE_*` 3종은 변환 없이 FE에 노출한다"* + `error.detail.graphVersion` 위치 유지. ⚠️ **범위 정밀화**: 무변형 대상은 **`PROFILE_*` 코드와 `error.detail`**이며 레인 공통 코드는 Spring이 변환한다(§2.5 변환표) — 구 문구는 봉투 전체 무변형을 요구해 문자 그대로면 BE 구현이 위반이었다. 🔴 **잔여 (3)**: FE가 `Access-Control-Expose-Headers: ETag` 없이 본문 필드만으로 동작하는지(C-11 연계) — **정본이 근거를 보강했다**: *"버전 값은 요청은 헤더, 응답은 본문이 정규다 — 응답 헤더는 CORS 제약으로 브라우저 JS가 읽지 못해 본문을 정규로 둔다"* |
...
| 버전 | 날짜 | 변경 |
|---|---|---|
| v0.32.2 | 2026-08-09 | **[#406] 구매자 `progress`에 `retrying` stage를 추가했다.** I-1 검색이 재시도 가능한 실패 뒤 실제 다음 시도에 들어갈 때만 본검색 드레인이 즉시 frame을 내며, fan-out 콜백이 여러 번이어도 턴당 1회로 제한한다. 기본 `spring_max_retries=0`(#394 한시 조치)에서는 해당 task·queue 경로를 전혀 타지 않아 기존 인라인 await 경로의 취소·ContextVar·trace 의미가 바이트 단위로 유지된다. 구제 체인·자동 완화 probe·`_run_search_unfiltered`에는 이번 관측을 배선하지 않는다. |
