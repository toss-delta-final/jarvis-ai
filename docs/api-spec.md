...
| (c) 역방향(질의 시점) | **AI → Spring** | 후보 검색(I-1, §4.6), 구매 이력 조회(I-19, §4.7), 장바구니 담기(I-2, §4.1)·조회(I-18, §4.9), 추천 목록 push(§4.2), 판매자 집계(I-6/7/13/14/15/16, §4.4), 상품 CRUD(I-9/10/11/12, §4.5), 생성물 배치(I-17, §4.8) | **전부 서비스 토큰(internal, `X-Internal-Token`) + 본문/쿼리 신원**(AI가 JWT `sub`에서 도출) — BE 실측 정합(v0.13.0) | 결정 7 / 경로 B / BE DB 정합 |
...
구매자의 자연어 질의를 받아 상품 추천/장바구니/상품 질문 등을 SSE로 스트리밍 응답한다. 소유: `SPEC-RECOMMEND-001`(추천 서브그래프), 상위 구매자 그래프 SPEC(라우팅).

> **[경로 정합 v0.13.0]** FE-대면 경로는 **`POST /ai/chat`**(BE DB 실측, 인증 필요). 본 문서 다른 위치의 `POST /chat` 표기는 이 경로로 읽는다. 구 `/api/chat`(인증 불필요) 행은 폐기본. (판매자는 `{AI_SERVER}/seller/chat`.)
...
### 3.5 `POST /events/session-end` (I-20) — 세션 종료 통지 (Spring → AI, best-effort, 멱등, 본 문서 소유)
...
X-Internal-Token: {서비스 토큰}   ← 전 구간 서비스 토큰 레인(v0.13.0)
```
...
- **인증**: `X-Internal-Token` 서비스 토큰(v0.13.0 통일, §1.2 레인 c). 상관관계 키(sessionId 등)로 사용자 목록 귀속.
...
| I-13 | `/internal/seller/{brandId}/events` | 행동 이벤트 집계 | behavior |
...
### 4.7 구매 이력 조회 API (I-19 `GET /internal/members/{id}/orders`, query-time) — [BE DB 등재 v0.14.0]
...
```
GET {SPRING_BASE_URL}/internal/members/{id}/orders?window={days}   ← BE 실측(I-19). {id}=userId(AI가 JWT sub 도출)
X-Internal-Token: {서비스 토큰}   ← 서비스 토큰 레인. userId는 AI가 JWT sub에서 도출해 쿼리/바디로 전달
```
...
| C-6 | **구매 이력 조회 = I-19** | BE DB 등재 `GET /internal/members/{id}/orders`(§4.7), **서비스 토큰 + {id}=userId(AI 도출)**, 응답 3필드(`productId`·`category`·`purchasedAt`). dedup(14-F)·프로필 소스 공용, 게스트 스킵. I-4(주문 상태 요약)와 별개 | I-19 / BE DB | 🔴 window·응답 스키마 |
...
| C-9 | **추천 목록 전달(push) API** | 경로 B: `POST /recommendations`(상관관계 키 + groups[{title/category, items[{productId,rank,reason}]}])(§4.2), **서비스 토큰**(v0.13.0) | 경로 B | 🔴 미확정 — 경로·listId 발급·상관관계 키 |
...
| C-13 | **[재정의 v0.8.0] 판매자 집계 API 5종** | BE 문서 채택 — `GET /internal/seller/{brandId}/{sales\|funnel\|events\|churn}` + 전역 `/internal/account-events`. `X-Internal-Token`, `brandId`=JWT 클레임(§2.6). 구 단일 `/seller/aggregates` 제안 폐기. 통계 답변 원천(§4.4) | BE 문서 / #9 | 🔴 **최우선** — 응답 스키마·전역 I-8 admin 소유·I-number 정합 |
