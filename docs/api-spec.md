...
> - 좌표 지시(`"3번째 줄 2번째"`)는 추천 표면에 `columns`가 없어 여전히 되물음이다 — 엉뚱한 칸을 담느니 한 번 더 묻는다.
