...
| 버전 | 날짜 | 변경 |
|---|---|---|
| v0.19.1 | 2026-08-02 | **[#138 후속] §2.9(c)의 FE→AI 스트림 전체 상한을 역할별로 분리했다.** #138 실측에서 구매자 total p95 10.5s·max 12.8s, 154턴 중 30s 초과 0건을 근거로 구매자는 **30s**(`stream_total_timeout_buyer_s`)로 명시하고, 판매자·미지정 역할은 **90s**(`stream_total_timeout_s`)를 유지했다. **와이어 계약(엔드포인트·SSE 이벤트·필드·오류 코드) 불변**이며, 두 역할 모두 초과 시 `done`(`finishReason=stop`) 정상 절단 규약을 유지한다. |
