...
**[vNEXT 신설, 이슈 #570] 렌더링 방식** — FE 지적대로 위 예시만으로는 `token.text`를 어떻게 그려야
...
- **`token`** (근거/답변 산문 증분, 0회 이상). **[vNEXT 사실 기록, 이슈 #570]** 판매자 스트림의
