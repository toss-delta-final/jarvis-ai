...
**(1) `progress`** — 진행 단계 알림 (0~1회). **[확정 v0.21.0, 이슈 #289, 정본 합의·등재 2026-08-05]**
...
| 필드 | 타입 | 필수 | 설명 |
|---|---|---|---|
| `stage` | string | 예 | 진행 단계 어휘. **확정 값은 `analyzing` 1종**(요청을 확인하는 중, intent 미확정 시점). `searching`/`relaxing`/`reranking`은 후속 확장 후보이며 미구현 |
| `message` | string | 아니오 | 사용자 노출 문구. 서버가 빈 값이면 `data`에 이 키 자체를 싣지 않고, 그때 FE가 `stage`로 자체 문구를 매핑한다 |

- **0~1회다 — FE는 도착을 전제하면 안 된다.** 나가면 **스트림의 첫 프레임**이지만, 그 앞에서 턴이 끝나면(LLM 미구성 → `error{LLM_UNAVAILABLE}`, 세션 상태 저장소 장애 → §2.5 스트림 전 오류 봉투) 0회다.
- **첫 stage가 `analyzing`(intent 중립)인 이유** — 이 프레임을 낼 수 있는 지점(decompose 앞)에서는 아직 추천/담기/주문조회/일반 대화 중 어느 intent인지 확정되지 않았다. `searching`을 그 자리에서 내면 비추천 턴(담기·주문조회 등)에 "검색 중"이라고 오라벨링하게 된다.
...
- **AI 구현 상태**: 구현은 완료됐으나 config `progress_events_enabled` **기본 `false`** + 운영·스테이징 기동 가드로 잠겨 있어, 실제 와이어는 아직 이 이벤트 없이 기존 6종만 나간다.
...
정상 흐름(추천): **[확정 v0.21.0, 이슈 #289]** `progress`(0~1회, 나가면 스트림 첫 프레임) → `conditions`(0~1회) → `token`(0회 이상) + `suggestions`(해당 시) → `products.ready`(성공 시 정확히 1회) → `done`(1회). `progress`는 추가 전용이며 **기존 6종의 상대 순서는 불변**이다 — `conditions`는 여전히 검색·자동 완화 뒤다(§2.9 c). **[v0.15.26] `budget`은 순서 계약에서 제외**했다(정본이 명세에서 제외 — 미구현, post-MVP). 장바구니 흐름: (`progress` →) `token`/`action` → `done`. `products.ready`는 목록 push 성공 이후에만 나타난다.
...
| `op` | `"update"` \| `"create"` \| `"delete"` \| `"ship"` | 실행할 쓰기 종류(I-11/I-10/I-12/**I-30** 매핑). **[v0.24.0, #297] `ship`(주문 발송 처리) 추가 — 🔶 초안, BE 협의 전. 정본(노션 CH-2/S-4) 개정은 후속(확정 2026-08-04)** |
| `productId` | number | 대상 상품 식별자(숫자 BIGINT, §2.6). `create`는 없을 수 있음(`null`), **`ship`은 항상 `null`** |
| `orderItemId` | number | **[v0.24.0, #297] `ship` 전용 키(그 외 op 에는 키 자체가 없다 — 추가 전용)** — 발송 대상 주문 아이템(숫자 BIGINT). I-29(§4.17) 조회 결과에서 해소한 값 |
...
- **[v0.24.0, #297] `op="ship"`(주문 발송, 🔶 초안)**: 흐름은 상품 쓰기와 동일하게 **기존 `product` 레인을 재사용**한다(확정 2026-08-04 — meta.lane 신설 없음). `meta(lane:"product")` → `draft{op:"ship", orderItemId}` → interrupt → confirm → **I-30 PATCH**(§4.18) → `done(panel:"refresh")`. confirm 의 실패적 결과(만료·이미 발송 409·전이 불가 400·소유 불일치 404)는 S-4 확정 규칙 그대로 **HTTP 200 + 안내 `token` + `done(panel:"keep")`** 이며, HITL 5대 안전장치(draftId 바인딩·명시 액션만·멱등성·Spring 하드게이트·TTL)가 동일하게 적용된다. 발송 대상 해소는 정본(노션 I-30) 설계상 `screen`(pageType=`seller_orders`) 맥락 우선, 부족하면 I-29(§4.17) 조회다 — **🔶 단, screen 경로는 AI 미구현이다(2026-08-05 현재): product 레인은 screen 을 draft 에이전트 입력에 주입하지 않아(§3.2 판매자 screen 주입은 supervisor·analysis 한정, #118) 현재는 I-29 조회로만 해소한다. screen 주입은 후속 작업.**
...
>
> **[v0.24.0, #297] ⚠️ I-번호 충돌 — 본 절의 I-29~I-33 재채번 제안.** 정본(노션 "📡 API 명세서" DB)에는 **I-29~I-31 이 판매자 주문·리뷰 API**(§4.17~§4.19, 2026-08-04 등재)로 배정돼 있다. 본 절의 채번은 등재 당시부터 미확정(🔴 C-26)이었으므로, **본 절을 I-34~I-38 로 재채번**하는 것을 제안한다(판매자 3종은 정본 유지). 확정 전까지 문서 내 상호 참조 시 "§3.9의 I-29(그래프)"와 "§4.17의 I-29(주문)"를 절 번호로 구분할 것. 개인화 파트 합의 후 본 절 번호를 일괄 치환한다.
...
### 4.17 자사 주문 조회 API (I-29 `GET /internal/seller/{brandId}/orders`) — 🔶 초안, BE 협의 전 [v0.24.0 신설, #297]
...
### 4.18 주문 아이템 발송 처리 API (I-30 `PATCH /internal/seller/{brandId}/order-items/{orderItemId}/status`) — 🔶 초안, BE 협의 전 [v0.24.0 신설, #297]
...
### 4.19 자사 상품 리뷰 조회 API (I-31 `GET /internal/seller/{brandId}/reviews`) — 🔶 초안, BE 협의 전 [v0.24.0 신설, #297]
