...
- **복구·초기 구축 [v0.25.1 개정 — #325]**: `ON_SALE` 단건 실패(enrichment·임베딩 등)는 `enrichment_item_attempts`(기본 2)회 재시도 후 dead-letter 로그로 격리하고 나머지 항목을 계속 처리하며, 페이지가 정상 종료되면 커서를 전진시킨다 — 종전 규약(실패 시 페이지 전체 미전진)에서는 실패 항목 1개가 뒤따르는 모든 변경을 영구 차단했다(head-of-line blocking, 운영 정지 #325). "자연 회복"이 실제로는 다음 주기에 같은 실패가 반복되는 것이었다. 격리 항목은 그 주기에 갱신되지 않으나 조용한 누락이 아니라 dead-letter ERROR 로그와 배치 결과 `failed` 카운트로 드러나며, 복구는 `run_batch --full`(전체 재구축)로 한다. **광역 장애 가드**: 페이지 `ON_SALE` 표본이 `artifacts_batch_failure_min_sample`(기본 5) 이상이고 실패 비율이 `artifacts_batch_failure_ratio_threshold`(기본 0.5) 이상이면 광역 장애로 보고 그 페이지는 커서를 전진시키지 않고 중단한다(종전 자연 회복 유지, 동일 커서로 다음 주기 재개). 표본이 `min_sample` 미만이면 비율 판정을 생략하고 격리+전진한다 — 운영 증분 페이지는 대개 소수 항목이라 poison 단건과 광역 장애를 비율만으로 구별할 수 없고, 구별 실패의 대가로 배치 전체를 막는 쪽이 더 나쁘다. `HIDDEN` 삭제 실패는 격리하지 않고 그대로 전파해 커서를 전진시키지 않는다(항목별 ack/DLQ 계약이 없어 삭제 이벤트를 skip-전진하면 유령 상품이 영구히 남는다). 계약 위반(`status` 미정의 값)은 바로 위 불릿대로 종전과 같이 페이지 전체 fail-closed — 불변. 초기 전체 구축도 커서 0부터 같은 API·같은 복구 규약으로 처리. **이 문단은 AI측 소비 동작 서술이며, I-17 요청 파라미터·응답 필드·오류 코드(와이어 계약)는 불변이다.**
