...
> `imageUrls` 는 **[v0.30.0, #506] 이미지 기반 상품 등록** 전용 선택 필드 — 이미지를 **새로 첨부한 턴에만** 싣는다(아래 표).
...
| `imageUrls` | array\<string\> | 아니오 | **[v0.30.0 신설, #506 — 🔶 초안, 정본(Notion) 개정 후속]** 새로 첨부한 상품 이미지의 **canonical S3 URL**(MVP 1장). **새로 첨부한 턴에만 싣는다** — 후속 턴(수정 요청 포함)엔 싣지 않는다(서버가 첫 분석을 보관, 매 턴 재분석 방지). 검증: http(s)·**500자 이하**·presigned(`X-Amz-Signature`) 거부 — 위반은 `400 BAD_REQUEST`. `action=="confirm"` 과 병용 금지(confirm 은 `draftId` 만) |
...
| `preview` | object | **[v0.30.0, #506] `create` 전용 키(그 외 op 에는 키 자체가 없다 — 추가 전용)** — 등록 미리보기 카드용 **표시 사본**. FE 는 create 카드를 `preview` 만 보고 그린다(`changes` 는 실행 정본 — 승인하면 그 값이 상품이 된다). 아래 `preview` 절 |

**`preview`** — 등록 미리보기 표시 사본 **[v0.30.0 신설, #506 — 🔶 초안, 정본(Notion) 개정 후속]**
