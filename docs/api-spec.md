...
| 버전 | 날짜 | 변경 |
|---|---|---|
| v0.31.1 | 2026-08-09 | **[#518] §3.2 `findings[].analysisType` 표에 `"review"` 등재 — 사본 드리프트 정정.** v0.25.0(#297)이 리뷰 워커를 6종째로 붙이면서 `AnalysisType` 에 `review` 를 넣었는데 이 열거 표만 5종에 멈춰 있었다. 코드(`workers.build_review_agent`·`REVIEW_PROMPT`·verifier F3 `check_type_match`)와 실 와이어는 처음부터 6종이었으므로 **신설 협의가 아니라 문서 정정**이며, FE 가 표를 근거로 미지 값 처리를 짰다면 이미 실 이벤트와 어긋나 있었다. **와이어 계약(엔드포인트·SSE 이벤트·필드·오류 코드) 불변.** 같은 PR 의 I-31 리뷰 추이(`bucket`)·감성 집계는 AI 가 §4.20 을 구간별로 여러 번 호출하는 **소비측 동작**이라 요청 파라미터가 늘지 않는다 — §4.20 개정 없음. |
