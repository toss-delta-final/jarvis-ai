...
- `value` — **[v0.32.8 신설, 이슈 #434] 선택.** 스칼라(문자열 또는 숫자). 예:
  ...
  - **`value` 있음 → 값 범위 제거는 멀티 값 축(`category`·`brand`)에만 적용**한다.
    ...
    - `category` — **[v0.32.8 확정, 이슈 #434 라운드2]** 그 턴에 검색한 카테고리 집합에서
...
> **[v0.32.8, #434]** 값 지정 category 제거는 **그 제거 턴에만** 남은 집합으로 재검색하며,
...
- **[v0.15.26] `field` 허용값 6종** — `category`(카테고리 · 이어폰) / `priceMax`(50,000원 이하) / `priceMin`(30,000원 이상) / `brand`(삼성 · LG) / `ratingMin`(평점 4.5+) / `keyword`(방수). **`conditions`가 내보내는 집합과 `conditionActions`가 지우는 집합은 동일하다.** 종전에는 예시 둘만 있어 허용 집합이 계약에 없었다. **[v0.32.8] 이 6종 집합은 불변이다** — 아래 값당 분리는 칩 **개수**만 바꾼다.
- **[v0.32.8 신설, 이슈 #434] 멀티 값 축(`category`·`brand`)은 값당 칩 1개**로 내보낸다 — 따라서 **같은 `field` 를 가진 칩이 배열에 여러 개 나올 수 있다**(위 예시). **`value` 는 항상 스칼라다.** 라벨 규칙: `category` → `"카테고리 · {값}"`, `brand` → `"{값}"`. 단일 값일 때는 종전 라벨·값과 동일하다.
  - **FE 주의** — 배열 키는 `field` 단독이 아니라 **`(field, value)` 조합**으로 잡아야 한다(같은 field 가 반복되므로).
  - **[v0.32.8 정정]** 종전 `brand` 칩의 `value` 는 리스트(`["삼성","LG"]`)였다 — 이는 §3.1 예시가 스칼라를 명시한 것과 어긋난 드리프트였고, 값당 분리로 자연 해소된다. **`brand` 칩 `value` 리스트→스칼라 정정**(FE 가시 변경).
