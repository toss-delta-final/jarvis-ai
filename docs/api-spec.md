...
| 버전 | 날짜 | 변경 |
|---|---|---|
| v0.30.1 | 2026-08-09 | **[#436] 구현 반영 상태 표기 갱신 — 계약 불변, 서술만.** (1) **I-24~I-28 Spring 구현 완료 반영**(§3.1·§4.12~4.16) — 근거: `jarvis-backend` main 실측, BE PR **#92**(I-24 삭제·I-25 수량 변경)·**#93**(I-26~I-28 찜 3종), 400 code·응답 id 타입 정렬은 PR **#112**. 종전 "🔶 확정 2026-08-05 — Spring 구현 진행 중, 배포 전에는 호출해도 응답하지 않는다"는 사실이 아니었다. (2) **§3.1 FE 수신 상태 정정** — "`ChatAction` 유니온에 신규 8종이 아직 없다"를 "10종 전부 있다(`ActionFailReason` `WISHLIST_ERROR`·헬퍼 `isWishlistMutatingAction()`/`isCartMutatingAction()` 포함)"로 정정 — 근거: `jarvis-frontend` main `08cd2c5` 실측, FE PR **#79**. (3) **AI 수량 변경 2종(`CART_QUANTITY_CHANGED`/`CART_QUANTITY_CHANGE_FAILED`)은 여전히 미구현**임을 §3.1·§4.13에 그대로 유지하고 **AI/Spring/FE 세 축을 별도 bullet으로 분리 서술** — 실 와이어의 상한은 지금 AI 구현이 정한다. (4) **표기 규약에 "상태 마커 근거 병기"·"세 축 분리" 2항목 신설**(#436이 근거 없는 마커의 비용을 실증 — #435 원인 추적이 "FE 수신부가 없어서인가"를 먼저 의심하며 한 라운드를 버렸다). **와이어 계약(엔드포인트·SSE 이벤트·필드·오류 코드) 불변** — 바뀌는 것은 구현 반영 상태 서술뿐이다. |
