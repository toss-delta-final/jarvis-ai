...
> 동기화: **v0.15.8 기준 2026-07-19**.

| 항목 | 값 |
|---|---|
| 문서 버전 | v0.15.8 |
| 작성일 | 2026-07-14 (v0.15.8 개정 2026-07-19) |
| 상태 | draft |
...
| C-6 | **[정정 v0.15.5] 구매 이력 = I-19** | `GET /internal/members/{id}/orders`(§4.7). camelCase·숫자 id(DDL)·`shippingFee` 0. **`status` = 6종**(`PAID/PREPARING/SHIPPING/DELIVERED/CANCELED/RETURNED`, Notion I-19) — 구 representativeStatus 8종은 **O-3(FE `/api/orders`) 오용** 폐기. **`category` 없음**(카테고리 dedup 불가, productId dedup만) | I-19 / Notion·DDL | 🟢 확정(status·타입). 🔴 category 갭 · Notion 페이지 stale(snake/문자열 id) BE 통보 |
