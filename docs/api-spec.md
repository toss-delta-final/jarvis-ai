...
|---|---|---|---|
| I-6 | `/internal/seller/{brandId}/sales` | 매출 시계열 · `from`/`to`(필수)·`granularity`(daily/weekly/monthly/summary). `series[{date,sales,orderCount,salesCount,isAnomaly,deviationPct}]`; `salesCount=SUM(order_item.quantity)`인 **판매 수량**이며 주문 건수 `orderCount`와 다르고 summary에는 없다 | sales_anomaly·conversion·general·recommend·chart |
| I-7 | `/internal/seller/{brandId}/funnel` | 구매전환 퍼널 4단(view→cart→checkout→purchase) · `from`/`to` | conversion·behavior·chart |
| I-13 | `/internal/seller/{brandId}/events` | 행동 이벤트 집계 · 5종 `product_view/add_to_cart/remove_from_cart/checkout_start/purchase_complete`. `rows[].counts`에 `removeFromCart`, `salesQuantity`, 체류 4필드(`medianDwellSeconds/avgDwellSeconds/dwellSampleCount/dwellSource`)를 포함한다; 체류는 1800초 초과·`_timeShifted`·음수를 제외하고 표본 없으면 네 필드 모두 `null` | behavior·conversion |
| I-16 | `/internal/seller/{brandId}/churn` | 이탈 코호트 · `members[{customerLabel,lastActivityAt,sessions30d,preChurnEvent}]`(상한 50). `lastLoginAt` 없음, 코호트 0명은 `churnRate:null`; `returnReasonsTop.count`는 아이템 수, `cancelCount`는 distinct 주문 수 | churn |
| I-14 | `/internal/seller/{brandId}/order-events` | 주문 상태 전이/조회. 목록 `rows[{orderId,orderItemId?,fromStatus,toStatus,actorType,reason,customerLabel,createdAt}]`+`total`; stats=`byStatus/cancelReasonsTop`; groupBy=memberId는 `customerLabel`을 반환한다. customerLabel은 브랜드별 HMAC 사례번호이며 재식별·orderId 대조 유도 금지, item 전이는 `orderItemId`가 있고 주문 전이는 null | sales_anomaly·conversion·churn·abuse·general |
...
| I-8 | `/internal/seller/{brandId}/account-events` | 자사 코호트 계정 이벤트 집계. 기존 전역 `/internal/account-events`는 admin용으로 존치한다. 응답은 `scope:"brand"`, `rows[{ipMasked,distinctMembers,suspiciousMemberCount,eventCount,firstSeen,lastSeen}]`; `isSuspicious/failCount/nullMemberRatio`는 제거됐고 코호트는 I-16과 같은 조인이다 | abuse |
...
**[v0.29.0 정본 동기화]** 성공한 삭제는 Spring이 트랜잭션 커밋 뒤 `remove_from_cart`를 server-side로 적재한다. AI·FE가 같은 이벤트를 별도로 보내지 않는다.
...
| 버전 | 날짜 | 변경 |
|---|---|---|
| v0.29.0 | 2026-08-09 | **[#472] Notion 정본 전수 대조 사본 동기화.** 이슈 #472 본문이 Notion을 정본으로 규정한 근거로, 2026-07-22의 “외부 사본 의존 폐기” 정책을 뒤집어 정본을 Notion으로 되돌렸다. §3.1의 CH-2 `budget` 제외 단서·§6.1 dispatch 목록, §3.5 I-20 `reason`, §3.7·§4.11 P-5 `source` 개명과 `NOT_PERSONALIZED` 미렌더 규칙, §4.2 I-21 필수 필드, §4.4 I-6 `salesCount`·I-8 브랜드 스코프/필드·I-13 이벤트/집계·I-14 식별자/스코프·I-16 churn, §4.5 I-9~I-12 `DELETED`, §4.6 I-1 리뷰 0건/실패 응답, §4.8 I-17 `brandId`, §4.12 I-24 server-side `remove_from_cart`, §4.14~4.16 I-26~I-28 internal 403 제거, §4.18~4.20 I-29~I-31 확정 상태, §3.8~3.9.4 I-32~I-36 즉시 물리 삭제 계약을 정본에 맞췄다. 코드 변경 또는 정본의 미해결 사항은 `docs/api-spec-canonical-audit.md`에 분리했다. |
