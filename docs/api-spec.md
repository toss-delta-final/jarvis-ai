...
> 동기화: **v0.15.3 기준 2026-07-18**.

| 항목 | 값 |
|---|---|
| 문서 버전 | v0.15.3 |
| 작성일 | 2026-07-14 (v0.15.3 개정 2026-07-18) |
| 상태 | draft |
...
| `productId` | string | 대상 상품 식별자(문자열, §2.6). `create`는 없을 수 있음 |
...
| C-5 | **[개정 07/17] `productId` 경계 타입 & `attributes`** | **[BE 확정] 원본 id = 숫자(BIGINT)**. internal(I-1/2/19/21) = 숫자, SSE/FE 경계 = 문자열 정규화(§2.6). `attributes` JSON 구조 미확정 | 결정 9-B / BE ERD v2 | 🟡 우리 코드 경계 변환 반영 · 🔴 SSE 문자열 최종·attributes 구조 |
