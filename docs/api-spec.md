...
| `catalogVersion` | string | ~~예~~ **아니오(폐기 예정)** | **[개정 v0.19.0, C-18] 입력에서 폐기한다.** Spring은 AI 인덱스의 동기화 시점을 알 수 없어 의미 있는 값을 만들 수 없다 — 값 생성이 AI로 넘어갔고 **응답의 `catalogVersion`이 정본**이다. 전환기 동안 Spring이 계속 보내도 깨지지 않도록 선택 필드로 완화했으며 AI는 받기만 하고 버린다. BE 전환 완료 후 제거 |
...
| `catalogVersion` | string | **[신설 v0.19.0, C-18]** 랭킹에 쓰인 **AI 카탈로그 인덱스의 상태 지문**. Spring이 P-5 개인화 캐시 키(`회원 id + 카탈로그 버전 + 알고리즘 버전`, §4.11)에 쓴다 — 인덱스가 갱신되면 값이 바뀌어 캐시가 자동 무효화된다. **불투명 해시**(`cat-` + 16 hex)이며 모델·차원 등 내부 사실을 담지 않는다(§3.7 [HARD]). 요청의 동명 필드를 대체한다 |
...
- **`catalogVersion` 값 생성이 AI로 이관됐다**(C-18, v0.19.0) — 지문 재료는 `(products 행 수, max(updated_at))`이다. I-17 배치가 upsert할 때마다 `updated_at`이 올라가므로 인덱스가 바뀌면 지문도 바뀌고 Spring의 P-5 개인화 캐시가 자동 무효화된다. 모델·차원은 재료에 넣지 않으며 결과를 sha256으로 잘라 **불투명**하게 만든다 — 이 값은 P-5 캐시 키를 거쳐 외부로 흐르므로 읽을 수 있는 형태면 내부 사실이 샌다. 인메모리 스토어는 `updated_at`이 없어 보유 `productId` 집합으로 지문을 만든다(테스트 규모 전용).
- **`reason`을 요청 경로에서 생성하지 않는다 — 미리 만들어 두고 고른다.** 초기 구현은 fast tier LLM 배치 1회로 문장을 만들었으나 **실측에서 예산을 넘겼다**(2026-07-31, gpt-5-nano, 카탈로그 7,220건): 후보 20개 7,970ms · 12개 3,852ms · 6개 2,102ms로 **항목 수에 선형**(출력 토큰 지배, 프롬프트는 2.3k자로 입력 탓이 아님)이라, 5개로 줄여도 2.0s 타임아웃을 5회 중 5회 넘겨 **reason이 한 건도 나오지 않았다.** 지금은 I-17 배치가 **상품당 1회** 만들어 `extras`에 넣어 둔 재료(`situation_tags`·`review_pros`, §4.8)에서 사용자 맥락에 맞는 것을 고른다 — 우선순위는 담기 > 조회 > 프로필 > 상품 고유 폴백이고, 매칭이 없으면 `null`이다(계약상 정상). 결과: **요청 경로 LLM 0회 · 결정적 · 종단 p50 45ms · reason 22/24건**. 문장 재료는 LLM 산출물이고 틀만 규칙이다.
...
| C-18 | **[신규 v0.18.0] I-22 `catalogVersion` 값 생성 주체** | 정본 I-22(§3.7)는 `catalogVersion`을 **Spring이 요청에 실어 보내는** 필수 필드로 규정하는데, 같은 문서가 *"FastAPI는 **자체** 카탈로그 인덱스(I-17로 동기화된 임베딩)로 순위를 매긴다"* 고 한다. **Spring은 AI의 인덱스 버전을 알 수 없다** — 보낼 수 있는 건 Spring 자기 카탈로그의 버전뿐이라, 그 값으로 캐시를 키잉하면 **AI 인덱스가 갱신돼도 캐시가 무효화되지 않는다**(P-5 캐시 키가 "회원 id + 카탈로그 버전 + 알고리즘 버전"이라 직격, §4.11). **[v0.19.0] 값 생성을 AI로 이관 확정**(AI팀 결정 2026-07-31) — 요청 필드는 선택으로 완화(받고 버림), **응답에 `catalogVersion` 신설**이 정본. 지문 재료는 `(행 수, max(updated_at))`이며 불투명 해시라 모델·차원을 노출하지 않는다 | I-22 / 이슈 #148 | 🟡 **AI측 구현·사본 개정 완료. 잔여 = 정본(Notion) 개정 + BE 전환** — 전환은 비파괴(요청 필드를 계속 보내도 동작)이므로 BE 일정에 종속되지 않는다. BE가 응답 값을 캐시 키로 쓰기 시작하면 요청 필드를 제거한다 |
...
| 버전 | 날짜 | 변경 |
|---|---|---|
| v0.19.0 | 2026-07-31 | **[#148, C-18] I-22 `catalogVersion` 값 생성 주체를 Spring → AI로 이관.** 정본은 Spring이 요청에 실어 보내게 규정했으나 **Spring은 AI 카탈로그 인덱스의 동기화 시점을 알 수 없어** 의미 있는 값을 만들 수 없었다. 그 값으로 P-5 개인화 캐시를 키잉하면(`회원 id + 카탈로그 버전 + 알고리즘 버전`, §4.11) **AI 인덱스가 갱신돼도 캐시가 무효화되지 않아** 낡은 추천이 10분간 서빙된다. (1) **응답에 `catalogVersion` 신설** — 랭킹에 쓰인 인덱스의 상태 지문. 재료는 `(products 행 수, max(updated_at))`이고 I-17 배치가 upsert할 때마다 값이 바뀐다. **불투명 해시**(`cat-` + 16 hex)로 만들어 모델·차원 등 내부 사실을 담지 않는다 — 이 값은 Spring 캐시 키를 거쳐 외부로 흐르므로 §3.7 [HARD] provenance 비노출이 적용된다. (2) **요청 필드는 필수 → 선택으로 완화**(폐기 예정) — Spring이 전환 전까지 계속 보내도 깨지지 않고, AI는 받기만 하고 버린다. **전환 전 과정이 비파괴**라 BE 일정에 종속되지 않는다. BE가 응답 값을 쓰기 시작하면 요청 필드를 제거하는 개정이 따른다. (3) `ArtifactStore` 공유 계약에 `catalog_version()`을 추가하고 양쪽 구현체를 함께 고쳤다. **🔴 잔여: 정본(Notion) I-22 개정 + BE 통보.** |
