...
**(1) `progress`** — 진행 단계 알림 (0회 이상). **[확정 v0.21.0, 이슈 #289, 정본 합의·등재 2026-08-05 / 다회 emit·어휘 확장 v0.27.0, 이슈 #396, 정본 개정 2026-08-06 / retrying 추가 v0.30.1, 이슈 #406]**
...
- **AI 구현 상태**: config `progress_events_enabled` **기본 `true`**(#396, 2026-08-06 FE 구현 완료로 해제 — 계약 등재 v0.21.0·운영 기동 가드 제거)로 실제 와이어에 나가며, stage 어휘·다회 emit은 v0.27.0부터 실제로 나가고 #406(v0.30.1)이 `retrying`을 더해 8종이 됐다. `retrying`은 `spring_max_retries > 0`일 때만 나가며 현재 기본값은 #394 한시 조치로 `0`이라 실제 와이어에는 나가지 않는다. 되돌리려면 `PROGRESS_EVENTS_ENABLED=false`.
...
| 버전 | 날짜 | 변경 |
|---|---|---|
| v0.30.1 | 2026-08-09 | **[#406] 구매자 `progress`에 `retrying` stage를 추가했다.** I-1 검색이 재시도 가능한 실패 뒤 실제 다음 시도에 들어갈 때만 본검색 드레인이 즉시 frame을 내며, fan-out 콜백이 여러 번이어도 턴당 1회로 제한한다. 기본 `spring_max_retries=0`(#394 한시 조치)에서는 해당 task·queue 경로를 전혀 타지 않아 기존 인라인 await 경로의 취소·ContextVar·trace 의미가 바이트 단위로 유지된다. 구제 체인·자동 완화 probe·`_run_search_unfiltered`에는 이번 관측을 배선하지 않는다. |
