...
- **[확정] 필수 클레임**:
  ...
  - `brandId` — **`role == "seller"`일 때 필수**(그 외 없음). 판매자 브랜드 식별자. 판매자 집계 역호출(§4.4)이 `{brandId}` path를 요구하므로 AI는 이를 **검증된 토큰 클레임에서만** 얻는다(요청 본문·발화에서 받지 않음 — IDOR 방지). Spring 발급 계약(🔴 C-1).
...
**(2) 상품 수정/등록/삭제 흐름 — [개정 v0.9.0, BE 문서 채택]**: 판매자 `product_agent`가 상품 쓰기를 **AI가 Spring internal API로 직접 수행**한다(구 "FE가 S-3 PATCH로 반영" 모델 **폐기**). 흐름: `token`(안내/근거) → **`draft`**(개정안 diff, 정확히 1회) → **HITL 승인 대기** → 승인 시 AI가 Spring 쓰기 호출(등록 I-10 / 수정 I-11 / 삭제 I-12, §4.5) → `token`(결과) → `done`.
- **읽기**: 대상 확인은 **I-9 자사 상품 목록**(§4.5)으로 조회(구 "I-7 상세 읽기"는 BE에서 I-7=퍼널이라 폐기, I-9로 대체).
- **HITL(Human-in-the-loop)**: 파괴적/민감 작업(**삭제 I-12** 필수, 수정도 권장)은 **LangGraph interrupt로 멈추고 판매자 승인을 받은 뒤에만** 실행한다. 승인/거절은 SSE **interrupt/resume**로 처리(판매자 SPEC에서 이벤트명 확정 🔴). "1번 적용해줘"처럼 판매자가 명시 승인해야 쓰기가 나간다.
- **soft delete**: 삭제는 물리 삭제 없이 `status=HIDDEN` 전환(§4.5 I-12). HITL(그래프) + soft delete(데이터) 이중 방어.
...
- **[HARD, 개정 v0.9.0] 반영은 AI가 Spring internal API로 직접 수행** — 판매자 승인(HITL) 후 AI가 **I-11 PATCH(수정)/I-10 POST(등록)/I-12 DELETE(삭제)**(§4.5, `X-Internal-Token`+`{brandId}`)를 호출한다. **구 "FE가 본인 JWT로 S-3 PATCH" 모델은 폐기**(BE가 판매자 쓰기를 internal API로 제공). 반영 결과는 `token`으로 안내.
- **[HARD] 대화 발화는 동의로 취급하지 않는다** — 채팅 발화만으로 쓰기가 나가지 않는다. 반영의 유일한 경로는 **HITL 승인**(interrupt/resume, 판매자의 명시 `[적용]`/"적용해줘")이다. 이 원칙은 유지되며, 구현이 "FE S-3 PATCH"에서 "AI 직접 쓰기 + HITL 게이트"로 바뀐 것이다.
...
| # | 항목 | 현재 상태 | 소유/근거 | 상태 |
|---|---|---|---|---|
| C-1 | **인증(auth)** | **확정**: RS256 + JWKS(Spring JWKS 노출, AI 로컬 검증), `401` 통일, 클레임 `sub`+`role`(member/guest/seller)+`exp`/`iat`/`iss`, `/seller/chat` role=seller(403)(§2.3). **[v0.8.0] `brandId` 클레임(role=seller 시 필수)** 추가 — 판매자 집계가 `{brandId}` path 요구(§2.6·§4.4) | 결정 19 / 2026-07-14 세션 확정 | 🔴 잔여 — **role 값 집합·토큰 TTL·`brandId` 클레임 발급 수용** |
