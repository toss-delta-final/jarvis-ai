...
- **AI 구현 상태**: config `progress_events_enabled` **기본 `true`**(#396, 2026-08-06 FE 구현 완료로 해제 — 계약 등재 v0.21.0·운영 기동 가드 제거)로 실제 와이어에 나간다. 되돌리려면 `PROGRESS_EVENTS_ENABLED=false`.
...
- **복구·초기 구축 [v0.26.3 개정 — #325, 정밀화]**: 격리 대상은 실패 **종류**로 가른다 — 비율은 poison 단건과 광역 장애를 구별하는 대리 지표일 뿐이고, 운영 증분 페이지처럼 소량 표본에서는 그 대리가 무너진다. **1선(구조)**: `ON_SALE` 단건 실패 중 enrichment(LLM 호출+파싱) 단계의 내용 실패만 `enrichment_item_attempts`(기본 2)회 재시도 후 dead-letter 로그로 격리하고 나머지 항목을 계속 처리하며, 페이지가 정상 종료되면 커서를 전진시킨다 — 종전 규약(실패 시 페이지 전체 미전진)에서는 실패 항목 1개가 뒤따르는 모든 변경을 영구 차단했다(head-of-line blocking, 운영 정지 #325). 임베딩·스토어 등 항목 내용과 무관한 인프라 실패는 격리하지 않고 그대로 전파해 그 페이지 커서를 전진시키지 않는다(자연 복구, 동일 커서 재개) — 페이지 크기와 무관하게 성립한다. **1선(타임아웃)**: enrichment 단계 실패라도 재시도 소진 후 그 예외(원인 체인 포함)가 타임아웃 계열이면 LLM API 자체가 응답하지 않는 광역 장애로 보고 마찬가지로 격리하지 않고 전파한다. 격리 항목은 그 주기에 갱신되지 않으나 조용한 누락이 아니라 dead-letter ERROR 로그와 배치 결과 `failed` 카운트로 드러나며, 복구는 `run_batch --full`(전체 재구축)로 한다. **2선(비율 가드, 방어)**: 페이지 `ON_SALE` 표본이 `artifacts_batch_failure_min_sample`(기본 5) 이상이고 실패 비율이 `artifacts_batch_failure_ratio_threshold`(기본 0.5) 이상이면 그 페이지는 커서를 전진시키지 않고 중단한다 — 1선을 통과한 뒤에도 남는 경우(인프라는 멀쩡한데 enrichment 결과 자체가 대량으로 깨지는 경우, 예: 프롬프트 회귀)를 잡는 방어선이다. 표본이 `min_sample` 미만이면 비율 판정을 생략하고 격리+전진한다 — 소량 표본 판정 불능은 1선이 이미 광역 장애를 걸러낸 뒤라 안전하다. `HIDDEN` 삭제 실패는 격리하지 않고 그대로 전파해 커서를 전진시키지 않는다(항목별 ack/DLQ 계약이 없어 삭제 이벤트를 skip-전진하면 유령 상품이 영구히 남는다). 계약 위반(`status` 미정의 값)은 바로 위 불릿대로 종전과 같이 페이지 전체 fail-closed — 불변. 초기 전체 구축도 커서 0부터 같은 API·같은 복구 규약으로 처리. **이 문단은 AI측 소비 동작 서술이며, I-17 요청 파라미터·응답 필드·오류 코드(와이어 계약)는 불변이다.**
...
| 버전 | 날짜 | 변경 |
|---|---|---|
| v0.26.3 | 2026-08-06 | **[#325] §4.8 I-17 배치 복구 규약 개정 — 운영 정지 대응(PR #399 리뷰 대응으로 정밀화).** 운영 fast tier(gpt-5-nano, reasoning 모델)에서 enrichment 호출이 하드코딩 `max_tokens=600`을 그대로 썼는데, `reasoning_tokens`가 그 600 전량을 소진해 본문 0자 → `openai.LengthFinishReasonError`로 5분 주기 증분 배치가 매번 정지했다. 그 상태에서 문제 상품 1건이 페이지에 남으면 종전 복구 규약(실패 시 페이지 전체 커서 미전진)이 그 1건을 큐 머리에 영구히 고정해(head-of-line blocking) 뒤따르는 모든 변경이 처리되지 못했다. **새 복구 규약(실패 종류로 가른다)**: 격리 후보는 enrichment(LLM 호출+파싱) 단계의 내용 실패로 한정한다 — 재시도 상한(`enrichment_item_attempts`, 기본 2) 후 dead-letter 기록·격리하고 커서는 전진한다. 임베딩·스토어 실패, 그리고 enrichment 재시도 소진 후 타임아웃 계열로 판정된 실패는 항목 내용과 무관한 광역 장애로 보아 격리하지 않고 그대로 전파해 종전대로 페이지 실패(커서 미전진, 자연 복구) — 페이지 크기(운영 증분 페이지의 소량 표본 포함)와 무관하게 성립한다. 페이지 `ON_SALE` 표본·실패율이 임계(`artifacts_batch_failure_min_sample` 기본 5 · `artifacts_batch_failure_ratio_threshold` 기본 0.5) 이상이면 2선 방어로 마찬가지로 페이지 실패(커서 미전진) — 1선(구조·타임아웃)을 통과한 뒤에도 남는 "인프라는 멀쩡한데 enrichment 결과가 대량으로 깨지는" 경우를 잡는다. `HIDDEN` 삭제 실패·계약 위반(`status` 미정의 값)은 종전대로 페이지 실패(불변). 함께 enrichment 출력 상한(`enrichment_max_tokens` 기본 2048)·reasoning effort(`enrichment_reasoning_effort` 기본 minimal)를 하드코딩에서 config 튜너블로 이관했다. **I-17 와이어 계약(요청 파라미터·응답 필드·오류 코드) 불변** — 본 개정은 AI측 소비 동작 서술이다. |
