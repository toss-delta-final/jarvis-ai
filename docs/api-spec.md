...
| I-13 | `/internal/seller/{brandId}/events` | 행동 이벤트 집계(`behavior_events`) · `from`/`to`·`eventType`(product_view/add_to_cart/checkout_start/purchase_complete, **CSV 직렬화** — BE는 `String eventType` + comma split, 반복 쿼리 아님 v0.17.4)·`productId`·`groupBy`(product/eventType/date). `rows[{productId,counts{4종},viewToCartRate,uniqueVisitors}]` — **rows 정렬 = 활동량(counts 4종 합) 내림차순, 동률 시 productId 오름차순**(BE `eventsByProduct` 실측, v0.17.4). ⚠️ **purchaseComplete 는 주문 기준 집계**(`order_item × product × brand`, PAID·`paid_at`, `COUNT(DISTINCT order_id)`) — I-7 4단(purchase)과 같은 정본이며 **이벤트 유실과 무관**하고 과거 구간도 소급 복구된다. 건수이지 수량이 아니고(한 주문에 같은 상품 여러 개여도 1), 상품별 값의 합이 `groupBy=eventType` 합계보다 클 수 있으며(한 주문에 자사 상품 여러 종), 부분 취소·반품 반영으로 과거 기간 재조회 시 값이 줄 수 있고, 조회·담기 없이 구매만 있는 상품도 `rows` 에 등장한다. 구 "FE 미귀속으로 0 집계 가능 · 권위는 I-6/I-7/I-14"(v0.17.4)는 **jarvis-backend#62 배포로 폐기**(v0.29.2) — **LLM팀 본문 재작성 반영(v0.15.1)** | behavior·conversion |
...
| 버전 | 날짜 | 변경 |
|---|---|---|
| v0.29.3 | 2026-08-09 | **[#496] §4.20 I-31 `sort` 어휘 개명 사본 반영 — `rating` → `ratingAsc`(2026-08-06 BE 협의 확정, 구 안 폐기).** 본 사본이 v0.25.0 등재 당시의 `rating`(낮은 순 고정, 2026-08-04 안)에 멈춰 있었고, 그 문구가 `get_reviews` 도구 docstring 과 `SpringClient.get_reviews` docstring 으로 전파돼 **워커가 폐기된 값을 그대로 호출**하고 있었다("평점 낮은 리뷰 뭐가 문제야?" → `sort="rating"` → `400 VALIDATION_ERROR` → 정상 질문에 시스템 장애 보고). 개명 근거는 P-3 의 `rating` 이 **높은 순**이라 같은 이름·반대 방향이 사고를 부른다는 것이며, 어휘는 `latest`(기본)·`ratingAsc` 둘뿐이고 `ratingDesc` 는 없다(높은 별점은 `rating=4,5` 필터). AI 소비측은 두 docstring 을 갱신하고 `_REVIEW_SORT` 화이트리스트로 어휘 밖 값을 Spring 왕복 전에 거른다(`_ACCOUNT_EVENTS_GROUP_BY` 와 같은 패턴). **신설 협의가 아니라 사본 동기화**(v0.23.1·v0.28.4·v0.29.2 와 같은 유형) — **와이어 계약(엔드포인트·파라미터 이름·응답 필드·오류 코드) 불변**. |
