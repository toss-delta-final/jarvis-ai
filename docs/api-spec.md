...
| `brandName` | string[] \| null | 아니오 | **다중 브랜드**(#100 P1, 방법 D). **[2026-07-27 정본 확정, BE 구현 완료 — 신설 협의 아님]** 반복 파라미터(`brandName=A&brandName=B`)로 전량 전송 → BE `WHERE brand IN (...)` OR 매칭 — 지정된 브랜드 중 **하나라도 일치**하면 후보에 포함. 단일은 값 1개(기존과 동일), 미전송 시 브랜드 필터 없음. 존재하지 않는 브랜드명은 무시하고 나머지로 검색하며 **전부 미존재일 때만 0건**을 반환한다(하나 잘못 넣었다고 추천 전체가 죽지 않도록). 아래 노트 참조 |
...
- **[2026-07-27 정본 확정, BE 구현 완료 — 신설 협의 아님, api-spec 사본 동기화 2026-08-08] `brandName` 은 리스트**: 반복 파라미터로 복수 브랜드 동시 지정 — 지정된 브랜드 중 **하나라도 일치**하면 후보에 포함. 존재하지 않는 브랜드명은 무시하고 나머지로 검색하며, **전부 미존재일 때만 0건**을 반환한다(하나 잘못 넣었다고 추천 전체가 죽지 않도록). BE 구현 확인: `InternalProductController#search`의 `@RequestParam List<String> brandName`(`origin/main 4c6b287`).
- 🔴 **[정본 미해결 — `brandName` 부분 매칭, AI 팀 회신 필요, api-spec 사본 동기화 2026-08-08]** 정본(Notion I-1)은 위 규약 끝에 **"⚠️ 부분 매칭 규칙은 LLM 팀 확인 필요"**를 미해결로 남겨 뒀다 — 정본이 우리에게 물어본 항목인데 이 사본에는 그 사실 자체가 없었다. **부분 매칭을 켤지(`color`처럼 `그레이`가 `다크그레이`를 잡는 방식, 위 참조) 정확 일치로 둘지는 사람 결정 대기다 — 이 사본은 답을 정하지 않는다.** 참고 실측(2026-08-08): 브랜드 2,368개 중 영문 포함 496·한영 혼합 209·공백 포함 173, 그리고 **다른 브랜드명의 접두사인 이름이 179개** — 부분 매칭을 켜면 `금강`이 `금강제화`를 끌어온다.
...
| `[].rating` | number | **조회 시 집계**(저장 avg 없음, DDL D9) — **AI 계산용(비표시, #100 P0)**: 평점 사후필터·rerank 신호. 표시는 CH-5. **[정본 규약, api-spec 사본 동기화 2026-08-08] 리뷰 0건이면 `rating: 0.0`**(I-17과 동일 규약) — `null`이 아니다. AI 스키마는 `float \| None`이라 BE가 필드를 안 실어도(`null`/미전송) 죽지 않는 관대 수신은 그대로 유지한다(정본 규약과 별개의 방어) |
| `[].reviewCount` | int | **조회 시 집계**(리뷰 개수) — **AI 계산용(비표시, #171)**: `rating` 과 짝지어 **"리뷰가 없어 rating=0"(reviewCount=0, 데이터 부재)** 와 **"리뷰가 있고 하한 미달"(reviewCount>0)** 를 가른다. **[정본 규약, api-spec 사본 동기화 2026-08-08] 리뷰 0건이면 `reviewCount: 0`**(I-17과 동일 규약) — `null`이 아니다. `null`/미전송이면 rating 이 지배(구 동작 폴백, AI 스키마 `int \| None` 관대 수신은 유지). 표시용 리뷰수는 여전히 경로 B(§4.3) |
...
#### 실패 응답 (api-spec 사본 동기화, 2026-08-08)

사본 §4.6에 실패 응답표가 통째로 없던 누락을 정본에서 그대로 옮긴다 — 신설 협의가 아니다.

| HTTP | code | 조건 |
|---|---|---|
| 400 | `VALIDATION_ERROR`(`fields` 없음) | `minPrice`·`maxPrice`가 숫자가 아님(`?minPrice=오만원`) — 타입 변환 실패. **AI는 LLM이 추출한 가격을 숫자로 정규화해 보내야 한다** |
| 401 | `INTERNAL_TOKEN_INVALID` | `X-Internal-Token` 헤더가 없거나 토큰값 불일치 — 상수시간 비교라 타이밍 공격이 통하지 않는다 |
| 401 | `INTERNAL_TOKEN_INVALID` | **서버에 토큰이 설정되지 않음(fail-closed)** — 환경변수가 비어 있으면 모든 요청을 거부한다. **배포 직후 모든 콜백이 401이면 먼저 이걸 의심할 것** |
| 500 | `INTERNAL_ERROR` | 서버 내부 오류(DB 장애 등). AI는 후보 없이 답변을 이어간다 |

#### 실패가 아닌 것 — 0건과 모순된 조건 (api-spec 사본 동기화, 2026-08-08)

전부 `200`이다 — 아래는 오류가 아니라 정상 응답이다.

| 입력 | 응답 | 이유 |
|---|---|---|
| 조건에 맞는 상품 0건 | `200` · `data: []` | 빈 배열은 정상 결과다. AI는 조건을 완화해 재질의하거나 I-3(§4.17)로 폴백한다 |
| 존재하지 않는 `categoryName` | `200` · `data: []` | 404가 아니다(잎 이름 불일치로 0건이 되는 사례는 위 `categoryName` 노트 참조) |
| 존재하지 않는 `brandName` 섞임 | `200` · 나머지로 검색 | 하나라도 유효하면 그것들로 검색. **전부 미존재일 때만 0건**(위 `brandName` 행·노트와 동일 규칙) |
| `minPrice > maxPrice` | `200` · `data: []` | 모순 조건도 검증하지 않는다 — SQL이 자연히 0건을 낸다 |
| 모든 파라미터 생략 | `200` · 판매 중 전체 | **서버 가드가 없다** — 정형조건 없는 요청 차단은 AI 단 책임(2026-07-27 상한 폐지). 정본 문구·AI 미구현 경위는 이미 §4.17에 인용돼 있어 여기서 반복하지 않는다 |
...
| 버전 | 날짜 | 변경 |
|---|---|---|
| v0.28.5 | 2026-08-08 | **[api-spec §4.6 사본 동기화, 사람 승인 2026-08-08] 정본(Notion I-1) 대조로 드러난 사본 드리프트 잔여 3건을 옮겼다 — 신설 협의가 아니다.** (1) **리뷰 0건 규약**: `[].rating`·`[].reviewCount` 행에 "리뷰 0건이면 `rating: 0.0`·`reviewCount: 0`"(I-17과 동일 규약)이라는 정본 확정을 추가했다 — 사본은 종전에 `null`/미전송이면 rating이 지배한다고만 적어 null과 0을 둘 다 상정했는데, `null` 관대 수신 서술(AI 스키마가 `float\|None`·`int\|None`이라 BE가 필드를 안 실어도 죽지 않는 방어)은 지우지 않고 정본 규약(0 확정)과 나란히 적었다. (2) **I-1 실패 응답표 신설**: 사본 §4.6에 실패 응답표가 0건이던 것을 정본 표(400 `VALIDATION_ERROR`·401 `INTERNAL_TOKEN_INVALID`×2·500 `INTERNAL_ERROR`) 그대로 옮기고, 「실패가 아닌 것 — 0건과 모순된 조건」(전부 200 — 0건 정상·존재하지 않는 categoryName·brandName 일부 미존재·`minPrice>maxPrice`·전 파라미터 생략) 표도 함께 옮겼다. 마지막 행("모든 파라미터 생략")은 이미 §4.17이 인용해 둔 정본 문구를 반복하지 않고 그쪽을 가리켰다(중복 흡수). (3) **`brandName` 두 가지**: (a) 행 끝의 낡은 "BE 배열 수용 협의 대상" 문구를 걷어내고 2026-07-27 정본 확정·BE 구현 완료(`InternalProductController#search`의 `@RequestParam List<String> brandName`, `origin/main 4c6b287`) 사실로 교체 + "전부 미존재일 때만 0건" 규칙을 명시했다. (b) 정본이 미해결로 남긴 "⚠️ 부분 매칭 규칙은 LLM 팀 확인 필요"가 사본에 존재조차 하지 않던 것을 🔴 **정본 미해결 · AI 팀 회신 필요**로 등재했다 — 참고 실측(브랜드 2,368개 중 영문 포함 496·한영 혼합 209·공백 포함 173·다른 브랜드명의 접두사인 이름 179개)만 병기하고 **답은 정하지 않는다**(사람 결정 대기). `app/`·`tests/`·`scripts/` 무변경, 계약 자체는 바뀌지 않는다(사본 옮겨 적기). |
