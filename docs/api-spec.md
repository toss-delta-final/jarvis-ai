...
| ↳ **I-1 검색만 재시도 1회** (v0.19.2) | **3s × 2 = 6s** (config `SPRING_MAX_RETRIES`) | **BE 관측 포인트** — 한 턴에 같은 검색 요청이 최대 2번 온다. 재시도 대상은 **타임아웃·연결 오류·응답 중단(서버가 응답 도중 연결 종료)·5xx 뿐**이며 4xx는 즉시 실패한다(다시 불러도 같은 결과). I-1은 GET·멱등이라 안전하고, **비멱등 호출(I-2 담기 등)에는 재시도를 걸지 않는다.** 재시도 총량은 first-token 10s 미만이어야 하며 기동 시점에 검증한다 |
