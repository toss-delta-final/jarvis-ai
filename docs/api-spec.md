  ...
  "edges": [
    {
      ...
      "predicate": "prefers",
      "object": { "nodeId": "priceBand:30000-50000", "type": "priceBand", "label": "3~5만원대" },
      "editable": true,
      "challenged": false
    },
    {
...
| `edges[].object.label` | string | 사람이 읽는 이름. 최대 길이 = config `profile_graph_label_max_chars` |
...
- **[신설 v0.32.7, #360] `type`+`label` 형태의 정규화가 실패하면 `400`이다.** 통제 어휘에 없는 라벨(카탈로그에 없는 카테고리명)·형식을 못 맞춘 `priceBand`·`ratingBand`가 여기 해당한다. **가까운 노드로 추측해 스냅하지 않는다** — 사용자가 지목하지 않은 대상에 취향이 붙으면 그것은 수정이 아니라 오염이고, 배치 경로와 다른 `nodeId`가 만들어지면 같은 취향이 두 `edgeId`를 얻어 **재파생 차단 표식을 우회한다**(`SPEC-PROFILE-GRAPH-149` REQ-PGRAPH-010). 정규화는 사전·파서 조회와 카탈로그 **exact 조회**까지만 쓰며 임베딩 근접 매칭을 부르지 않는다([HARD] 요청 경로 LLM 0회 — §3.8 규약과 동일).
