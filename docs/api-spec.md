...
- **개인화 중지 중에도 200 + 전체 그래프를 반환한다.** 중지는 "쓰지 않는다"이지 "숨긴다"가 아니다 — 보존된 데이터를 검토하고 지우려면 볼 수 있어야 한다. **[명시 v0.32.7, #360] `markdown`도 그대로 내려간다** — 중지 시 `null`이 되는 것은 §3.4이고 본 절이 아니다(§3.9.5 규약이 §3.4를 지목한다). 취향 관리 화면은 중지 상태에서도 설명 문단이 필요하다.
...
| 버전 | 날짜 | 변경 |
|---|---|---|
| v0.32.8 | 2026-08-10 | **[#360] §2.9 (c) 에 그래프 표면 타임아웃 행 2개와 구현 직후 실측표를 등재한다 — 와이어 계약 불변.** 조회 2s(`profile_graph_read_budget_s`) · 변경 3s(`profile_graph_write_budget_s`)이고 **Spring 타임아웃이 각각 1s 길다**(3s·4s) — 그 부등식이 깨지면 Spring 이 먼저 끊어 AI 의 `504 UPSTREAM_TIMEOUT` 이 관측 불가능한 죽은 계약이 된다. 실측(2026-08-10, n=40/엔드포인트 · edge 20건 문서 · 단일 동시성): I-32 p95 **47.8ms** · I-37 **46.0ms** · I-33 **55.0ms** · I-34 **92.4ms** · I-36 **128.7ms** 로 예산의 1.5~4.3% 다. **측정 한계를 함께 적었다** — 실 pg-profile 에 ASGI 직결로 재 uvicorn·소켓·실 네트워크가 빠졌으므로 이 수치는 **하한**이고, 그래도 여유가 20~65배라 상한을 조정할 근거는 아직 없다. 함께 **§3.8 구현 노트 2건을 해소 표기로 전환** — *"본 절과 §3.9는 구현이 없다(#150)"* 는 #356·#358·#360 으로 거짓이 됐고, 노트 5 의 *"예산은 제안이며 실측이 아니다 — §2.9 (c) 에 행을 추가하지 않은 이유가 이것이다"* 는 그 사유가 소멸했다. |
| v0.32.7 | 2026-08-10 | **[#360] §3.8·§3.9 구현 착수가 드러낸 빈칸을 채운다 — 와이어 계약(엔드포인트·필드·오류 코드)은 불변이다.** 추가·삭제된 필드도 코드도 없고, 이미 있는 것들 사이의 **적용 조건과 우선순위**를 명시한다. 다섯 항목 모두 노션 정본 `I-32`·`I-33`·`M-11`·`M-12`를 대조해 **어디에도 답이 없음을 확인한 뒤** 정했다. (1) **§3.9.1 두 `409`의 우선순위** — 스테일한 `If-Match`로 `purchased` edge를 고치면 `PROFILE_EDGE_NOT_EDITABLE`이 이긴다. 반대로 하면 FE가 규약대로 재조회 후 재시도하고 **그 재시도가 결국 같은 코드를 받아** 왕복이 낭비된다. 정본 `M-12`가 두 코드의 FE 대응을 "재조회 후 재시도" vs "재시도해도 소용없다"로 갈라 적어 둔 것이 근거다. (2) **§3.9.1 `type`+`label` 정규화 실패는 `400`** — 통제 어휘 밖 라벨을 가까운 노드로 스냅하면 배치 경로와 다른 `nodeId`가 만들어져 **같은 취향이 두 `edgeId`를 얻고 재파생 차단 표식을 우회한다**(REQ-PGRAPH-010). 구 계약의 `400` 목록은 `nodeId` 형태만 다뤘다. (3) **§3.9.1 부분 변경의 기본값 출처** — 생략한 쪽은 대상 edge의 현재 값을 쓰되 **잠금 아래에서 읽어야** 한다. 미리 읽어 채우면 재전송이 이미 사라진 edge를 찾다 실패해 §7.2의 *"재전송 판정이 `404`보다 앞"* 이 깨진다. (4) **§3.8 투영 대상 = `active`** — 정본 `I-32`가 규약으로 명시했는데 본 절에 사본이 없었다. 같은 대조에서 **드리프트 1건도 잡았다** — `SPEC-PROFILE-GRAPH-149` REQ-PGRAPH-021이 `active` **+ `promoted`** 로 적혀 정본과 어긋나 있었고(SPEC v0.3.3에서 정정), 그 조건을 그대로 구현했다면 **화면에 안 보이는데 추천에는 쓰이는 취향**이 생겨 정본이 요구한 *"요약 생성이 같은 규칙을 쓴다"* 가 깨졌다. `promoted`는 병합 엔진 내부 히스테리시스 값이며 소비처가 0건이다(실측). (5) **§3.8 `markdown`은 중지 중에도 내려간다** — §3.9.5가 지목한 `null` 대상은 §3.4이며 본 절이 아니다. 함께 **구현 노트 2·3을 해소 표기로 전환**한다: `404` 기본 항목을 매핑에 등재했고(1번 `409` 함정은 그대로 유효), #358이 `revision` CAS·멱등 원장·저널·감사를 깔아 *"낙관적 동시성 기계장치가 저장소에 전혀 없다"* 가 더는 사실이 아니다. `412`는 계속 채택하지 않는다. |
