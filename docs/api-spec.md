...
> 동기화: **v0.15.15 기준 2026-07-21**.

| 항목 | 값 |
|---|---|
| 문서 버전 | v0.15.15 |
| 작성일 | 2026-07-14 (v0.15.15 개정 2026-07-21) |
| 상태 | draft |
...
- **[v0.15.15, 이슈 #62/#64]** session-end(§3.5)은 별도 `eventId` 필드가 없다 — 멱등은 **그 통지가 저장할 세션 대화 내용에서 파생한 키**로 판정한다. session-end 는 세션의 최종 종료가 아니라 프로필 **저장 체크포인트**여서 한 `sessionId` 에 여러 번 정당하게 오므로(§3.5), `(userId, sessionId)` 고정 키로 잡으면 새 대화가 쌓인 정당한 재저장을 중복으로 오인해 유실한다. 내용 기반 키는 **같은 내용 재전송(at-least-once)만** 중복 처리하고, 새 내용 체크포인트는 정상 처리한다.
...
- **`reason` 허용값(v0.15.5 재편)**: `PRODUCT_NOT_FOUND` / `CART_ERROR` **2종**. ~~`OUT_OF_STOCK`~~은 **폐기(v0.15.5)** — 담기 시 재고검증 없음(재고 차감=주문 시점, 품절=stock 0; BE I-2·C-2 오류코드에 부재, §4.1). ~~`GUEST_NOT_ALLOWED`~~는 **폐기** — 게스트도 담기 허용(v0.6.0, 결정 8 개정 필요 §8 항목 7).
...
#### 요청 (Request) — **[v0.15.15 확정, 이슈 #62]** BE 실측 페이로드 정렬
...
| 필드 | 타입 | 필수 | 설명 |
|---|---|---|---|
| `userId` | number(BIGINT) | 예 | 세션 소유 회원 식별자(JWT `sub`와 동종 숫자 id, 프로필 스코프) |
| `sessionId` | string | 예 | 종료된 세션 식별자(불투명 스레드 키). 최대 길이 = config `chat_key_max_chars`(§2.6) |
| `reason` | string | 아니오 | 종료 사유(`logout`/`tabClose`/`inactivityTimeout`/`newConversation` 등) — **enum 미강제·방어적 수용** |

> **[v0.15.15 변경 — 이슈 #62]** 구 초안의 `eventId`·`endedAt`를 **제거**하고 `userId`를 **string → number(BIGINT)**로 정정했다(BE 실측 payload 정합). 멱등 키는 별도 필드 대신 **`userId`+`sessionId` 파생 복합키**(§2.7)로 전환한다. 종전 스키마와 불일치해 `POST /events/session-end`가 상시 `400`을 반환하던 문제를 해소한다.

- **저장 체크포인트(비종결)**: session-end 는 세션의 최종 종료가 아니라 프로필 저장을 앞당기는 신호다 — 한 `sessionId` 가 살아있는 동안(예: tabClose 후 재활동 → inactivityTimeout) **여러 번 정당하게** 올 수 있다.
- **best-effort**: 통지 유실 시 다음 sleep-time 배치의 미처리 스레드 스캔이 회수(SPEC-PROFILE-001 REQ-PROF-050/051).
- **멱등**: 멱등 키는 **저장 대상 대화 내용에서 파생**한다(§2.7) — 같은 내용 재수신(at-least-once)은 델타·프로필 중복 처리 안 하고(REQ-PROF-051), 새 대화가 쌓인 재통지는 새 저장으로 처리한다. 저장할 새 내용이 없으면 no-op.
- 응답: `202 Accepted`(신규 `{"status":"accepted"}` / 중복 `{"status":"duplicate"}`). `userId`·`sessionId` 누락 또는 타입 오류는 `400`(§2.5 봉투).
...
> **잔여 협의(C-3)** — 대부분 해소, 서비스 토큰만 남음: (1) ~~재고 오류 `OUT_OF_STOCK`~~ **폐기(v0.15.5)** — 담기 재고검증 없음(재고 차감=주문 시점, 품절=stock 0, BE 오류코드 부재). (2) ~~`CART_OPTION_REQUIRED` options 스키마~~ **해소(BE 2026-07-18)** — `error.detail.options: [{optionId, name, extraPrice}]`. (3) ~~`productId` 타입~~ **해소(v0.15.3)** — 숫자 BIGINT(§2.6). (4) 🔴 **서비스 토큰(`X-Internal-Token`) 발급·교환 방식** — 유일 잔여.
...
### 4.8 AI 생성물 갱신 배치 (bulk export pull) — 🟡 골격 BE 확정(2026-07-18)·잔여 3건 저영향 [v0.5.1 신설, v0.15.12 갱신]
...
{
  "items": [
    { "productId": 10293, "status": "ACTIVE", "updatedAt": "2026-07-15T10:00:00Z", "name": "여행용 방수 파우치", "description": "…", "category": "여행용품/보안용품", "brand": "트래블메이트", "attributes": { "방수": true, "용량": "2L" } },
    { "productId": 10877, "status": "DELISTED", "updatedAt": "2026-07-15T10:01:00Z" }
  ],
...
- **흐름**: AI 배치 잡이 주기적으로 변경분 조회(커서 기반, `hasMore` 루프로 페이지 소진) → `DELISTED`는 AI 생성물 삭제/비활성 → 나머지는 enrichment(Haiku, Layer 2 속성·상황 태그 추출) → `search_doc` 조립 → 임베딩(**Google `gemini-embedding-001` API**, 결정 6 개정 2026-07-20 — 셀프호스트 torch 폐기) → AI Postgres upsert. **상품 원본 컬럼은 저장하지 않는다** — 산출물만 저장.
...
| C-3 | **[v0.6.0 재작성] 장바구니 담기 API(I-2)** | **BE 문서 채택**: `POST /internal/cart/items` 단건 + `X-Internal-Token` + 본문 신원(JWT `sub` 유래) + `optionId` + quantity 1~99 합산 + **게스트 담기 허용**. 옵션 되물음 멀티턴(`CART_OPTION_REQUIRED` options 목록)(§4.1) | BE I-2 문서 / 결정 7 / 결정 8 개정(§8 항목 7) | 🟢 **[해소 v0.15.5] 담기 재고검증 없음** — 재고 차감=주문 시점, 품절=stock 0, **`OUT_OF_STOCK` 폐기**. 🟢 **[해소 v0.15.8] options 스키마**(BE 2026-07-18) — `error.detail.options:[{optionId,name,extraPrice}]`. 🔴 잔여 — 서비스 토큰 발급만 |
...
| Q7 | 게스트 담기 실패 **3종·차단 없음** 최종? | ✅ GUEST_NOT_ALLOWED 폐기(§3.1·§4.1). **[갱신 v0.15.5] `OUT_OF_STOCK`도 폐기 → 실패 2종**(PRODUCT_NOT_FOUND/CART_ERROR) | 갱신 |
...
> **[v0.15.12 갱신]** Q5(I-13 재작성)·Q6(관리자 문의 MVP 제외)로 **해소**. **Q4 I-17 스키마 골격은 BE 확정(2026-07-18)** — 잔여는 스키마가 아닌 🔴 선결 1건 **배치 MVP/post-MVP 스코프**(config vs 파이프라인 모순). Q2는 초안 반영, 🔴 선결 **Q2 reason 적재 형식(FE 조율)** 잔여. 나머지(Q1·Q3·Q7·Q8·Q9)는 확정 회신 가능.
