...
| 버전 | 날짜 | 변경 |
|---|---|---|
| v0.32.4 | 2026-08-09 | **[#406] 구매자 `progress`에 `retrying` stage를 추가하고, 사람의 명시 지시로 #394를 원복했다.** I-1 검색이 재시도 가능한 실패 뒤 실제 다음 시도에 들어갈 때만 본검색 드레인이 즉시 frame을 내며 fan-out 콜백이 여러 번이어도 턴당 1회로 제한한다. 기본 `spring_max_retries=1`이라 queue/task 드레인 경로가 기본이며, 미룬 턴은 #306 가드로 재시도를 끄고 재시도를 0으로 끈 배포는 인라인 경로를 유지한다. 최악 구제 체인의 꼬리 예약 보호를 위해 `rescue_budget_mode=narrow`를 함께 적용했다(DESIGN-SHARED-BUDGET-384 §4). #394 원복 조건으로 적힌 BE #395 배포 여부는 이 저장소에서 확인되지 않았다. 구제 체인·자동 완화 probe·`_run_search_unfiltered`에는 이번 retrying 관측을 배선하지 않는다. |
