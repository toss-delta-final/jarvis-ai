...
| `NO_PROFILE` | 프로필 없음(신규 회원) 또는 `signals`가 비어 개인화 근거 없음 — **[v0.32.9] 또는 회원이 개인화를 중지함(§3.9.5). 중지는 `signals` 유무와 무관하게 이 값이다** | P-4 인기상품은 별도 컴포넌트가 조회하고, P-5는 `source=NOT_PERSONALIZED`로 추천 컴포넌트를 렌더하지 않게 한다 |
...
> **[v0.32.9] 개인화 중지는 이 어휘가 아니다.** 중지는 장애가 아니라 정상 동작이므로
...
- **[v0.22.0 · 근거 정정 v0.32.9] 개인화를 중지한 회원(§3.9.5)은 `outcome: "NO_PROFILE"`** 로 답한다. **`signals` 가 비어 있지 않아도 그렇다** — 중지는 아래 판정 기준(질의 벡터를 만들 수 없음)의 **결과가 아니라 그보다 앞서는 단락(short-circuit)** 이다.
...
- **`NO_PROFILE`의 판정 기준 = 질의 벡터를 만들 수 없음** — 시그널 임베딩도 프로필 벡터도 없는 경우다. 정본이 적은 두 사유(*"프로필 없음(신규 회원)"*·*"signals 비어있음"*)가 **둘 다 있어야** 성립한다. 뒤집으면 **둘 중 하나만 있어도 개인화한다** — 시그널이 비어도 대화로 쌓인 취향만으로 홈이 개인화되고, 프로필이 없어도 방금 본 상품으로 개인화된다. 시그널 상품이 아직 I-17로 인덱싱되지 않은 경우는 시그널이 없는 것과 같게 취급한다.
  - **[v0.32.9] 예외는 하나 — 개인화 중지.** 중지 회원은 이 기준을 **타기 전에** `NO_PROFILE` 로 단락한다(위 §3.7 규약). 여기 기준은 "근거가 있는가"를 재는데 중지는 "쓰지 않기로 했는가"라 축이 다르다. 구현이 이 노트만 읽고 벡터 항 제거로 처리하면 시그널 있는 중지 회원이 `PERSONALIZED` 를 받는다.
...
- I-22가 `outcome=NO_PROFILE` — 프로필 없음(신규 회원) 또는 시그널이 비어 개인화 근거 없음, **[v0.32.9] 또는 회원이 개인화를 중지함(§3.9.5 — 시그널 유무와 무관)**
