...
| 작성일 | 2026-07-14 (v0.32.16 개정 2026-08-10 — **[#570] §3.1 `token`에 렌더링 방식을 신설하고 §3.2 `token`에 판매자 사실 기록 문단을 더했다.** FE가 렌더링하는 마크다운은 `\n` 줄바꿈 · `- ` 불릿 목록 · `1. ` 번호 목록 · `**강조**` 4종뿐이고 그 밖은 AI 결정론적 문구(코드가 직접 조립하는 템플릿) 기준으로 미발신이며, 마크다운은 개별 `token` 프레임이 아니라 그 턴에 도착한 `token.text`를 이어붙인 누적 문자열 기준으로 해석한다. `rerank.overallComment`·`decompose.reply` 같은 LLM 자유 문장은 프롬프트로 금지할 뿐 결정론적으로 보장하지 않는다 — `_strip_unsafe`의 공백류 접기가 표·코드펜스 같은 여러 줄 구성만 구조적으로 못 서게 할 뿐이고, 안전성을 실제로 지탱하는 것은 FE가 4종 밖 문법을 해석하지 않고 문자 그대로 표시한다는 규칙이다. §3.2에는 판매자 스트림이 아직 이 4종 범위를 보장하지 않는다는 **사실만** 기록했다(동작 변경 없음, 판매자 코드·프롬프트 불변). 와이어 계약(엔드포인트·이벤트·필드·오류 코드) 자체는 불변 — 바뀌는 것은 렌더링 계약 서술뿐이다.) (v0.32.15 개정 2026-08-10 — **[#508] 옵션별 재고 도입 — I-1·I-3 `options`/`optionCount` 품절 제외 기준, I-2 옵션 필터·재고부족 승격, I-18 `maxQuantity` 신설.** BE 가 재고 원천을 상품 단위(`product.stock_quantity`)에서 옵션 단위(`product_stock`, 02 D33)로 전환하고 2026-08-09 구매자 쪽을 전량 배포했다(BE 저장소 `origin/main` 커밋 `4845776`·`40c6a35`, 노션 「BE 협의 요청」 페이지 확인 회신 "구매자 쪽은 끝났습니다"). §4.6/§4.17 `[].options`는 품절 옵션을 제외한 배열이 되고 `[].optionCount`는 그 필터된 목록의 크기(= "구매 가능한 것" 기준)이며, 두 값이 서로 다르면 이제 20개 절단만을 뜻한다(#455 정합 가드는 그대로 유효 — 신 계약에서도 절단이면 여전히 자동 선택을 막는다). 전 옵션 품절 상품은 검색 결과에서 아예 빠진다. §4.1 `error.detail.options`는 같은 기준으로 필터되고, 남은 옵션이 없으면 `CART_OPTION_REQUIRED` 대신 `availableStock: 0` 을 동반한 `CART_STOCK_INSUFFICIENT` 로 온다 — `availableStock`은 이제 **담은 옵션의 남은 수량**(구 "재고는 상품 단위" 서술 폐기). §4.9 `items[].maxQuantity`(신설)는 담은 옵션의 재고 기준, `purchaseState`의 `SOLD_OUT` 판정 근거도 "상품 단위 공유 재고"에서 "옵션별 재고의 OR"로 바뀐다(판정 규칙 자체는 불변). ⚠️ **한계** — 옵션은 상품당 한 종류뿐이라 옵션이 사이즈인 상품은 색상별로 못 거른다(BE 원문: "「빨간 셔츠」 요청이 전부 걸러진다고 보시면 안 됩니다") — 이슈 #454 의 색상 표기 이형 흡수(승인 동의어 사전)가 여전히 필요한 이유다. **2026-08-10 운영
...
**[v0.32.16 신설, 이슈 #570] 렌더링 방식** — FE 지적대로 위 예시만으로는 `token.text`를 어떻게 그려야
...
- **`token`** (근거/답변 산문 증분, 0회 이상). **[v0.32.16 사실 기록, 이슈 #570]** 판매자 스트림의
