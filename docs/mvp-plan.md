...
## 4. 판매자 그래프 (통계 I-6 · draft I-7)

| 항목 | 내용 |
|---|---|
| 목표 | 매출/판매 통계 Q&A + 상품 상세 수정 draft 제안 |
| 접근 | 별도 그래프. 통계=I-6 집계, draft=I-7 읽기→개정안 |
| 관련 파일 | `app/agents/seller`, `app/api/seller.py`, `spring_client`(get_seller_aggregates/get_product_detail) |
| 계약 | §3.2 SSE · §4.4 I-6 · §4.5 I-7 |

- **통계**: `get_seller_aggregates`(I-6). sellerId는 JWT sub, brandId는 Spring 내부 해소(AI 미보유). 집계값만 반환.
- **draft**: I-7 상세 읽기 → LLM 개정안 → SSE `draft{productId, changes:[{field,before,after}]}` → FE diff 카드 → **FE가 판매자 JWT로 Spring S-3 PATCH 실행**(AI 표면 밖).
- 이벤트는 `token`/`draft`/`done`/`error`만. `finishReason`=`stop` 단일.
- 주의: **채팅 발화 ≠ 동의** — 반영은 항상 FE의 명시적 [적용]. role=seller 없으면 403.
...
- **흐름**: `GET /products/changes?since={cursor}` (hasMore 루프) → DELISTED는 생성물 삭제/비활성 → enrichment(Haiku, Layer 2 속성·상황 태그) → search_doc 조립 → 임베딩(셀프호스트 1024-dim) → AI Postgres upsert.
