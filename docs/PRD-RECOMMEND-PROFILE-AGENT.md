...
| 문서 | 성격 | 우선순위 |
|---|---|---|
| `api-spec.md` v0.15.8 | 외부 인터페이스 계약(엔드포인트·SSE 이벤트·필드·오류 코드)의 **정본** | 최우선 — 계약 충돌 시 이 문서가 이긴다 |
...
| 결과 통지 | SSE `action`(`CART_ADDED{cartItemId}` / `CART_ADD_FAILED{reason}`). `reason`은 `PRODUCT_NOT_FOUND`/`CART_ERROR` 2종(`OUT_OF_STOCK`은 v0.15.5로 폐기 — 담기 시점 재고검증 없음) | REQ-CART-050~052 |
...
| UC-5 | "기억해" 즉시 기억 | 대화 중 | "이거 기억해줘" 발화 | 3조건 게이트 우회, fact 즉시 store 기록(단, `profile_summary` 재생성은 다음 세션 finalization) |
...
Spring I-20: logout/newConversation ───┐
                                      ├──▶ 공통 session finalizer (고정키 claim)
AI idle scheduler: inactivityTimeout ─┘        │ Spring만 영구 완료 / idle은 checkpoint 후 해제
                                              │ 저장된 회원 세션 버퍼 ← 정합성의 실제 원천
                                              │ (탭 닫기 신호·HTTP 자기 호출 없음)
                                              ▼
...
repo-local 정본 api-spec.md v0.15.19를 상위 외부 계약으로 참조한다. camelCase, productId 등 상품/옵션/장바구니/주문 id는 숫자(BIGINT), guestId만 UUID 문자열. 신원은 항상 JWT `sub` 도출(요청 본문 신뢰 금지).
...
추천(`app/agents/buyer/recommendation/`)·장바구니(`app/agents/buyer/cart/`)는 실제 구현 완료(이슈 #2/#3, PR #15/#16) — 구현 시 각 파일 docstring의 SPEC § 참조를 따랐다(장바구니는 `SPEC-CART-001`). 프로필 파이프라인(`reader`/`builder`/`gate`)은 아직 `NotImplementedError` 스텁. `app/pipelines/*`(카탈로그 데이터 구축)는 별도 담당.
...
- [`api-spec.md`](api-spec.md) v0.15.8 — 외부 계약 정본
...
- [`specs/SPEC-CART-001.md`](specs/SPEC-CART-001.md) v0.2.4 — 장바구니 서브그래프 EARS 명세(2026-07-17 신설, 기획 저장소 정본 등록 전)
- [`app/agents/buyer/cart/{graph,state}.py`](../app/agents/buyer/cart/graph.py) — 장바구니 실제 구현(이슈 #3, PR #16)
