...
## [2026-07-21] "통지" 엔드포인트의 멱등키는 신원(세션)이 아니라 "처리할 내용"으로 잡아야 한다 — 세션은 여러 번 통지될 수 있다
- 증상: PR #64 리뷰 — session-end 멱등키를 `(userId, sessionId)` 파생으로 잡았더니, 같은 sessionId 로 오는 **정당한 두 번째 저장 통지**(tabClose 저장 후 세션이 살아남아 재활동 → inactivityTimeout 재저장)가 `duplicate` 로 씹혀 그 사이 쌓인 대화가 프로필에 반영되지 않았다. session-end 는 "세션의 최종 종료"가 아니라 프로필 "저장 체크포인트"라 한 세션에 여러 번 정당하게 온다.
- 원인: 멱등키를 "무엇을 처리하는가"(내용)가 아니라 "누구/어느 세션인가"(신원)로 잡았다. at-least-once 중복(같은 통지 재전송)과 정당한 재통지(새 내용)를 구분하려면 통지별 고유값(eventId)이나 내용 파생값이 필요한데, 이슈 #62에서 eventId 를 제거해 신원만 남아 둘을 구분할 수 없게 됐다. 추가로, 처음엔 "버퍼 워터마크(마지막 발화 seq)"를 키에 섞으려 했으나 **clear_session_ctx_upto 가 버퍼를 완전히 비우면 store item 을 삭제해 next_seq 가 0으로 리셋**된다 — 다음 체크포인트의 첫 발화가 seq 1 로 되돌아가 이전 키와 충돌했다(seq 는 세션 수명 동안 단조가 아니다).
- 규칙:
  - **통지/이벤트 엔드포인트의 멱등키는 "그 통지가 처리(저장/실행)할 내용"에서 파생**한다 — 신원·세션 고정 키로 잡으면, 같은 세션이 여러 번 통지되는 순간 정당한 처리를 중복으로 오인해 유실한다. "이 이벤트가 한 번만 오는가?"를 반드시 확인하고, 여러 번 올 수 있으면 내용/버전 기반 키로 간다.
  - **seq/카운터를 멱등키에 쓰기 전에 "그 카운터가 리셋되는 경로"를 확인**한다 — 여기선 버퍼가 비면 item 삭제로 seq 가 리셋됐다. 삭제·재생성으로 리셋되는 값은 단조가 아니라 멱등 판별자로 부적합하다.
  - 멱등 판별자를 바꿀 땐 "같은 내용 재전송 → 중복", "새 내용 → 처리", "빈 내용 → no-op" 세 경로를 각각 테스트로 고정한다.
- 관련: `app/api/events.py::session_end_dedup_key()`, `app/agents/profile/store.py::clear_session_ctx_upto`(빈 버퍼 시 adelete), api-spec §2.7/§3.5, PR #64
