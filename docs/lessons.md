...
## [2026-08-03] 프롬프트 편집 축을 묶거나 단일 실행만 보면 효과와 지터를 혼동한다
- 증상: 옵션 예시 교체와 종결 가지 정리를 함께 넣은 후보는 한 축이 좋아지고 다른 축이 나빠져 어느
  편집의 효과인지 가릴 수 없었다. `"일반형"`도 한 N=8 실행에서는 6/8이었지만 같은 프롬프트의
  재실행에서는 3/8·2/8로 돌아왔고, 동일 해시 실행 간 축당 ±2 지터가 있었다.
- 원인: 서로 다른 편집 A·B를 한 후보에 넣고 단일 실행의 상승을 각 편집의 인과 효과로 해석했다.
- 규칙: **프롬프트 후보는 편집 축 하나씩 분리하고 독립 2회 이상 분포로 채택한다.** 묶은 후보의
