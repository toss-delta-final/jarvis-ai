...
- 수정(#208 PR): 휴대폰·주민번호 두 패턴의 경계를 `(?<!\d)`/`(?!\d)` 에서 **hex 경계** `(?<![0-9a-fA-F])`/`(?![0-9a-fA-F])` 로 바꿨다. UUID 는 그룹 크기가 8-4-4-4-12 라 10~11자리 매치가 항상 한쪽에서 hex 문자에 붙고 하이픈 경계에서도 성립하지 않아, 오탐이 **구조적으로** 사라진다(50만 표본 0건). 위 규칙 3 의 두 대안은 쓰지 않았다 — "검사 대상을 사용자 유래로 한정"은 fail-closed 검증기 안에 면제 목록을 만들어 드리프트를 부르고(지문 `sessionFp` 도 자체 생성이라 목록이 계속 자란다), "해당 필드만 마스킹"은 하필 `dotted_order` 가 LangSmith 의 **정렬 키**라 마스킹하면 트레이스 트리가 깨진다. 다만 규칙 3 이 지적한 **폭발 반경**(오탐 1건이 트레이스 전량 폐기 → 운영 관측 손실)은 이 수정으로도 그대로다 — 별도 과제.
