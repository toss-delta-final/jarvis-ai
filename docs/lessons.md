...
## [2026-08-09] 정상적으로 빈 보조 사전도 무동작 신호를 남긴다
- 증상: 색상 동의어 확장이 켜진 채 `approved` 행이 0건이면 확장은 무동작인데 아무 신호도 남지
  않았다. httpx는 1원소 리스트와 스칼라를 같은 URL로 직렬화하므로, 와이어가 종전과 같아 밖에서는
  정상 확장과 구분할 수 없었다.
- 원인: 빈 사전은 로드 실패가 아니라 성공한 정상 TTL 캐시 값이라 실패 경고 경로에 걸리지 않았다.
- 규칙: 보조 사전·확장이 정상적으로 비어 있을 수 있으면 빈 결과 자체를 로드 경계에서 TTL 창당
  한 번 경고해 "켜져 있는데 무동작"을 관측 가능하게 만든다. 기동 시 건수를 확인하려고 DB를
  강제하지 말고 런타임 lazy 조회를 유지한다. 빈 사전은 확장하지 않는다는 의도를 호출부의
  `if mapping:`으로 드러내되, 이를 와이어 버그 수정으로 오해하지 않는다.
- 관련: `app/pipelines/color_synonyms.py::get_synonym_map` ·
  `app/services/spring_client.py::search_products` · #505(#461 승계)
