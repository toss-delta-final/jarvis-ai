...
## [2026-08-09] 빈 승인 사전은 배열 한 원소가 아니라 기존 단수 와이어로 degrade한다
- 증상: 색상 동의어 확장이 켜진 채 `approved` 행이 0건이면 `expand_color`가 원문 한 값을
  돌려줬지만, 호출부가 그 값을 `color=[원문]`으로 I-1에 전송했다. HTTP 반복 파라미터 의미는
  같아 보여도 확장이 실제로 무동작이라는 관측 신호가 없고 종전 단수 경로의 바이트 보존도 깨졌다.
- 원인: 승인 사전의 빈 결과를 정상 TTL 캐시 값으로는 취급했지만, '확장할 값 없음'을 호출부에서
  구분하지 않아 리스트 계약을 무조건 선택했다.
- 규칙: 보조 사전이 빈 정상값이면 로드 경계에서 TTL 창당 한 번 원인·조치를 경고하고, 요청은
  `None` 경로로 남겨 기존 단수 와이어를 보존한다. 기동 시 사전 건수를 확인하려고 DB를 강제하지
  말고 런타임 lazy 조회를 유지한다.
- 관련: `app/pipelines/color_synonyms.py::get_synonym_map` ·
  `app/services/spring_client.py::search_products` · #461
