...
## [2026-08-07] 조건의 극성을 뒤집으면 그 조건이 읽는 목록의 **완전성 요구**도 함께 뒤집힌다
- 증상: #386 리뷰 대응으로 찜 해제 가드를 `"조회 표지가 보이면 막는다"` → `"해제 표지가 보일
  때만 허용한다"` 로 뒤집으면서, 그 판정이 읽던 `wishlist_remove_markers` 목록은 그대로 뒀다.
  그 결과 `"찜 목록에서 빼줘"`·`"위시리스트에서 빼줘"`·`"찜 리스트에서 제거해줘"` 같은 **정상
  해제 발화가 되물음으로 떨어졌다.** 단위 테스트는 `"찜 빼줘"` 계열만 덮고 있어 못 잡았고,
  `evals/combo_matrix` 의 observed 드리프트 가드(#424)가 잡았다
  (`combo-0047.actionType: WISHLIST_REMOVED → None`).
- 원인: 그 목록은 원래 `classify_cart_utterance` 의 **라우팅 표지**이고, 가드에서 "예외" 조건
  (누락돼도 무해)으로 쓰일 때는 불완전해도 됐다. 뒤집으면서 같은 목록이 "허용 근거" 조건이
  됐는데 — **누락이 곧 기능 손실**로 의미가 정반대가 됐는데도 목록의 완전성을 재검토하지 않았다.
  남의 목적으로 만들어진 목록을 빌려 쓴 것이 애초의 잘못이다.
- 규칙: **`if X` 를 `if not X` 로 뒤집을 때는 X 가 읽는 데이터의 완전성 요구가 어떻게 바뀌는지
  먼저 적어 보라.** "이 목록이 불완전하면 무엇이 깨지나"의 답이 뒤집기 전후로 달라지면, 목록을
  그대로 쓰면 안 된다 — 그 목적에 맞는 판정을 따로 만들어야 한다(#386 은 "찜 명사 + 해제 동사"
  결합으로 구조화했다). 그리고 **"기존 테스트 N건 통과"를 무회귀의 증거로 쓰지 마라** — 그건
  "내 테스트가 그 경로를 안 덮는다"는 뜻일 수 있다.
- 관련: `app/agents/buyer/cart/wishlist.py::_has_removal_evidence`,
  `app/core/config.py` (`wishlist_scope_markers`·`wishlist_removal_verb_markers`),
  `evals/combo_matrix` observed 드리프트 가드(#424)
