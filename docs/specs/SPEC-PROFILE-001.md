...
- **REQ-PROF-034** (Unwanted): The `builder` 2단계 **shall not** fact를 삭제하지 않는다 — 폐기 대신 `superseded_by`로 supersede하여 이력을 보존한다. 재확인되지 않은 선호는 EMA 감쇠를 적용한다(결정 4-A 5).
  - **[적용 범위 한정 v0.7.0, 이슈 #149] 이 금지는 기계(파이프라인) 경로를 구속한다.** 사용자가 명시적으로 요구한 삭제는 두 등급으로 나뉘며 이 금지의 대상이 아니다: (a) **개별 삭제 = 억제 후 물리 삭제** — 투영·요약·랭킹에서 즉시 제외하고, **[개정 v0.8.0, 이슈 #322] undo 창(`graph_undo_window_s`, 기본 5분) 이내에는 복구 가능하며 창이 닫히면 원문을 물리 삭제하고 재승격 차단용 tombstone 만 남긴다**(v0.7.0 은 원문을 무기한 보관하는 억제였다 — 사용자가 지웠다고 믿는 문장의 원문을 계속 보관할 이유가 없다). (b) **전체 초기화 = 물리 삭제** — 사용자 자신의 명시적 요청이며 기계 자동 처리가 아니므로 본 조항의 구속 대상이 아니고, 변경 감사 로그(api-spec §6.3 c)로 추적한다. 즉 **금지되는 것은 기계가 사용자 데이터를 조용히 지우는 것**이고 사용자 자신의 삭제권은 금지 대상이 아니다 — (a)의 물리 삭제도 **기계가 개시한 삭제가 아니라 사용자 요청의 지연 집행**이다. 규격은 `SPEC-PROFILE-GRAPH-149` §6.3·§6.7.
  - **좁은 예외 2건** **[개정 v0.8.0, 이슈 #322 — v0.7.0 은 "1건·유일"이라고 단정했다]**: (1) 민감 주제에서 파생된 항목은 보존기간 경과 시 **기계가 물리 삭제해야 한다**(신규 SPEC REQ-PGRAPH-077). (2) 사용자가 개별 삭제한 항목의 원문은 **undo 창 만료 시 물리 삭제해야 한다**(REQ-PGRAPH-025). 둘 다 *의무*인 하드 삭제이며 성격이 다르다 — (1)은 기계가 판정한 민감도를 근거로 기계가 개시하고, (2)는 사용자 요청을 유예 후 집행한다. 하드 삭제가 일어나는 지점을 한곳에서 셀 수 있도록 함께 적는다.
...
- **AC-PROF-31 (사용자 개별 삭제 = 즉시 억제 → undo 창 → 원문 물리 삭제)** **[개정 v0.8.0, 이슈 #322]**: **Given** 승격된 fact를 가진 회원, **When** 사용자가 그 항목을 삭제하면, **Then** 그 항목은 투영·요약·랭킹에서 즉시 제외되고 **undo 창(`graph_undo_window_s`) 이내에는 복구할 수 있으며, 창이 지나면 원문이 물리 삭제되어 복구되지 않고**, **어느 시점에도 이후 배치가 같은 취향을 재파생해 다시 나타나게 하지 않는다**(REQ-PROF-015/034, 신규 SPEC REQ-PGRAPH-022/023/025/026/031).
...
- **기계 경로의** fact 폐기 대신 supersede(이력 보존). **사용자 요청 삭제는 개별(즉시 억제 → undo 창 → 원문 물리 삭제)/전체 초기화(즉시 물리 삭제) 2등급이며 항상 감사된다** — 금지 대상은 기계가 조용히 지우는 것이다(REQ-PROF-034/085, AC-PROF-13/31/32, v0.7.0 이슈 #149, **개별 삭제 개정 v0.8.0 이슈 #322**). 기계 경로 하드 삭제가 의무인 좁은 예외는 **2건**(민감 파생 보존기간 만료 · 개별 삭제 undo 창 만료)이다.
