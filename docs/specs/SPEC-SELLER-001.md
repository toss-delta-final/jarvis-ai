...
> **버전**: v1.1.0 (provider 토글 정합, 2026-07-23) · **상태**: MVP 구현 완료(1~4-3단계) — **정본 승격(기획 저장소 `.moai/specs/`) 은 미완**
...
- provider·모델 ID·API key·reasoning effort·Anthropic temperature는 Settings 주입이다. `LLM_PROVIDER` 기본은 `openai`이며 `anthropic`으로 전환할 수 있다.
