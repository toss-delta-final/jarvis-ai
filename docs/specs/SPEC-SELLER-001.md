...
| 6 | `init_chat_model("openai:gpt-4o-mini")` | **Anthropic 2-tier** — Haiku 4.5 / Sonnet 5 (§8) | CLAUDE.md · pyproject |
...
```mermaid
flowchart TD
    START((START)) --> SUP["supervisor<br/>구조화 출력 라우팅 (Haiku, t=0)"]
    ...
    subgraph AnalysisTeam["분석 서브그래프 (Send 팬아웃 → 팬인)"]
        ...
        W --> RPT["report_agent (Sonnet)"]
        RPT --> VER["report_verifier<br/>결정론 검증 + LLM 채점 (≤3회 루프)"]
        VER -->|미달| RPT
        VER -->|통과| REC["recommend_agent (Sonnet)<br/>읽기 전용 · 구조화 추천"]
...
- 이벤트는 **`token` / `draft` / `done` / `error` 4종만**(§3.2). `done.finishReason="stop"` 단일. 구매자 이벤트(products.ready 등) emit 금지.
- **first-token 10s**: supervisor 라우팅 직후 진행 token을 먼저 흘린다(§2).
- **상한 90s는 목표치**(2026-07-17 확정) — 분석 스트림은 초과 가능. 타임아웃은 Settings 주입으로 두고, Send 팬아웃 병렬화·모델 티어(Haiku)·검증 루프 상한(≤3)으로 단축한다. 지속 초과 시 §2.9의 판매자 스트림 예외(상한 연장)를 명세 개정으로 제안 🔴.
...
## 8. 모델 배정 (Anthropic 2-tier)

| 역할 | 모델 | temperature |
|---|---|---|
| supervisor · analysis_planner · 분석 워커 5종 · report_verifier judge | **Haiku 4.5** | 0 |
| report_agent · recommend_agent | **Sonnet 5** | 0.2 |

- 근거: 라우팅·분류·정형 분석은 경량, 서술·추천 품질은 상위 티어 — 리포의 2-tier 원칙(README) 그대로.
- 모델 ID·temperature는 Settings 주입. 모델 버전 변경은 일관성 리셋 이벤트로 CHANGELOG에 기록(§10-①).
...
| # | 장치 | 본 SPEC 조정 |
|---|---|---|
| ① | temperature=0 (라우터·워커·judge) | 모델을 Haiku 4.5로(§8). 버전 고정·변경 시 기록 |
