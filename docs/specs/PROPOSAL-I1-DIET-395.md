...
|---|---|---|---|---|
| 1. `size` 상한 (⏸ 이번 송부 보류) | **협의 2 반영 시** 기본 1,000·하드 3,000 / **미반영 시** 하드 1,500 + 결정적 정렬 + `totalCount` 동반 | 처리율 역산(12.3MB/7.74s≈1.6MB/s, 남는 창 1.9s) — 다이어트 적용 N=3,000은 1.85s(1.9초 창 경계에 붙음), 미적용 N=3,000은 3.4s(예산 초과)(§4) | 중(쿼리 `LIMIT`+정렬 컬럼 선정+응답 필드 1개 추가) | (요청 자체를 보류해 미송부) |
| 2. 필드 축소 | `attributes` 내부 `_extra`·`_source_pid`·`_domain`·`_category` 4키 제외 (`summary`·`options`·`optionCount`는 전부 유지) | 항목당 바이트 **46.3~49.8%** 감소(§3·§5) + `keyword` LIKE 오탐 감소(§5) | 소(직렬화 코드에서 필드 몇 개 제외) | **수용** |
...
⏸ 이번 BE 송부에서는 보류한다(2026-08-07 결정). 절단된 상품은 그 턴의 후보에 아예 들어오지
못하고, 결정적 정렬은 재현성을 줄 뿐 관련도를 보장하지 않는다 — 그 손실을 감수할지가 미결이라
요청 2·3을 먼저 보낸다. 아래 분석은 재개 시점에 그대로 쓴다.
...
3. 그래서 제안은 **(a) 상한값은 크게 + (b) 절단 순서를 결정적으로 + (c) `totalCount` 동반** 3종
   ...
   **결론이 협의 2(필드 축소)의 수용 여부에 갈린다** — 이게 이 절에서 가장 중요한 대목이다.
   ...
   거의 붙어 있다. **그래서 요청 1을 재개할 때 하드 상한은 3,000보다 보수적으로 잡는 편이
...
**요청 1 — I-1 `size` 상한 + 결정적 정렬 + `totalCount` (⏸ 이번 송부 보류)**

⏸ **이번 BE 송부에서는 보내지 않는다**(2026-08-07 결정) — 절단된 상품이 후보에 아예 안 들어오는
손실을 감수할지가 미결이라 요청 2·3만 먼저 보낸다(§4). 아래 문안은 재개 시점에 그대로 쓴다.
...
**BE 회신(2026-08-07)**: 해당 없음 — 요청 자체를 이번 송부에서 보류했다(위 ⏸ 참조).
...
생각합니다. (2) 그게 지금 당장 어렵다면, 향후 후보 수 상한을 도입하게 되면(저희 쪽 `size`
상한 요청은 이번 송부에서는 보류했고 별도로 다시 논의드릴 예정입니다) 이미 BE에 있는 인기
상품 경로의 id-IN 배치 집계(`ReviewService#getStats`)로 I-1도 전환할 여지가 열립니다 — 그
시점에 다시 문의드리겠습니다. (3) 추가로 `review` 테이블에 `(product_id, status)` 복합
인덱스가 있는지도 확인 부탁드립니다.
...
⏸ **바로 아래 `size`·`totalCount` 관련 초안 두 개(AI → Spring 요청 표의 `size` 행, 응답 표의
`totalCount` 필드 추가)는 요청 1 보류로 함께 보류한다**(2026-08-07 결정, §4·§8 요청 1) — 재개
시점에 그대로 쓴다. 아래 `attributes` 축소 diff와 그다음 `rating`/`reviewCount` 캐싱 반영 diff는
**BE 가 이미 수용·회신했으므로 보류가 아니다**(§8 요청 2·3 회신).
...
`summary`·`options`·`optionCount`는 **모두 유지**(2026-08-07 최종 결정, §5) — 응답 표에서
삭제할 필드는 없다.
...
- **2026-08-07 사용자 결정**: 요청 1(`size` 상한)은 폐기가 아니라 **이번 BE 송부에서 보류**한다
  — 절단된 상품이 그 턴의 후보에 아예 안 들어오는 손실을 감수할지가 아직 미결이라서다(§4).
  요청 2에서 `summary`는 제외 요청 대상에서 뺀다(= BE에 유지 요청) — `_extra` 계열과 달리 AI
  쪽에 사본이 없어(I-17이 안 준다) 되살리려면 계약 개정이 다시 필요해 되돌리기 비용이
  절감분보다 크다(§2·§5). 두 결정 모두 근거 분석(§4·§2)은 그대로 남겨 재개 시 쓴다.
...
| `size` 상한 | **보류** — 재개 여부 미결 | §4·§8 요청 1, 문안 보존 |
