...
| 3. rating/reviewCount 비정규화 | 근본: 리뷰 쓰기 시 집계 컬럼 갱신 / 중간: size 상한 도입 후 id-IN 배치 집계로 전환 | 검색마다 `LEFT JOIN Review ... GROUP BY` 도는 구조(§6) | 중(근본)~소(중간, 기존 코드 재사용) | 반정규화 대신 **별도 캐싱 도입**으로 갈음(`review(product_id,status)` 인덱스는 OK, §6) |
...
**BE 회신(2026-08-07)**: 반정규화는 채택하지 않고 **별도 캐싱 도입**으로 갈음한다. 인덱스는
...
비난이 아니라 **재측정에서 확인할 지점**이라는 톤으로 적는다 — §8 끝의 "추가 확인 요청"이 그
확인을 위한 질문이고, §10 재측정 절차의 (a)가 그 결과를 실측으로 검증한다.
...
**BE 회신(2026-08-07)**: 반정규화 대신 **별도 캐싱 도입**으로 갈음. `review(product_id, status)`
인덱스는 **OK**(§6 참조).

---

**추가 확인 요청 — 평점 캐싱 4건**(2026-08-07, 이미 BE에 송부한 문안 그대로)
...
아래 `attributes` 축소 diff와 `rating`/`reviewCount` 캐싱 반영 diff만 남긴다 — **BE가 이미
수용·회신했다**(§8 요청 2·3 회신).
...
`[].rating`·`[].reviewCount`의 서술에 아래 diff를 제안한다(**BE 2026-08-07 회신 — 반정규화 대신
캐싱 도입으로 갈음, §6**). 값의 의미·타입은 불변이고 **최신성만 캐시 정책에 따라 달라진다** —
구체 문구(캐시 단위·TTL·무효화 시점·최대 stale 시간)는 §8 끝 "추가 확인 요청 4건"의 회신을 받은
뒤 확정한다(지금은 모르는 값을 지어내지 않는다):

```diff
-| `[].rating` | number | **조회 시 집계**(저장 avg 없음, DDL D9) — **AI 계산용(비표시, #100 P0)**: 평점 사후필터·rerank 신호. 표시는 CH-5 |
+| `[].rating` | number | **[2026-08-07, #395 BE 회신] 조회 시 집계 → 캐싱 도입 예정**(반정규화
+대신 — 캐시 단위·TTL·무효화 시점은 확인 중, §8 "추가 확인 요청" 참조). 값 의미·타입은 불변,
+최신성만 캐시 정책에 따라 달라진다. **AI 계산용(비표시, #100 P0)**: 평점 사후필터·rerank 신호.
+표시는 CH-5 |
-| `[].reviewCount` | int | **조회 시 집계**(리뷰 개수) — **AI 계산용(비표시, #171)**: `rating`과 짝지어 **"리뷰가 없어 rating=0"(reviewCount=0, 데이터 부재)**와 **"리뷰가 있고 하한 미달"(reviewCount>0)**를 가른다. `null`/미전송이면 rating이 지배(구 동작 폴백). 표시용 리뷰수는 여전히 경로 B(§4.3) |
+| `[].reviewCount` | int | **[2026-08-07, #395 BE 회신] 조회 시 집계 → 캐싱 도입 예정**(반정규화
+대신 — 캐시 단위·TTL·무효화 시점은 확인 중, §8 "추가 확인 요청" 참조). 값 의미·타입은 불변.
+**AI 계산용(비표시, #171)**: `rating`과 짝지어 "리뷰가 없어 rating=0"(reviewCount=0, 데이터
+부재)와 "리뷰가 있고 하한 미달"(reviewCount>0)를 가른다. `null`/미전송이면 rating이 지배(구
+동작 폴백). 표시용 리뷰수는 여전히 경로 B(§4.3). **stale이 길면 이 판별이 흔들릴 수 있다**
+(§6·§8 확인 요청 ③) |
```
...
- **BE 반영 후 재측정 절차**(이슈 #395 완료 조건 "무필터 요청의 응답 시간이 3초 예산 안에
  들어온다(재측정)"을 닫는 데 쓴다) — **트리거: BE가 §8 요청 2(필드 축소)·요청 3(캐싱)를
  배포한 뒤** 아래 절차를 수행한다(그 전에는 재측정해도 의미가 없다 — 다이어트 전 응답을 재는
  꼴이다). 새 스크립트를 만들지 않고 기존 것으로 한다:
...
### 이슈 #395 완료 조건 대비 현재 상태 (2026-08-07)

| 항목 | 상태 | 비고 |
|---|---|---|
| 필드 축소(`attributes` 4키) | **BE 수용**, 배포 대기 | §8 요청 2, §9 diff 준비됨 |
| rating/reviewCount | **BE 캐싱 도입 확정**, 세부 미확정 | §8 "추가 확인 요청 4건" 회신 대기, §9 diff는 값·타입 불변만 반영 |
| `review(product_id,status)` 인덱스 | **BE 확인 완료(OK)** | §6 |
| `size` 상한 | **❌ 폐지** — 재개 계획 없음 | §4, 근거만 기록 보존(§8 요청 1 문안은 삭제) |
| 완료 조건 "무필터 3초 예산 재측정" | **미실시** | BE 배포 후 트리거(위 절차 (a)~(d)) |
