...
### 5.1 summary 분리 규칙 (`split_report_summary`)

REPORT_PROMPT 이 "1. 핵심 요약(2~3문장) 먼저" 를 강제하지만 LLM 산출이라 어길 수 있다 — 코드 fallback 필수.

1. `report.split("\n\n", 1)[0]` — 첫 빈 줄 전까지를 후보로.
2. 후보가 비었거나 **300자 초과**(첫 문단 실패 신호)면: 전문 앞 **200자 절단 + "…"**.
3. 항상 non-empty string 반환 (report 자체가 빈 문자열이면 "" — degrade 루프 소진 케이스, FE 는 body 로 fallback).
