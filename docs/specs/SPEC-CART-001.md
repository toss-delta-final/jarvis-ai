...
class CartItem(BaseModel):
    cart_item_id: int
    product_id: int
    product_name: str | None      # 🔴 포함 여부 미확정(C-16)
    option_id: int | None
    option_name: str | None       # 🔴 포함 여부 미확정(C-16)
    quantity: int
    price: float | None
```
...
- **REQ-CART-036** (Ubiquitous): 자연어 응답 생성에는 `productName`(상품명)이 필요하나, I-18 응답에 이 필드가 포함되는지는 미확정이다(🔴 C-16, §9 OPEN-CART-2).
...
- [ ] AC-CART-01~09 전 시나리오가 통과(pytest, Spring 목(mock) 또는 계약 테스트).
