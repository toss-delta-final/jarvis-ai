...
- **REQ-CART-026** (Event-Driven, 2026-07-31 이슈 #114): **When** `CART_OPTION_REQUIRED`의 옵션 후보가 **정확히 1개**이고 그것이 직전 호출에 실은 `optionId`와 다르면, the cart 서브그래프 **shall** 되묻지 **않고** 그 `optionId`로 I-2를 즉시 재호출하며, 성공 시 자동 선택한 옵션명을 담기 안내 문구(`action.message`)에 밝힌다 — 선택지가 하나면 되물어도 답이 정해져 있어 왕복만 늘어난다. 후보가 2개 이상이면 AI가 임의로 고르지 **않는다**(REQ-CART-020 유지).
