...
prompt=865ed6fd771e (repo:_SYSTEM) · scope=6c54c2656063 · tier=fast · model=gpt-5-nano · fixture=intent-probe-anchors-b-v6 · N=8

#240 축 순서 요약: `235/144/91/31/12/48/31/16`
...
| `categoryClear` 카테고리-무관 리셋 | 28/32 (87.5%) | resolvedCategoryAction 이 clear 인 표본 수 | 리셋 4발화 × categoryPrior 컨텍스트 × N (N=8 이면 32) |
| | | ⚠ 직접 비교 금지: baselines/fast-2026-08-04 (이 축이 존재하지 않던 런) | |
...
| | | ⚠ 직접 비교 금지: baselines/fast-2026-08-04 (이 축이 존재하지 않던 런) | |
| `conditionOnlyNoCategoryQuery` 조건 전용 categoryQueries 비움 | 38/40 (95.0%) | categoryLegs(leg 원문 직렬화)가 빈 문자열인 표본 수 — decompose 가 이 발화에서 categoryQueries 를 하나도 못박지 않은 경우 | 조건 전용 5발화 × none 컨텍스트 × N (N=8 이면 40) |
...
| `optionAnswer` 옵션 답변 | 31/32 (96.9%) | intent 가 cart_add 이고 cart.optionId 까지 기대값과 일치한 표본 수 (하나만 맞으면 오답 — 옵션을 잘못 담는 것은 실패다) | 옵션 답변 4발화 × PENDING_CART 컨텍스트 × N |
...
| `switchLegacy2` 전환(#240 2발화) | 12/16 (75.0%) | intent 가 cart_add 이고 cart.productId 가 **되물음 상품이 아닌** LAST_RECOMMENDATIONS 안의 상품인 표본 수 — #240 정의 | #240 이 쓴 전환 2발화(`이어폰으로 할래`·`다른 거 담아줘`) × PENDING_CART × N |
...
| `category-replace-003\|categoryPrior` | 8 | 8 | general 1, recommend 7 |
...
| `demonstrative-003\|none` | 8 | 8 | general 2, recommend 6 |
