...
prompt=f99a98867e4a (repo:_SYSTEM) · scope=6c54c2656063 · tier=fast · model=gpt-5-nano · fixture=intent-probe-anchors-b-v6 · N=8
...
| `categoryReplace` 카테고리 교체 | 19/24 (79.2%) | resolvedCategoryAction 이 replace 인 표본 수 | 교체 3발화 × categoryPrior 컨텍스트 × N (N=8 이면 24) |
| | | ⚠ 직접 비교 금지: baselines/fast-2026-08-04 (이 축이 존재하지 않던 런) | |
| `conditionOnlyNoCategoryQuery` 조건 전용 categoryQueries 비움 | 35/40 (87.5%) | categoryLegs(leg 원문 직렬화)가 빈 문자열인 표본 수 — decompose 가 이 발화에서 categoryQueries 를 하나도 못박지 않은 경우 | 조건 전용 5발화 × none 컨텍스트 × N (N=8 이면 40) |
...
| `general` general | 26/48 (54.2%) | intent 가 general 인 표본 수 | general 2발화 × 컨텍스트 3종 × N |
...
| `general-001\|pendingCart` | 8 | 8 | general 6, order_status 2 |
| `general-002\|lastRecommendations` | 8 | 8 | recommend 8 |
| `general-002\|none` | 8 | 8 | general 5, recommend 3 |
