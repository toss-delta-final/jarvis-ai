...
`baselines/fast-2026-08-05-84/` — **#84 출고 구성 대조쌍**을 픽스처 **v2**(64셀)로 잰 두 런.
루트가 출고 구성이고 `no-classifier/` 가 **결함 재현 팔**이다 — **같은 커밋·같은 프롬프트에서
`--no-classifier` 플래그 하나만 다르다.** 분류기를 끄면 `categoryClear` **0/32**, 켜면 **32/32**
이고 기존 8축은 두 팔이 서로의 **±1 안**이다(결함의 재현·해소·회귀 부재가 한 쌍으로 나온다).
그 디렉터리 README 에 전 축 대조표와 인라인 `categoryAction` 필드를 기각한 근거(전환 축
