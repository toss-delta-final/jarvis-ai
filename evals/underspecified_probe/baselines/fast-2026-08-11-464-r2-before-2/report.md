...
|---|---|---|---|---|---|
| `expansionGateWouldFireRate` 전개 게이트 발동률(판정 True 표본 중) | 49/107 (45.8%) | CI95 [36.7%, 55.2%] | exploratory | detect_expansion_need(...) 가 사유를 돌려준 표본 | 판정 True 인 recommend 표본(F-1) |
...
| `missRate` 미탐율 | 5/112 (4.5%) | CI95 [1.9%, 10.0%] | confirmatory-primary | 판정 False | expectedReask=true 앵커의 recommend 표본 — 프로덕션은 intent==recommend 인 턴에서만 is_underspecified_turn 을 호출한다(F-1) |
| `missRateUnderExpansionAssumption` 미탐율(전개 가정 상한) | 54/112 (48.2%) | CI95 [39.2%, 57.4%] | exploratory | (판정 False) 또는 (판정 True ∧ detect_expansion_need 가 사유를 돌려줌) | expectedReask=true 앵커의 recommend 표본(F-1) |
| `missRateWithNonRecommendIntent` 미탐율(비-recommend 포함, 참고용) | 5/112 (4.5%) | CI95 [1.9%, 10.0%] | exploratory | 판정 False | expectedReask=true 앵커의 표본 전부(intent 무관, 포함판) |
...
| 슬라이스 | 점수 | CI95 |
|---|---|---|
| no_condition | 33/38 (86.8%) (exploratory: N<40) | [72.7%, 94.2%] |
...
outcome 분포: {'correctReject': 128, 'hit': 107, 'miss': 5}
...
- `expansionGateWouldFireRate`: 49/107 (45.8%)
- `missRateUnderExpansionAssumption`: 54/112 (48.2%)
