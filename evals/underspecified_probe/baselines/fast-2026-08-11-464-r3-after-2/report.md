...
|---|---|---|---|---|---|
| `expansionGateWouldFireRate` 전개 게이트 발동률(판정 True 표본 중) | 48/108 (44.4%) | CI95 [35.4%, 53.8%] | exploratory | detect_expansion_need(...) 가 사유를 돌려준 표본 | 판정 True 인 recommend 표본(F-1) |
...
| `judgmentAccuracy` 판정 정확도 | 212/216 (98.1%) | CI95 [95.3%, 99.3%] | exploratory | 판정 == expectedReask | recommend 표본(게이트 제외)(F-1) |
| `judgmentAccuracyWithGateSlice` 판정 정확도 | 236/240 (98.3%) | CI95 [95.8%, 99.4%] | exploratory | 판정 == expectedReask | recommend 표본(게이트 포함)(F-1) |
| `missRate` 미탐율 | 4/112 (3.6%) | CI95 [1.4%, 8.8%] | confirmatory-primary | 판정 False | expectedReask=true 앵커의 recommend 표본 — 프로덕션은 intent==recommend 인 턴에서만 is_underspecified_turn 을 호출한다(F-1) |
| `missRateUnderExpansionAssumption` 미탐율(전개 가정 상한) | 52/112 (46.4%) | CI95 [37.5%, 55.6%] | exploratory | (판정 False) 또는 (판정 True ∧ detect_expansion_need 가 사유를 돌려줌) | expectedReask=true 앵커의 recommend 표본(F-1) |
| `missRateWithNonRecommendIntent` 미탐율(비-recommend 포함, 참고용) | 4/112 (3.6%) | CI95 [1.4%, 8.8%] | exploratory | 판정 False | expectedReask=true 앵커의 표본 전부(intent 무관, 포함판) |
...
| constraint_price | 2/40 (5.0%) | [1.4%, 16.5%] |
| constraint_budget_set | 13/32 (40.6%) (exploratory: N<40) | [25.5%, 57.7%] |
| what_axis | 0/0 (해당 없음) | N/A |
| blocking_rating | 0/0 (해당 없음) | N/A |
| multiturn_gate | 0/0 (해당 없음) | N/A |
...
| 슬라이스 | 점수 | CI95 |
|---|---|---|
| no_condition | 35/38 (92.1%) (exploratory: N<40) | [79.2%, 97.3%] |
...
| constraint_budget_set | 100.0% (exploratory: N<40) | 0.0% |
...
outcome 분포: {'correctReject': 128, 'hit': 108, 'miss': 4}
...
- `expansionGateWouldFireRate`: 48/108 (44.4%)
- `missRateUnderExpansionAssumption`: 52/112 (46.4%)

## 셀별 표본 수

| 셀 | 슬라이스 | 표본 | 시도 |
|---|---|---|---|
| `buy-under-0001` | constraint_budget_set | 8 | 9 |
| `buy-under-0002` | no_condition | 8 | 8 |
| `buy-under-0003` | constraint_price | 8 | 11 |
| `buy-under-0004` | what_axis | 8 | 9 |
| `buy-under-0005` | what_axis | 8 | 12 |
| `buy-under-0006` | multiturn_gate | 8 | 10 |
...
| `under-cp-0001` | constraint_price | 8 | 9 |
