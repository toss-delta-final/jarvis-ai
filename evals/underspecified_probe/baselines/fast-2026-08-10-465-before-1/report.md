...
|---|---|---|---|---|---|
| `expansionGateWouldFireRate` 전개 게이트 발동률(판정 True 표본 중) | 45/101 (44.6%) | CI95 [35.2%, 54.3%] | exploratory | detect_expansion_need(...) 가 사유를 돌려준 표본 | 판정 True 인 recommend 표본(F-1) |
...
| `missRate` 미탐율 | 12/112 (10.7%) | CI95 [6.2%, 17.8%] | confirmatory-primary | 판정 False | expectedReask=true 앵커의 recommend 표본 — 프로덕션은 intent==recommend 인 턴에서만 is_underspecified_turn 을 호출한다(F-1) |
| `missRateUnderExpansionAssumption` 미탐율(전개 가정 상한) | 57/112 (50.9%) | CI95 [41.8%, 60.0%] | exploratory | (판정 False) 또는 (판정 True ∧ detect_expansion_need 가 사유를 돌려줌) | expectedReask=true 앵커의 recommend 표본(F-1) |
| `missRateWithNonRecommendIntent` 미탐율(비-recommend 포함, 참고용) | 12/112 (10.7%) | CI95 [6.2%, 17.8%] | exploratory | 판정 False | expectedReask=true 앵커의 표본 전부(intent 무관, 포함판) |
...
| 슬라이스 | 점수 | CI95 |
|---|---|---|
| no_condition | 30/39 (76.9%) (exploratory: N<40) | [61.7%, 87.4%] |
| constraint_price | 0/32 (0.0%) (exploratory: N<40) | [0.0%, 10.7%] |
| constraint_budget_set | 15/29 (51.7%) (exploratory: N<40) | [34.4%, 68.6%] |
...
outcome 분포: {'correctReject': 127, 'falseAlarm': 1, 'hit': 100, 'miss': 12}
...
- `expansionGateWouldFireRate`: 45/101 (44.6%)
- `missRateUnderExpansionAssumption`: 57/112 (50.9%)
