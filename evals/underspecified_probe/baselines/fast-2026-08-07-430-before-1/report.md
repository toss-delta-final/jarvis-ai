...
| constraint_price | 39/40 (97.5%) | [87.1%, 99.6%] |
...
|---|---|---|---|---|---|
| `expansionGateWouldFireRate` 전개 게이트 발동률(판정 True 표본 중) | 0/1 (0.0%) | CI95 [0.0%, 79.3%] | exploratory | detect_expansion_need(...) 가 사유를 돌려준 표본 | 판정 True 인 recommend 표본(F-1) |
...
| `judgmentAccuracy` 판정 정확도 | 105/216 (48.6%) | CI95 [42.0%, 55.2%] | exploratory | 판정 == expectedReask | recommend 표본(게이트 제외)(F-1) |
| `judgmentAccuracyWithGateSlice` 판정 정확도 | 129/240 (53.8%) | CI95 [47.4%, 59.9%] | exploratory | 판정 == expectedReask | recommend 표본(게이트 포함)(F-1) |
| `missRate` 미탐율 | 111/112 (99.1%) | CI95 [95.1%, 99.8%] | confirmatory-primary | 판정 False | expectedReask=true 앵커의 recommend 표본 — 프로덕션은 intent==recommend 인 턴에서만 is_underspecified_turn 을 호출한다(F-1) |
| `missRateUnderExpansionAssumption` 미탐율(전개 가정 상한) | 111/112 (99.1%) | CI95 [95.1%, 99.8%] | exploratory | (판정 False) 또는 (판정 True ∧ detect_expansion_need 가 사유를 돌려줌) | expectedReask=true 앵커의 recommend 표본(F-1) |
| `missRateWithNonRecommendIntent` 미탐율(비-recommend 포함, 참고용) | 111/112 (99.1%) | CI95 [95.1%, 99.8%] | exploratory | 판정 False | expectedReask=true 앵커의 표본 전부(intent 무관, 포함판) |
...
| constraint_price | 1/40 (2.5%) | [0.4%, 12.9%] |
...
|---|---|---|
| no_condition | 0/0 (해당 없음) | N/A |
| constraint_price | 0/1 (0.0%) (exploratory: N<40) | [0.0%, 79.3%] |
| constraint_budget_set | 0/0 (해당 없음) | N/A |
| what_axis | 0/0 (해당 없음) | N/A |
| blocking_rating | 0/0 (해당 없음) | N/A |
| multiturn_gate | 0/0 (해당 없음) | N/A |
...
outcome 분포: {'correctReject': 128, 'hit': 1, 'miss': 111}
...
- `expansionGateWouldFireRate`: 0/1 (0.0%)
- `missRateUnderExpansionAssumption`: 111/112 (99.1%)
