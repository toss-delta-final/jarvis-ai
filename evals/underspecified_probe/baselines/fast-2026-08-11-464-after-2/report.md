...
|---|---|---|---|---|---|
| `expansionGateWouldFireRate` 전개 게이트 발동률(판정 True 표본 중) | 46/103 (44.7%) | CI95 [35.4%, 54.3%] | exploratory | detect_expansion_need(...) 가 사유를 돌려준 표본 | 판정 True 인 recommend 표본(F-1) |
...
| `missRate` 미탐율 | 10/112 (8.9%) | CI95 [4.9%, 15.7%] | confirmatory-primary | 판정 False | expectedReask=true 앵커의 recommend 표본 — 프로덕션은 intent==recommend 인 턴에서만 is_underspecified_turn 을 호출한다(F-1) |
| `missRateUnderExpansionAssumption` 미탐율(전개 가정 상한) | 55/112 (49.1%) | CI95 [40.0%, 58.2%] | exploratory | (판정 False) 또는 (판정 True ∧ detect_expansion_need 가 사유를 돌려줌) | expectedReask=true 앵커의 recommend 표본(F-1) |
| `missRateWithNonRecommendIntent` 미탐율(비-recommend 포함, 참고용) | 10/112 (8.9%) | CI95 [4.9%, 15.7%] | exploratory | 판정 False | expectedReask=true 앵커의 표본 전부(intent 무관, 포함판) |
...
| 슬라이스 | 점수 | CI95 |
|---|---|---|
| no_condition | 34/37 (91.9%) (exploratory: N<40) | [78.7%, 97.2%] |
| constraint_price | 0/35 (0.0%) (exploratory: N<40) | [0.0%, 9.9%] |
| constraint_budget_set | 11/30 (36.7%) (exploratory: N<40) | [21.9%, 54.5%] |
...
outcome 분포: {'correctReject': 127, 'falseAlarm': 1, 'hit': 102, 'miss': 10}
...
- `expansionGateWouldFireRate`: 46/103 (44.7%)
- `missRateUnderExpansionAssumption`: 55/112 (49.1%)
...
| `buy-under-0006` | multiturn_gate | 8 | 11 |
| `buy-under-0007` | blocking_rating | 8 | 11 |
| `under-br-0001` | blocking_rating | 8 | 11 |
