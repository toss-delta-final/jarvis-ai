...
`falseAlarmRate`(multiturn_gate 제외): 4/104 (3.8%)
CI95 [1.5%, 9.5%]
...
|---|---|---|---|---|---|
| `expansionGateWouldFireRate` 전개 게이트 발동률(판정 True 표본 중) | 57/107 (53.3%) | CI95 [43.9%, 62.4%] | exploratory | detect_expansion_need(...) 가 사유를 돌려준 표본 | 판정 True 인 recommend 표본(F-1) |
| `falseAlarmRate` 오탐율 | 4/104 (3.8%) | CI95 [1.5%, 9.5%] | confirmatory-secondary | 판정 True | expectedReask=false 앵커의 recommend 표본(multiturn_gate 슬라이스 제외)(F-1) |
| `falseAlarmRateWithGateSlice` 오탐율 | 4/128 (3.1%) | CI95 [1.2%, 7.8%] | exploratory | 판정 True | expectedReask=false 앵커의 recommend 표본(게이트 포함)(F-1) |
| `falseAlarmRateWithNonRecommendIntent` 오탐율(비-recommend 포함, 참고용) | 4/104 (3.8%) | CI95 [1.5%, 9.5%] | exploratory | 판정 True | expectedReask=false 표본(intent 무관, multiturn_gate 제외, 포함판) |
...
| `judgmentAccuracy` 판정 정확도 | 203/216 (94.0%) | CI95 [90.0%, 96.4%] | exploratory | 판정 == expectedReask | recommend 표본(게이트 제외)(F-1) |
| `judgmentAccuracyWithGateSlice` 판정 정확도 | 227/240 (94.6%) | CI95 [91.0%, 96.8%] | exploratory | 판정 == expectedReask | recommend 표본(게이트 포함)(F-1) |
| `missRate` 미탐율 | 9/112 (8.0%) | CI95 [4.3%, 14.6%] | confirmatory-primary | 판정 False | expectedReask=true 앵커의 recommend 표본 — 프로덕션은 intent==recommend 인 턴에서만 is_underspecified_turn 을 호출한다(F-1) |
| `missRateUnderExpansionAssumption` 미탐율(전개 가정 상한) | 64/112 (57.1%) | CI95 [47.9%, 65.9%] | exploratory | (판정 False) 또는 (판정 True ∧ detect_expansion_need 가 사유를 돌려줌) | expectedReask=true 앵커의 recommend 표본(F-1) |
...
| what_axis | 60/64 (93.8%) | [85.0%, 97.5%] |
...
| constraint_budget_set | 20/32 (62.5%) (exploratory: N<40) | [45.3%, 77.1%] |
| what_axis | 0/0 (해당 없음) | N/A |
| blocking_rating | 0/0 (해당 없음) | N/A |
| multiturn_gate | 0/0 (해당 없음) | N/A |
...
| 슬라이스 | 점수 | CI95 |
|---|---|---|
| no_condition | 35/37 (94.6%) (exploratory: N<40) | [82.3%, 98.5%] |
| constraint_price | 1/35 (2.9%) (exploratory: N<40) | [0.5%, 14.5%] |
| constraint_budget_set | 19/31 (61.3%) (exploratory: N<40) | [43.8%, 76.3%] |
| what_axis | 2/4 (50.0%) (exploratory: N<40) | [15.0%, 85.0%] |
| blocking_rating | 0/0 (해당 없음) | N/A |
| multiturn_gate | 0/0 (해당 없음) | N/A |
...
outcome 분포: {'correctReject': 124, 'falseAlarm': 4, 'hit': 103, 'miss': 9}

### 미탐 원인 축(ablation 이 뒤집은 축 — 복수 가능, `multiple` = 단일 축으로 안 뒤집힘)

| 축 | 미탐 표본 수 |
|---|---|
| `filters.attrConditions` | 5 |
| `multiple` | 3 |
...
|---|---|
| `categoryQueries;semanticQueryIsFallback` | 3 |
| `filters.attrConditions` | 5 |
| `semanticQueryIsFallback` | 1 |
...
- `expansionGateWouldFireRate`: 57/107 (53.3%)
- `missRateUnderExpansionAssumption`: 64/112 (57.1%)
