...
| constraint_price | 11/40 (27.5%) | [16.1%, 42.8%] |
...
|---|---|---|---|---|---|
| `expansionGateWouldFireRate` 전개 게이트 발동률(판정 True 표본 중) | 40/94 (42.6%) | CI95 [33.0%, 52.6%] | exploratory | detect_expansion_need(...) 가 사유를 돌려준 표본 | 판정 True 인 recommend 표본(F-1) |
...
| `judgmentAccuracy` 판정 정확도 | 198/216 (91.7%) | CI95 [87.2%, 94.7%] | exploratory | 판정 == expectedReask | recommend 표본(게이트 제외)(F-1) |
| `judgmentAccuracyWithGateSlice` 판정 정확도 | 222/240 (92.5%) | CI95 [88.5%, 95.2%] | exploratory | 판정 == expectedReask | recommend 표본(게이트 포함)(F-1) |
| `missRate` 미탐율 | 18/112 (16.1%) | CI95 [10.4%, 24.0%] | confirmatory-primary | 판정 False | expectedReask=true 앵커의 recommend 표본 — 프로덕션은 intent==recommend 인 턴에서만 is_underspecified_turn 을 호출한다(F-1) |
...
| `missRateWithNonRecommendIntent` 미탐율(비-recommend 포함, 참고용) | 18/112 (16.1%) | CI95 [10.4%, 24.0%] | exploratory | 판정 False | expectedReask=true 앵커의 표본 전부(intent 무관, 포함판) |
...
| constraint_price | 29/40 (72.5%) | [57.2%, 83.9%] |
...
| 슬라이스 | 점수 | CI95 |
|---|---|---|
| no_condition | 29/37 (78.4%) (exploratory: N<40) | [62.8%, 88.6%] |
| constraint_price | 0/29 (0.0%) (exploratory: N<40) | [0.0%, 11.7%] |
...
outcome 분포: {'correctReject': 128, 'hit': 94, 'miss': 18}

### 미탐 원인 축(ablation 이 뒤집은 축 — 복수 가능, `multiple` = 단일 축으로 안 뒤집힘)

| 축 | 미탐 표본 수 |
|---|---|
| `filters.attrConditions` | 6 |
| `multiple` | 6 |
...
|---|---|
| `categoryQueries;semanticQueryIsFallback` | 6 |
| `filters.attrConditions` | 6 |
| `semanticQueryIsFallback` | 6 |
...
- `expansionGateWouldFireRate`: 40/94 (42.6%)
