...
| 슬라이스 | 점수 | CI95 |
|---|---|---|
| no_condition | 39/39 (100.0%) (exploratory: N<40) | [91.0%, 100.0%] |
...
|---|---|---|---|---|---|
| `expansionGateWouldFireRate` 전개 게이트 발동률(판정 True 표본 중) | 1/1 (100.0%) | CI95 [20.7%, 100.0%] | exploratory | detect_expansion_need(...) 가 사유를 돌려준 표본 | 판정 True 인 recommend 표본(F-1) |
...
| `judgmentAccuracy` 판정 정확도 | 105/215 (48.8%) | CI95 [42.2%, 55.5%] | exploratory | 판정 == expectedReask | recommend 표본(게이트 제외)(F-1) |
| `judgmentAccuracyWithGateSlice` 판정 정확도 | 129/239 (54.0%) | CI95 [47.6%, 60.2%] | exploratory | 판정 == expectedReask | recommend 표본(게이트 포함)(F-1) |
| `missRate` 미탐율 | 110/111 (99.1%) | CI95 [95.1%, 99.8%] | confirmatory-primary | 판정 False | expectedReask=true 앵커의 recommend 표본 — 프로덕션은 intent==recommend 인 턴에서만 is_underspecified_turn 을 호출한다(F-1) |
| `missRateUnderExpansionAssumption` 미탐율(전개 가정 상한) | 111/111 (100.0%) | CI95 [96.7%, 100.0%] | exploratory | (판정 False) 또는 (판정 True ∧ detect_expansion_need 가 사유를 돌려줌) | expectedReask=true 앵커의 recommend 표본(F-1) |
| `missRateWithNonRecommendIntent` 미탐율(비-recommend 포함, 참고용) | 110/112 (98.2%) | CI95 [93.7%, 99.5%] | exploratory | 판정 False | expectedReask=true 앵커의 표본 전부(intent 무관, 포함판) |
...
| 슬라이스 | 점수 | CI95 |
|---|---|---|
| no_condition | 0/39 (0.0%) (exploratory: N<40) | [0.0%, 9.0%] |
...
| no_condition | 0/0 (해당 없음) | N/A |
| constraint_price | 0/0 (해당 없음) | N/A |
| constraint_budget_set | 1/1 (100.0%) (exploratory: N<40) | [20.7%, 100.0%] |
| what_axis | 0/0 (해당 없음) | N/A |
| blocking_rating | 0/0 (해당 없음) | N/A |
| multiturn_gate | 0/0 (해당 없음) | N/A |
...
|---|---|---|
| **전체(게이트 제외)** | 48.8% | 48.4% |
| no_condition | 0.0% (exploratory: N<40) | 0.0% |
| constraint_price | 0.0% | 0.0% |
| constraint_budget_set | 3.1% (exploratory: N<40) | 0.0% |
...
outcome 분포: {'correctReject': 128, 'hit': 2, 'miss': 110}
...
|---|---|
| `categoryQueries;semanticQueryIsFallback` | 9 |
| `filters.attrConditions;semanticQueryIsFallback` | 4 |
| `semanticQueryIsFallback` | 97 |
...
- `nonRecommendIntentCount`(앵커별·intent별, F-1): {'under-nc-0002': {'general': 1}} — intent != 'recommend' 표본 수(앵커별·intent별) — 프로덕션은 이 표본에서 is_underspecified_turn 에 도달하지 않는다(F-1). confirmatory 축 분모에서 제외된 표본의 노출 크기이며, 그 실패는 intent 라우팅 축(evals/intent_probe)의 소관이다.
- `expansionGateWouldFireRate`: 1/1 (100.0%)
- `missRateUnderExpansionAssumption`: 111/111 (100.0%)
