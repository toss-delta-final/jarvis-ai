...
| constraint_price | 12/40 (30.0%) | [18.1%, 45.4%] |
...
|---|---|---|---|---|---|
| `expansionGateWouldFireRate` 전개 게이트 발동률(판정 True 표본 중) | 44/95 (46.3%) | CI95 [36.6%, 56.3%] | exploratory | detect_expansion_need(...) 가 사유를 돌려준 표본 | 판정 True 인 recommend 표본(F-1) |
...
| `judgmentAccuracy` 판정 정확도 | 199/216 (92.1%) | CI95 [87.8%, 95.0%] | exploratory | 판정 == expectedReask | recommend 표본(게이트 제외)(F-1) |
| `judgmentAccuracyWithGateSlice` 판정 정확도 | 223/240 (92.9%) | CI95 [89.0%, 95.5%] | exploratory | 판정 == expectedReask | recommend 표본(게이트 포함)(F-1) |
...
| `missRateUnderExpansionAssumption` 미탐율(전개 가정 상한) | 61/112 (54.5%) | CI95 [45.2%, 63.4%] | exploratory | (판정 False) 또는 (판정 True ∧ detect_expansion_need 가 사유를 돌려줌) | expectedReask=true 앵커의 recommend 표본(F-1) |
...
| constraint_price | 28/40 (70.0%) | [54.6%, 81.9%] |
...
| 슬라이스 | 점수 | CI95 |
|---|---|---|
| no_condition | 32/40 (80.0%) | [65.2%, 89.5%] |
| constraint_price | 12/40 (30.0%) | [18.1%, 45.4%] |
...
| 슬라이스 | 점수 | CI95 |
|---|---|---|
| no_condition | 30/38 (78.9%) (exploratory: N<40) | [63.7%, 88.9%] |
| constraint_price | 0/28 (0.0%) (exploratory: N<40) | [0.0%, 12.1%] |
| constraint_budget_set | 14/29 (48.3%) (exploratory: N<40) | [31.4%, 65.6%] |
| what_axis | 0/0 (해당 없음) | N/A |
| blocking_rating | 0/0 (해당 없음) | N/A |
| multiturn_gate | 0/0 (해당 없음) | N/A |
...
outcome 분포: {'correctReject': 128, 'hit': 95, 'miss': 17}

### 미탐 원인 축(ablation 이 뒤집은 축 — 복수 가능, `multiple` = 단일 축으로 안 뒤집힘)

| 축 | 미탐 표본 수 |
|---|---|
| `filters.attrConditions` | 8 |
| `multiple` | 7 |
...
| `filters.attrConditions` | 8 |
| `filters.attrConditions;semanticQueryIsFallback` | 2 |
| `semanticQueryIsFallback` | 2 |
...
- `expansionGateWouldFireRate`: 44/95 (46.3%)
- `missRateUnderExpansionAssumption`: 61/112 (54.5%)
