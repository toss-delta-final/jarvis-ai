...
| 슬라이스 | 점수 | CI95 |
|---|---|---|
| no_condition | 5/40 (12.5%) | [5.5%, 26.1%] |
...
|---|---|---|---|---|---|
| `expansionGateWouldFireRate` 전개 게이트 발동률(판정 True 표본 중) | 56/101 (55.4%) | CI95 [45.7%, 64.8%] | exploratory | detect_expansion_need(...) 가 사유를 돌려준 표본 | 판정 True 인 recommend 표본(F-1) |
...
| `judgmentAccuracy` 판정 정확도 | 201/216 (93.1%) | CI95 [88.9%, 95.7%] | exploratory | 판정 == expectedReask | recommend 표본(게이트 제외)(F-1) |
| `judgmentAccuracyWithGateSlice` 판정 정확도 | 225/240 (93.8%) | CI95 [89.9%, 96.2%] | exploratory | 판정 == expectedReask | recommend 표본(게이트 포함)(F-1) |
| `missRate` 미탐율 | 13/112 (11.6%) | CI95 [6.9%, 18.9%] | confirmatory-primary | 판정 False | expectedReask=true 앵커의 recommend 표본 — 프로덕션은 intent==recommend 인 턴에서만 is_underspecified_turn 을 호출한다(F-1) |
| `missRateUnderExpansionAssumption` 미탐율(전개 가정 상한) | 67/112 (59.8%) | CI95 [50.6%, 68.4%] | exploratory | (판정 False) 또는 (판정 True ∧ detect_expansion_need 가 사유를 돌려줌) | expectedReask=true 앵커의 recommend 표본(F-1) |
| `missRateWithNonRecommendIntent` 미탐율(비-recommend 포함, 참고용) | 13/112 (11.6%) | CI95 [6.9%, 18.9%] | exploratory | 판정 False | expectedReask=true 앵커의 표본 전부(intent 무관, 포함판) |
...
| 슬라이스 | 점수 | CI95 |
|---|---|---|
| no_condition | 35/40 (87.5%) | [73.9%, 94.5%] |
...
| 슬라이스 | 점수 | CI95 |
|---|---|---|
| no_condition | 33/35 (94.3%) (exploratory: N<40) | [81.4%, 98.4%] |
| constraint_price | 0/35 (0.0%) (exploratory: N<40) | [0.0%, 9.9%] |
| constraint_budget_set | 21/29 (72.4%) (exploratory: N<40) | [54.3%, 85.3%] |
| what_axis | 2/2 (100.0%) (exploratory: N<40) | [34.2%, 100.0%] |
| blocking_rating | 0/0 (해당 없음) | N/A |
| multiturn_gate | 0/0 (해당 없음) | N/A |
...
outcome 분포: {'correctReject': 126, 'falseAlarm': 2, 'hit': 99, 'miss': 13}
...
- `expansionGateWouldFireRate`: 56/101 (55.4%)
- `missRateUnderExpansionAssumption`: 67/112 (59.8%)
