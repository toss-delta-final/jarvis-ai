...
|---|---|---|---|---|---|
| `expansionGateWouldFireRate` 전개 게이트 발동률(판정 True 표본 중) | 52/105 (49.5%) | CI95 [40.1%, 58.9%] | exploratory | detect_expansion_need(...) 가 사유를 돌려준 표본 | 판정 True 인 recommend 표본(F-1) |
...
| `missRateUnderExpansionAssumption` 미탐율(전개 가정 상한) | 60/112 (53.6%) | CI95 [44.4%, 62.5%] | exploratory | (판정 False) 또는 (판정 True ∧ detect_expansion_need 가 사유를 돌려줌) | expectedReask=true 앵커의 recommend 표본(F-1) |
...
outcome 분포: {'correctReject': 127, 'falseAlarm': 1, 'hit': 104, 'miss': 8}
...
- `expansionGateWouldFireRate`: 52/105 (49.5%)
- `missRateUnderExpansionAssumption`: 60/112 (53.6%)
