...
**짝이 되는 실 LLM 좁은 격자는 아직 없다.** 2026-08-09 시점에 이 저장소 자격증명으로는 실행이
불가능했다 — OpenAI는 `credit_balance_exhausted`(크레딧 소진), Anthropic 키는
`authentication_error`(401)다. 즉 위 표의 숫자에서 **벤더 지연 기여분을 아직 뺄 수 없다**.
크레딧이 복구되면 같은 조건(같은 Spring·같은 시드·같은 커밋)에서 동시성 1~5 × 30건만 좁게 뜬 뒤
두 baseline의 차이를 벤더 지연 기여분으로 기록한다.
