-- 카테고리 사전 부트스트랩 시드 (이슈 #401) — leaf 1007개, embedding 은 NULL.
--
-- ⚠️ 이 파일은 생성물이다 — 손으로 고치지 마라. `scripts/derive_category_seed.py` 로 재생성한다.
-- 정본은 `db/catalog/seed/categories.json`(같은 원천에서 파생, 자세한 절차는
-- `db/catalog/seed/README.md`). 여기서는 행만 만든다(embedding NULL) — 임베딩은 2단계 배치
-- (`app.pipelines.category_seed.seed_from_file`)가 채운다(02_categories.sql 관례 그대로).
--
-- docker-entrypoint-initdb.d 는 컨테이너가 "완전히 새로" 뜰 때(빈 볼륨) 1회만 실행한다 —
-- 이미 만들어진 컨테이너의 볼륨에는 자동 반영되지 않는다. 기존 DB 에는 이렇게 수동 적용한다:
-- docker exec -i jarvis-ai-pg-catalog-1 psql -U jarvis -d catalog < db/catalog/init/04_categories_seed.sql

INSERT INTO categories (category) VALUES
    ('DIY자재/용품 > 반제품'),
    ('DIY자재/용품 > 벽지'),
    ('PC부품 > CPU'),
    ('PC부품 > 노트북 메모리(RAM)'),
    ('PC부품 > 데스크탑 메모리(RAM)'),
    ('TV > 106cm 이하 (~42인치)'),
    ('TV > 107~137cm (43~54인치)'),
    ('TV > 138~175cm (55~69인치)'),
    ('TV > 176~213cm (70~84인치)'),
    ('TV > 215cm 이상 (85인치~)'),
    ('TV > TV주변기기'),
    ('TV > 리퍼/전시/중고'),
    ('TV > 사이니지 TV'),
    ('e쿠폰/상품권 > 레스토랑/뷔페'),
    ('e쿠폰/상품권 > 맛집/분식'),
    ('e쿠폰/상품권 > 모바일 상품권'),
    ('e쿠폰/상품권 > 문화/영화/교육'),
    ('e쿠폰/상품권 > 버거/샌드위치'),
    ('e쿠폰/상품권 > 베이커리/도넛'),
    ('e쿠폰/상품권 > 아이스크림/빙수'),
    ('e쿠폰/상품권 > 카페/음료'),
    ('e쿠폰/상품권 > 편의점'),
    ('e쿠폰/상품권 > 피자/치킨'),
    ('가공식품 > 드레싱'),
    ('가공식품 > 면류/국수'),
    ('가공식품 > 밀가루/홈베이킹/믹스'),
    ('가공식품 > 소금'),
    ('가공식품 > 소스'),
    ('가공식품 > 수입라면'),
    ('가공식품 > 시럽/조청'),
    ('가공식품 > 식용유/오일'),
    ('가공식품 > 잼'),
    ('가공식품 > 조미료/양념'),
    ('가공식품 > 컵라면'),
    ('가정의료용품 > 거즈'),
    ('가정의료용품 > 밴드'),
    ('가정의료용품 > 벌레방어용품'),
    ('가정의료용품 > 붕대'),
    ('가정의료용품 > 상처소독/연고'),
    ('가정의료용품 > 절단기'),
    ('가정의료용품 > 탈지면'),
    ('가정의료용품 > 파스'),
    ('가정의료용품 > 휴대용 약통'),
    ('강아지용품 > 간식'),
    ('강아지용품 > 사료'),
    ('강아지용품 > 영양제'),
    ('거실가구 > 거실장/TV장'),
    ('거실가구 > 소파'),
    ('거실가구 > 의자'),
    ('거실가구 > 테이블'),
    ('건강관리용품 > 구강관리용품'),
    ('건강관리용품 > 두피관리용품'),
    ('건강관리용품 > 디스크/관절'),
    ('건강관리용품 > 뜸/뜸기구'),
    ('건강관리용품 > 물리치료용품'),
    ('건강관리용품 > 부항기'),
    ('건강관리용품 > 손소독제'),
    ('건강관리용품 > 수액시트'),
    ('건강관리용품 > 수지침'),
    ('건강관리용품 > 숙면용품'),
    ('건강관리용품 > 숯용품'),
    ('건강관리용품 > 지압기'),
    ('건강관리용품 > 코건강관리용품'),
    ('건강식품 > 인삼/한방재료'),
    ('건강식품 > 홍삼'),
    ('건강측정용품 > 기타 측정용품'),
    ('건강측정용품 > 만보계'),
    ('건강측정용품 > 신장계'),
    ('건강측정용품 > 체온계'),
    ('건강측정용품 > 체외진단기'),
    ('건강측정용품 > 체중계'),
    ('건강측정용품 > 혈압계'),
    ('건과/견과 > 건과일/말랭이'),
    ('건과/견과 > 견과류'),
    ('게임 > PC게임'),
    ('게임 > 게임기'),
    ('게임 > 게임기 주변기기'),
    ('계절가전 > 선풍기'),
    ('계절가전 > 에어컨'),
    ('계절가전 > 제습기'),
    ('고양이용품 > 간식'),
    ('고양이용품 > 배변/위생용품'),
    ('고양이용품 > 사료'),
    ('고양이용품 > 영양제'),
    ('곡물 > 쌀'),
    ('곡물 > 잡곡/혼합곡'),
    ('골프 > 골프클럽'),
    ('골프 > 골프피팅용품'),
    ('공구 > 전동공구'),
    ('공구 > 절삭공구'),
    ('과일 > 과일선물세트'),
    ('과일 > 국산과일'),
    ('과일 > 냉동/간편과일'),
    ('과일 > 수입과일'),
    ('과자/간식 > 빵/베이커리'),
    ('과자/간식 > 스낵/시리얼'),
    ('과자/간식 > 원물간식'),
    ('과자/간식 > 초콜릿/사탕/젤리/껌'),
    ('관상어/수족관 > 관상어'),
    ('관상어/수족관 > 사료'),
    ('관상어/수족관 > 수조/어항'),
    ('구기/라켓/스포츠 > 야구'),
    ('구기/라켓/스포츠 > 축구'),
    ('기저귀 > 수영장기저귀'),
    ('기저귀 > 일회용기저귀'),
    ('기저귀 > 천기저귀'),
    ('기저귀 > 천기저귀 액세서리'),
    ('김치 > 김치 기타'),
    ('김치 > 맛김치/볶음김치'),
    ('김치 > 별미김치'),
    ('김치 > 여행용/소포장 김치'),
    ('김치 > 포기김치'),
    ('꽃/원예 > 꽃/식물'),
    ('꽃배달 > 근조화환'),
    ('꽃배달 > 꽃다발'),
    ('꽃배달 > 꽃상자'),
    ('꽃배달 > 비누꽃다발'),
    ('꽃배달 > 웨딩플라워/부케'),
    ('꽃배달 > 축하화환'),
    ('낚시 > 낚싯대'),
    ('낚시 > 루어낚시/바늘'),
    ('낚시 > 민물낚시'),
    ('남성가방 > 가방액세서리'),
    ('남성가방 > 노트북가방'),
    ('남성가방 > 백팩'),
    ('남성가방 > 서류가방'),
    ('남성가방 > 숄더백'),
    ('남성가방 > 스포츠가방'),
    ('남성가방 > 에코백'),
    ('남성가방 > 카메라가방'),
    ('남성가방 > 크로스백'),
    ('남성가방 > 클러치백'),
    ('남성가방 > 토트백'),
    ('남성가방 > 히프색/슬링백'),
    ('남성신발 > 기능화'),
    ('남성신발 > 레인부츠'),
    ('남성신발 > 로퍼'),
    ('남성신발 > 모카신'),
    ('남성신발 > 보트슈즈'),
    ('남성신발 > 부츠'),
    ('남성신발 > 샌들'),
    ('남성신발 > 수제화'),
    ('남성신발 > 스니커즈'),
    ('남성신발 > 슬리퍼'),
    ('남성신발 > 슬립온'),
    ('남성신발 > 신발용품'),
    ('남성신발 > 운동화'),
    ('남성신발 > 워커'),
    ('남성신발 > 웰트화'),
    ('남성신발 > 정장구두'),
    ('남성의류 > 니트/스웨터'),
    ('남성의류 > 맨투맨'),
    ('남성의류 > 바지/팬츠'),
    ('남성의류 > 비치웨어'),
    ('남성의류 > 셔츠/남방'),
    ('남성의류 > 재킷'),
    ('남성의류 > 점퍼'),
    ('남성의류 > 정장/슈트'),
    ('남성의류 > 조끼/베스트'),
    ('남성의류 > 청바지'),
    ('남성의류 > 카디건'),
    ('남성의류 > 코트'),
    ('남성의류 > 테마의류'),
    ('남성의류 > 트레이닝복'),
    ('남성의류 > 티셔츠'),
    ('남성의류 > 후드티'),
    ('남성화장품 > 남성BB크림'),
    ('남성화장품 > 남성로션'),
    ('남성화장품 > 남성마스크팩'),
    ('남성화장품 > 남성메이크업'),
    ('남성화장품 > 남성선케어'),
    ('남성화장품 > 남성스킨'),
    ('남성화장품 > 남성에센스'),
    ('남성화장품 > 남성크림'),
    ('남성화장품 > 남성클렌징'),
    ('남성화장품 > 남성화장품세트'),
    ('남성화장품 > 올인원'),
    ('냉장/냉동식품 > 국/탕/찌개'),
    ('냉장/냉동식품 > 밥류'),
    ('냉장/냉동식품 > 안주류'),
    ('냉장/냉동식품 > 즉석요리'),
    ('냉장/냉동식품 > 핫도그'),
    ('냉장/냉동식품 > 햄/소시지/베이컨'),
    ('냉장고 > 4도어 냉장고'),
    ('냉장고 > 김치 냉장고'),
    ('냉장고 > 냉동고'),
    ('냉장고 > 리퍼/전시/중고'),
    ('냉장고 > 양문형 냉장고'),
    ('냉장고 > 업소용 냉장고'),
    ('냉장고 > 일반형 냉장고'),
    ('냉장고 > 전용냉장고'),
    ('네일케어 > 네일리무버'),
    ('네일케어 > 네일아트용품'),
    ('네일케어 > 네일영양제'),
    ('네일케어 > 네일케어도구'),
    ('네일케어 > 네일팁/스티커'),
    ('네일케어 > 매니큐어'),
    ('네일케어 > 젤네일'),
    ('노트북 > ASUS'),
    ('노트북 > LG전자')
ON CONFLICT (category) DO NOTHING;

INSERT INTO categories (category) VALUES
    ('노트북 > 삼성전자'),
    ('눈건강용품 > 기타 관리용품'),
    ('눈건강용품 > 렌즈세척기'),
    ('눈건강용품 > 소프트렌즈관리용품'),
    ('눈건강용품 > 하드렌즈관리용품'),
    ('다이어트식품 > 다이어트 보조식품'),
    ('다이어트식품 > 다이어트차/음료'),
    ('다이어트식품 > 뷰티푸드'),
    ('당뇨관리용품 > 침/바늘'),
    ('당뇨관리용품 > 혈당계'),
    ('당뇨관리용품 > 혈당측정지'),
    ('데스크탑 > 브랜드PC'),
    ('데스크탑 > 올인원PC'),
    ('도서/음반 > 어린이'),
    ('도서/음반 > 유아'),
    ('도서/음반 > 유아동 기획세트'),
    ('등산/아웃도어 > 남성등산복'),
    ('등산/아웃도어 > 남아 아웃도어'),
    ('등산/아웃도어 > 여성등산복'),
    ('렌털/구독 서비스 > 가전렌털'),
    ('렌털/구독 서비스 > 리빙렌털'),
    ('렌털/구독 서비스 > 서비스가입/구독'),
    ('리모델링가구 > 시공/리모델링'),
    ('마스크 > KF인증마스크'),
    ('마스크 > 덴탈/일회용마스크'),
    ('마스크 > 방한/패션마스크'),
    ('마스크 > 특수마스크'),
    ('메이크업 > 립메이크업'),
    ('메이크업 > 바디메이크업'),
    ('메이크업 > 아이메이크업'),
    ('메이크업 > 치크/하이라이터'),
    ('메이크업 > 페이스메이크업'),
    ('모니터 > TV 모니터'),
    ('모니터 > 게이밍 모니터'),
    ('모니터 > 일반 모니터'),
    ('몸매관리용품 > 가슴관리'),
    ('몸매관리용품 > 기타 체형관리'),
    ('몸매관리용품 > 다리관리'),
    ('몸매관리용품 > 바른자세용품'),
    ('몸매관리용품 > 얼굴관리'),
    ('문구/사무용품 > 문구용품'),
    ('물티슈 > 기능성 물티슈'),
    ('물티슈 > 물티슈'),
    ('물티슈 > 물티슈액세서리'),
    ('물티슈 > 비데물티슈'),
    ('밀키트/쿠킹박스 > 면/파스타'),
    ('밀키트/쿠킹박스 > 분식/야식'),
    ('밀키트/쿠킹박스 > 양식'),
    ('밀키트/쿠킹박스 > 일식'),
    ('밀키트/쿠킹박스 > 중식/동남아'),
    ('밀키트/쿠킹박스 > 퓨전/기타'),
    ('밀키트/쿠킹박스 > 한식'),
    ('바디케어 > 데오드란트'),
    ('바디케어 > 바디로션'),
    ('바디케어 > 바디미스트'),
    ('바디케어 > 바디스크럽'),
    ('바디케어 > 바디슬리밍'),
    ('바디케어 > 바디오일'),
    ('바디케어 > 바디워시'),
    ('바디케어 > 바디크림'),
    ('바디케어 > 비누'),
    ('바디케어 > 생활선물세트'),
    ('바디케어 > 샤워코롱'),
    ('바디케어 > 제모/왁싱'),
    ('바디케어 > 태닝용품'),
    ('바디케어 > 풋케어'),
    ('바디케어 > 핸드크림'),
    ('반찬/젓갈 > 반찬'),
    ('반찬/젓갈 > 젓갈'),
    ('병원/의료용품 > 가위'),
    ('병원/의료용품 > 겸자류'),
    ('병원/의료용품 > 기타 검진기구/소모품'),
    ('병원/의료용품 > 라이트'),
    ('병원/의료용품 > 루페'),
    ('병원/의료용품 > 링거대'),
    ('병원/의료용품 > 메스대'),
    ('병원/의료용품 > 병실용침대'),
    ('병원/의료용품 > 병원용 혈압계'),
    ('병원/의료용품 > 산소통'),
    ('병원/의료용품 > 석션기/네뷸라이저'),
    ('병원/의료용품 > 설압자'),
    ('병원/의료용품 > 신체검사용품'),
    ('병원/의료용품 > 청진기'),
    ('분유 > 산양분유'),
    ('분유 > 특수분유'),
    ('뷰티소품 > 기름종이'),
    ('뷰티소품 > 눈썹칼'),
    ('뷰티소품 > 마사지소품'),
    ('뷰티소품 > 마스크시트'),
    ('뷰티소품 > 메이크업소품'),
    ('뷰티소품 > 메이크업정리함'),
    ('뷰티소품 > 미용가위'),
    ('뷰티소품 > 브러쉬'),
    ('뷰티소품 > 속눈썹뷰러'),
    ('뷰티소품 > 타투스티커'),
    ('뷰티소품 > 퍼프'),
    ('뷰티소품 > 피부관리기'),
    ('뷰티소품 > 화장솜'),
    ('뷰티소품 > 화장품용기'),
    ('뷰티소품 > 화장품파우치'),
    ('브랜드 남성가방 > 가방소품'),
    ('브랜드 남성가방 > 노트북가방'),
    ('브랜드 남성가방 > 백팩'),
    ('브랜드 남성가방 > 서류가방'),
    ('브랜드 남성가방 > 숄더백'),
    ('브랜드 남성가방 > 크로스백'),
    ('브랜드 남성가방 > 클러치백'),
    ('브랜드 남성가방 > 토트백'),
    ('브랜드 남성가방 > 파우치'),
    ('브랜드 남성가방 > 히프색'),
    ('브랜드 남성신발 > 로퍼'),
    ('브랜드 남성신발 > 샌들'),
    ('브랜드 남성신발 > 스니커즈'),
    ('브랜드 남성신발 > 슬립온'),
    ('브랜드 남성신발 > 신발소품'),
    ('브랜드 남성신발 > 운동화'),
    ('브랜드 남성신발 > 워커/부츠'),
    ('브랜드 남성신발 > 정장구두'),
    ('브랜드 남성신발 > 조리/슬리퍼'),
    ('브랜드 남성신발 > 캔버스화'),
    ('브랜드 남성신발 > 컴포트화/기능성화'),
    ('브랜드 남성신발 > 키높이슈즈'),
    ('브랜드 남성의류 > 니트/스웨터'),
    ('브랜드 남성의류 > 바지/팬츠'),
    ('브랜드 남성의류 > 셔츠/남방'),
    ('브랜드 남성의류 > 재킷'),
    ('브랜드 남성의류 > 점퍼'),
    ('브랜드 남성의류 > 정장/슈트'),
    ('브랜드 남성의류 > 조끼/베스트'),
    ('브랜드 남성의류 > 카디건'),
    ('브랜드 남성의류 > 코트'),
    ('브랜드 남성의류 > 티셔츠'),
    ('브랜드 시계 > 브랜드 남성시계'),
    ('브랜드 시계 > 브랜드 아동시계'),
    ('브랜드 시계 > 브랜드 여성시계'),
    ('브랜드 시계 > 시계소품'),
    ('브랜드 언더웨어 > 남성 언더웨어'),
    ('브랜드 언더웨어 > 여성 언더웨어'),
    ('브랜드 여성가방 > 가방소품'),
    ('브랜드 여성가방 > 노트북가방'),
    ('브랜드 여성가방 > 백팩'),
    ('브랜드 여성가방 > 서류가방'),
    ('브랜드 여성가방 > 숄더백'),
    ('브랜드 여성가방 > 크로스백'),
    ('브랜드 여성가방 > 클러치백'),
    ('브랜드 여성가방 > 토트백'),
    ('브랜드 여성가방 > 파우치'),
    ('브랜드 여성가방 > 히프색'),
    ('브랜드 여성신발 > 가죽부츠'),
    ('브랜드 여성신발 > 레인부츠'),
    ('브랜드 여성신발 > 로퍼'),
    ('브랜드 여성신발 > 부티'),
    ('브랜드 여성신발 > 샌들'),
    ('브랜드 여성신발 > 스니커즈'),
    ('브랜드 여성신발 > 슬립온'),
    ('브랜드 여성신발 > 신발소품'),
    ('브랜드 여성신발 > 오픈슈즈'),
    ('브랜드 여성신발 > 운동화'),
    ('브랜드 여성신발 > 조리/슬리퍼'),
    ('브랜드 여성신발 > 캔버스화'),
    ('브랜드 여성신발 > 컴포트화/기능성화'),
    ('브랜드 여성신발 > 키높이슈즈'),
    ('브랜드 여성신발 > 털부츠'),
    ('브랜드 여성신발 > 패딩부츠'),
    ('브랜드 여성신발 > 펌프스/힐'),
    ('브랜드 여성신발 > 플랫슈즈'),
    ('브랜드 여성의류 > 니트/스웨터'),
    ('브랜드 여성의류 > 레깅스'),
    ('브랜드 여성의류 > 바지/팬츠'),
    ('브랜드 여성의류 > 베스트/조끼'),
    ('브랜드 여성의류 > 블라우스'),
    ('브랜드 여성의류 > 셔츠'),
    ('브랜드 여성의류 > 스커트/치마'),
    ('브랜드 여성의류 > 원피스'),
    ('브랜드 여성의류 > 재킷'),
    ('브랜드 여성의류 > 점퍼'),
    ('브랜드 여성의류 > 정장'),
    ('브랜드 여성의류 > 청바지'),
    ('브랜드 여성의류 > 카디건'),
    ('브랜드 여성의류 > 코트'),
    ('브랜드 여성의류 > 트레이닝복'),
    ('브랜드 여성의류 > 티셔츠'),
    ('브랜드 여행가방 > 보스턴가방'),
    ('브랜드 여행가방 > 소프트 캐리어'),
    ('브랜드 여행가방 > 여권케이스'),
    ('브랜드 여행가방 > 여행가방 세트'),
    ('브랜드 여행가방 > 여행용 소품'),
    ('브랜드 여행가방 > 하드 캐리어'),
    ('브랜드 잡화/소품 > 기타 액세서리'),
    ('브랜드 잡화/소품 > 넥타이'),
    ('브랜드 잡화/소품 > 머플러'),
    ('브랜드 잡화/소품 > 모자'),
    ('브랜드 잡화/소품 > 선글라스/안경테'),
    ('브랜드 잡화/소품 > 손수건'),
    ('브랜드 잡화/소품 > 숄'),
    ('브랜드 잡화/소품 > 스카프'),
    ('브랜드 잡화/소품 > 스타킹'),
    ('브랜드 잡화/소품 > 양말'),
    ('브랜드 잡화/소품 > 양산'),
    ('브랜드 잡화/소품 > 우산')
ON CONFLICT (category) DO NOTHING;

INSERT INTO categories (category) VALUES
    ('브랜드 잡화/소품 > 장갑'),
    ('브랜드 주얼리 > 브랜드 남성주얼리'),
    ('브랜드 주얼리 > 브랜드 여성주얼리'),
    ('브랜드 주얼리 > 예물'),
    ('브랜드 지갑/벨트 > 남성벨트'),
    ('브랜드 지갑/벨트 > 남성지갑'),
    ('브랜드 지갑/벨트 > 멜빵/서스펜더'),
    ('브랜드 지갑/벨트 > 여성벨트'),
    ('브랜드 지갑/벨트 > 여성지갑'),
    ('생리대/성인기저귀 > 생리대'),
    ('생리대/성인기저귀 > 성인기저귀'),
    ('생리대/성인기저귀 > 체내형/탐폰'),
    ('생활가전 > 비데'),
    ('생활가전 > 청소기'),
    ('생활가전 > 청소기부품'),
    ('생활잡화 > 난방텐트'),
    ('생활잡화 > 모기장'),
    ('생활잡화 > 문풍지/틈막이'),
    ('생활잡화 > 방충망'),
    ('생활잡화 > 에어캡/방풍비닐'),
    ('생활잡화 > 우산꽂이/우산포장기'),
    ('생활잡화 > 해충퇴치 용품'),
    ('선케어 > 선밤'),
    ('선케어 > 선스프레이'),
    ('선케어 > 선크림/선블록'),
    ('선케어 > 선파우더/쿠션'),
    ('선케어 > 셀프태닝/메이크업'),
    ('선케어 > 자외선 차단패치'),
    ('세제/방향/살충 > 세탁세제'),
    ('세제/방향/살충 > 주방세제'),
    ('세제/방향/살충 > 청소세제/세정제'),
    ('세탁기/건조기 > 건조기'),
    ('세탁기/건조기 > 드럼세탁기'),
    ('세탁기/건조기 > 세탁기 건조기 세트'),
    ('세탁기/건조기 > 업소용'),
    ('세탁기/건조기 > 의류관리기'),
    ('세탁기/건조기 > 일반세탁기'),
    ('세탁기/건조기 > 탈수기'),
    ('세탁용품 > 기타 세탁용품'),
    ('세탁용품 > 다리미판'),
    ('세탁용품 > 다림질용품'),
    ('세탁용품 > 빨래건조대'),
    ('세탁용품 > 빨래바구니'),
    ('세탁용품 > 빨래판'),
    ('세탁용품 > 빨랫솔'),
    ('세탁용품 > 빨랫줄'),
    ('세탁용품 > 세탁망'),
    ('세탁용품 > 세탁볼'),
    ('소동물용품 > 토끼용품'),
    ('소동물용품 > 햄스터용품'),
    ('손발건강용품 > 기능성 건강슈즈'),
    ('손발건강용품 > 발관리용품'),
    ('손발건강용품 > 발지압/발매트'),
    ('손발건강용품 > 족욕/족탕기'),
    ('손발건강용품 > 파라핀배스'),
    ('손발건강용품 > 파라핀용액'),
    ('수납가구 > CD/DVD장'),
    ('수납가구 > 기타 수납소품'),
    ('수납가구 > 리퍼/전시/중고'),
    ('수납가구 > 소품 수납정리함'),
    ('수납가구 > 수납선반'),
    ('수납가구 > 수납장'),
    ('수납정리용품 > 리빙박스'),
    ('수납정리용품 > 행거'),
    ('수산 > 건어물'),
    ('수산 > 생선류'),
    ('수산 > 수산선물세트'),
    ('수산 > 해산물'),
    ('수산 > 해조류'),
    ('수영/수상레저 > 비치수영복'),
    ('수영/수상레저 > 실내수영복'),
    ('수영/수상레저 > 실내수영용품'),
    ('수영/수상레저 > 키즈수영복'),
    ('수유용품 > 노리개젖꼭지'),
    ('수유용품 > 모유저장팩'),
    ('수유용품 > 유축기'),
    ('수유용품 > 일회용젖병/비닐팩'),
    ('수유용품 > 젖꼭지'),
    ('수유용품 > 젖병'),
    ('수유용품 > 치발기'),
    ('수입명품 > 골프'),
    ('수입명품 > 남성가방'),
    ('수입명품 > 남성의류'),
    ('수입명품 > 남성지갑'),
    ('수입명품 > 럭셔리 라이프'),
    ('수입명품 > 벨트'),
    ('수입명품 > 선글라스'),
    ('수입명품 > 시계'),
    ('수입명품 > 안경테'),
    ('수입명품 > 여성가방'),
    ('수입명품 > 여성신발'),
    ('수입명품 > 여성의류'),
    ('수입명품 > 여성지갑'),
    ('수입명품 > 여행가방'),
    ('수입명품 > 유아동의류'),
    ('수입명품 > 유아동잡화'),
    ('수입명품 > 쥬얼리'),
    ('수입명품 > 패션소품/잡화'),
    ('순금/돌반지 > 골드바'),
    ('순금/돌반지 > 순금제품'),
    ('순금/돌반지 > 순은제품'),
    ('순금/돌반지 > 실버바'),
    ('스마트기기 > VR기기'),
    ('스마트기기 > 리퍼/전시/중고'),
    ('스마트기기 > 미라캐스트'),
    ('스마트기기 > 스마트램프'),
    ('스마트기기 > 스마트밴드'),
    ('스마트기기 > 스마트스위치'),
    ('스마트기기 > 스마트온도계'),
    ('스마트기기 > 스마트워치'),
    ('스마트기기 > 스마트체중계'),
    ('스마트기기 > 스마트홈캠'),
    ('스마트기기 > 헬스케어'),
    ('스키/보드/겨울스포츠 > 보드복'),
    ('스키/보드/겨울스포츠 > 스키'),
    ('스키/보드/겨울스포츠 > 스키복'),
    ('스킨케어 > 넥크림'),
    ('스킨케어 > 로션/에멀션'),
    ('스킨케어 > 마사지크림'),
    ('스킨케어 > 미스트'),
    ('스킨케어 > 수분크림'),
    ('스킨케어 > 스킨/토너'),
    ('스킨케어 > 스킨케어 세트'),
    ('스킨케어 > 아이크림'),
    ('스킨케어 > 앰플'),
    ('스킨케어 > 에센스/세럼'),
    ('스킨케어 > 탄력크림'),
    ('스킨케어 > 팩/마스크'),
    ('스킨케어 > 페이스오일'),
    ('스킨케어 > 화이트닝크림'),
    ('스포츠 신발 > 운동화/스니커즈'),
    ('스포츠 의류 > 니트/스웨터'),
    ('스포츠 의류 > 운동복/유니폼'),
    ('스포츠 의류 > 저지'),
    ('스포츠 의류 > 집업'),
    ('스포츠 의류 > 트랙재킷'),
    ('스포츠 의류 > 패딩/다운점퍼'),
    ('스포츠 의류 > 플리스'),
    ('스포츠 의류 > 후드티셔츠'),
    ('스포츠 잡화 > 스포츠 글라스'),
    ('스포츠 잡화 > 스포츠 모자'),
    ('스포츠 잡화 > 스포츠 시계'),
    ('스포츠 잡화 > 스포츠 용품'),
    ('시계 > 남성시계'),
    ('시계 > 시계소품'),
    ('시계 > 여성시계'),
    ('신생아의류 (0~24개월) > 배냇저고리'),
    ('신생아의류 (0~24개월) > 상하복세트'),
    ('신생아의류 (0~24개월) > 손발싸개'),
    ('신생아의류 (0~24개월) > 신생아모자/보닛'),
    ('신생아의류 (0~24개월) > 아우터'),
    ('신생아의류 (0~24개월) > 우주복'),
    ('신생아의류 (0~24개월) > 출산선물세트'),
    ('실버용품 > 돋보기'),
    ('실버용품 > 보행보조용품'),
    ('실버용품 > 보행차/실버카'),
    ('실버용품 > 스틸휠체어'),
    ('실버용품 > 알루미늄휠체어'),
    ('실버용품 > 욕창방지매트리스'),
    ('실버용품 > 욕창용품'),
    ('실버용품 > 전동휠체어'),
    ('실버용품 > 침구류'),
    ('실버용품 > 환자용 배변용품'),
    ('아동/주니어가구 > 공부상'),
    ('아동/주니어가구 > 서랍장'),
    ('아동/주니어가구 > 수납장'),
    ('아동/주니어가구 > 옷장'),
    ('아동/주니어가구 > 책상'),
    ('아동/주니어가구 > 침대'),
    ('아웃도어가구 > 기타 아웃도어가구'),
    ('아웃도어가구 > 리퍼/전시/중고'),
    ('아웃도어가구 > 벤치'),
    ('아웃도어가구 > 야외용 테이블'),
    ('아웃도어가구 > 야외용의자'),
    ('악기 > 건반악기'),
    ('악기 > 현악기'),
    ('안마용품 > 기타 안마용품'),
    ('안마용품 > 눈 안마기'),
    ('안마용품 > 다리/발 안마기'),
    ('안마용품 > 대형 안마의자'),
    ('안마용품 > 리퍼/전시/중고'),
    ('안마용품 > 목/어깨 안마기'),
    ('안마용품 > 소형 안마의자'),
    ('안마용품 > 손 안마기'),
    ('안마용품 > 핸드형 안마기'),
    ('안전/보안용품 > 안전보호구'),
    ('안전/보안용품 > 안전시설용품'),
    ('언더웨어/잠옷 > 남성 내의'),
    ('언더웨어/잠옷 > 남성 러닝/팬티 세트'),
    ('언더웨어/잠옷 > 남성 보정속옷'),
    ('언더웨어/잠옷 > 남성 스포츠언더레이어/기능성발열내의'),
    ('언더웨어/잠옷 > 남성 팬티'),
    ('언더웨어/잠옷 > 남성러닝'),
    ('언더웨어/잠옷 > 속바지/나시'),
    ('언더웨어/잠옷 > 여성 내의'),
    ('언더웨어/잠옷 > 여성 보정속옷'),
    ('언더웨어/잠옷 > 여성 브라'),
    ('언더웨어/잠옷 > 여성 브라/팬티 세트'),
    ('언더웨어/잠옷 > 여성 속옷소품'),
    ('언더웨어/잠옷 > 여성 스포츠언더레이어/기능성발열내의')
ON CONFLICT (category) DO NOTHING;

INSERT INTO categories (category) VALUES
    ('언더웨어/잠옷 > 여성 팬티'),
    ('언더웨어/잠옷 > 잠옷/홈웨어'),
    ('언더웨어/잠옷 > 테마속옷'),
    ('여성가방 > 가방액세서리'),
    ('여성가방 > 노트북가방'),
    ('여성가방 > 백팩'),
    ('여성가방 > 비치백'),
    ('여성가방 > 쇼퍼백'),
    ('여성가방 > 숄더백'),
    ('여성가방 > 스포츠가방'),
    ('여성가방 > 에코백'),
    ('여성가방 > 왕골백'),
    ('여성가방 > 크로스백'),
    ('여성가방 > 클러치백'),
    ('여성가방 > 토트백'),
    ('여성가방 > 파우치'),
    ('여성가방 > 히프색/슬링백'),
    ('여성신발 > 기능화'),
    ('여성신발 > 레인부츠'),
    ('여성신발 > 로퍼'),
    ('여성신발 > 부츠'),
    ('여성신발 > 부티'),
    ('여성신발 > 샌들'),
    ('여성신발 > 스니커즈'),
    ('여성신발 > 슬리퍼'),
    ('여성신발 > 슬립온'),
    ('여성신발 > 신발용품'),
    ('여성신발 > 운동화'),
    ('여성신발 > 워커'),
    ('여성신발 > 펌프스/힐'),
    ('여성신발 > 플랫슈즈'),
    ('여성의류 > 니트/스웨터'),
    ('여성의류 > 바지/팬츠'),
    ('여성의류 > 블라우스'),
    ('여성의류 > 비치웨어'),
    ('여성의류 > 셔츠/남방'),
    ('여성의류 > 스커트/치마'),
    ('여성의류 > 원피스'),
    ('여성의류 > 재킷'),
    ('여성의류 > 점퍼'),
    ('여성의류 > 정장세트'),
    ('여성의류 > 조끼/베스트'),
    ('여성의류 > 청바지'),
    ('여성의류 > 카디건'),
    ('여성의류 > 코트'),
    ('여성의류 > 테마의상'),
    ('여성의류 > 트레이닝복'),
    ('여성의류 > 티셔츠'),
    ('여행가방/소품 > 네임택'),
    ('여행가방/소품 > 보스톤가방'),
    ('여행가방/소품 > 보조가방'),
    ('여행가방/소품 > 소프트 캐리어'),
    ('여행가방/소품 > 여권케이스'),
    ('여행가방/소품 > 여행소품'),
    ('여행가방/소품 > 여행파우치'),
    ('여행가방/소품 > 이민/유학용가방'),
    ('여행가방/소품 > 하드 캐리어'),
    ('영상가전 > 리퍼/전시/중고'),
    ('영상가전 > 미니빔'),
    ('영상가전 > 영상플레이어'),
    ('영상가전 > 프로젝터'),
    ('영상가전 > 프로젝터 액세서리'),
    ('오토바이/스쿠터 > 오토바이'),
    ('오토바이/스쿠터 > 오토바이 헬멧'),
    ('오토바이/스쿠터 > 오토바이부품'),
    ('온열/찜질용품 > 기타 찜질용품'),
    ('온열/찜질용품 > 반신욕기'),
    ('온열/찜질용품 > 손난로'),
    ('온열/찜질용품 > 전신사우나'),
    ('온열/찜질용품 > 좌욕기'),
    ('온열/찜질용품 > 좌훈기'),
    ('온열/찜질용품 > 찜질기'),
    ('온열/찜질용품 > 찜질팩'),
    ('온열/찜질용품 > 핫팩'),
    ('외출용품 > 유모차'),
    ('외출용품 > 유모차용품'),
    ('요가/필라테스 > 스트레칭'),
    ('요가/필라테스 > 요가/필라테스용품'),
    ('요가/필라테스 > 요가복'),
    ('요가/필라테스 > 짐볼'),
    ('요가/필라테스 > 폼롤러'),
    ('욕실용품 > 샤워기/수전'),
    ('욕실용품 > 수건/타월'),
    ('욕실용품 > 욕실수납'),
    ('우유/유제품 > 두유'),
    ('우유/유제품 > 요구르트/요거트'),
    ('우유/유제품 > 우유'),
    ('우유/유제품 > 치즈'),
    ('유아가구/침구 > 범퍼/가드'),
    ('유아가구/침구 > 아기의자/범보의자'),
    ('유아가구/침구 > 유아의자'),
    ('유아가구/침구 > 유아책꽂이'),
    ('유아가구/침구 > 유아책상'),
    ('유아가구/침구 > 유아책장'),
    ('유아가구/침구 > 유아침대'),
    ('유아동식/영양제 > 유아동 간식'),
    ('유아동식/영양제 > 유아동 건강식품'),
    ('유아동식/영양제 > 유아동식'),
    ('유아동신발 > 남아신발'),
    ('유아동신발 > 여아신발'),
    ('유아동잡화 > 신생아잡화'),
    ('유아동잡화 > 여아잡화'),
    ('유아목욕/스킨케어 > 유아목욕용품'),
    ('유아세제/위생용품 > 위생/건강용품'),
    ('유아세제/위생용품 > 유아구강용품'),
    ('유아세제/위생용품 > 유아세제'),
    ('유아안전/실내용품 > 놀이방매트'),
    ('유아안전/실내용품 > 로커'),
    ('유아안전/실내용품 > 바운서'),
    ('유아안전/실내용품 > 쏘서'),
    ('유아안전/실내용품 > 유아안전용품'),
    ('유아안전/실내용품 > 점퍼루'),
    ('유아안전/실내용품 > 흔들의자'),
    ('음향가전 > AV케이블'),
    ('음향가전 > 스피커'),
    ('음향가전 > 오디오'),
    ('음향가전 > 이어폰'),
    ('음향가전 > 턴테이블'),
    ('음향가전 > 헤드폰'),
    ('음향가전 > 홈시어터'),
    ('이미용가전 > 고데기'),
    ('이미용가전 > 구강세정기'),
    ('이미용가전 > 에어브러시'),
    ('이미용가전 > 전기면도기'),
    ('이미용가전 > 전동칫솔'),
    ('이미용가전 > 헤어드라이어'),
    ('이유용품 > 과즙망'),
    ('이유용품 > 빨대컵'),
    ('이유용품 > 스푼'),
    ('이유용품 > 아동용물병'),
    ('이유용품 > 아동용컵'),
    ('이유용품 > 이유식조리기'),
    ('이유용품 > 젓가락'),
    ('이유용품 > 턱받이'),
    ('이유용품 > 포크'),
    ('인테리어소품 > 시계'),
    ('인테리어소품 > 액자'),
    ('임부복/소품 > 관리용품'),
    ('임부복/소품 > 수유용의류'),
    ('임부복/소품 > 의류'),
    ('임부복/소품 > 잠옷/속옷'),
    ('임부복/소품 > 잡화'),
    ('자동차기기 > 내비게이션'),
    ('자동차기기 > 내비게이션액세서리'),
    ('자동차기기 > 블랙박스'),
    ('자동차기기 > 블랙박스액세서리'),
    ('자동차기기 > 후방카메라/감지기'),
    ('자동차용품 > 램프'),
    ('자동차용품 > 타이어'),
    ('자동차용품 > 휠'),
    ('자전거 > 자전거'),
    ('자전거 > 자전거관리용품'),
    ('장난감 > 로봇'),
    ('장난감 > 작동/조립완구'),
    ('재활운동용품 > 기타 재활보조용품'),
    ('재활운동용품 > 다리보호대'),
    ('재활운동용품 > 무릎보호대'),
    ('재활운동용품 > 발목보호대'),
    ('재활운동용품 > 손목보호대'),
    ('재활운동용품 > 스포츠/운동용품'),
    ('재활운동용품 > 스포츠테이프'),
    ('재활운동용품 > 의료용압박스타킹'),
    ('재활운동용품 > 자세교정용품'),
    ('재활운동용품 > 재활운동/관리기구'),
    ('재활운동용품 > 팔보호대'),
    ('재활운동용품 > 허리보호대'),
    ('저장장치 > SSD'),
    ('저장장치 > 무선 외장하드'),
    ('저장장치 > 외장HDD'),
    ('저장장치 > 외장하드케이스'),
    ('저주파/적외선용품 > 저주파 안마기'),
    ('저주파/적외선용품 > 저주파 자극기'),
    ('저주파/적외선용품 > 저주파 패드'),
    ('저주파/적외선용품 > 적외선 조사기'),
    ('전기/산업자재 > 보수용품'),
    ('전기/산업자재 > 전기생활용품'),
    ('전동레저/인라인/킥보드 > 보드 용품/잡화'),
    ('전동레저/인라인/킥보드 > 스케이트보드'),
    ('전동레저/인라인/킥보드 > 인라인/롤러스케이트'),
    ('전통식품 > 전통과자/한과'),
    ('전통식품 > 전통떡'),
    ('전통식품 > 전통식 음료'),
    ('전통식품 > 전통장류'),
    ('전통식품 > 전통주'),
    ('조류용품 > 간식'),
    ('조류용품 > 기타 조류용품'),
    ('조류용품 > 모이/사료'),
    ('조류용품 > 모이통/식기'),
    ('조류용품 > 위생/미용용품'),
    ('조류용품 > 자동부화기'),
    ('조류용품 > 조류 영양제'),
    ('조명 > 전구'),
    ('조명 > 조명'),
    ('주니어의류 (9~14세) > 남녀공용의류'),
    ('주니어의류 (9~14세) > 주니어 여아의류'),
    ('주방가구 > 레인지대'),
    ('주방가구 > 식탁'),
    ('주방가구 > 식탁의자'),
    ('주방가구 > 주방수납장'),
    ('주방가전 > 가스레인지')
ON CONFLICT (category) DO NOTHING;

INSERT INTO categories (category) VALUES
    ('주방가전 > 전기레인지'),
    ('주방가전 > 전기밥솥'),
    ('주방용품 > 냄비'),
    ('주방용품 > 프라이팬'),
    ('주방잡화 > 고무장갑'),
    ('주방잡화 > 냄비/컵/수저받침'),
    ('주방잡화 > 빨대'),
    ('주방잡화 > 일회용식기/도시락'),
    ('주얼리 > 귀걸이'),
    ('주얼리 > 남성 주얼리'),
    ('주얼리 > 목걸이/팬던트'),
    ('주얼리 > 반지'),
    ('주얼리 > 발찌'),
    ('주얼리 > 브로치/배지'),
    ('주얼리 > 아동용주얼리'),
    ('주얼리 > 주얼리 소품'),
    ('주얼리 > 주얼리세트'),
    ('주얼리 > 팔찌'),
    ('지갑/벨트 > 남성벨트'),
    ('지갑/벨트 > 남성지갑'),
    ('지갑/벨트 > 동전/통장 지갑'),
    ('지갑/벨트 > 머니클립'),
    ('지갑/벨트 > 멜빵/서스펜더'),
    ('지갑/벨트 > 스마트폰 카드지갑'),
    ('지갑/벨트 > 여성벨트'),
    ('지갑/벨트 > 여성지갑'),
    ('지갑/벨트 > 지갑/벨트세트'),
    ('지갑/벨트 > 카드/명함지갑'),
    ('지갑/벨트 > 카드목걸이'),
    ('채소 > 감자/고구마/옥수수'),
    ('채소 > 냉동/간편채소'),
    ('채소 > 당근/뿌리채소'),
    ('채소 > 두부/묵류'),
    ('채소 > 배추/잎줄기채소'),
    ('채소 > 버섯/나물류'),
    ('채소 > 상추/깻잎/쌈채소'),
    ('채소 > 오이/고추/열매채소'),
    ('채소 > 채소 기타'),
    ('채소 > 파/마늘/양념채소'),
    ('청소용품 > 기타 청소도구'),
    ('청소용품 > 먼지떨이'),
    ('청소용품 > 밀대걸레/패드'),
    ('청소용품 > 분리수거함'),
    ('청소용품 > 빗자루/쓰레받기'),
    ('청소용품 > 청소솔'),
    ('청소용품 > 청소슬리퍼'),
    ('청소용품 > 청소장갑'),
    ('청소용품 > 청소포'),
    ('청소용품 > 테이프클리너'),
    ('청소용품 > 회전탈수 밀대'),
    ('청소용품 > 휴지통'),
    ('축산 > 기타 육류'),
    ('축산 > 달걀'),
    ('축산 > 닭고기'),
    ('축산 > 돼지고기'),
    ('축산 > 수입소고기'),
    ('축산 > 양고기'),
    ('축산 > 오리고기'),
    ('축산 > 축산가공식품'),
    ('축산 > 축산선물세트'),
    ('축산 > 한우'),
    ('취미 > 피규어/캐릭터'),
    ('침구 > 요/토퍼'),
    ('침구 > 이불'),
    ('침구 > 침대/매트커버'),
    ('침실가구 > 매트리스'),
    ('침실가구 > 침대'),
    ('카메라/주변기기 > DSLR카메라'),
    ('카메라/주변기기 > 렌즈용품'),
    ('카메라/주변기기 > 미러리스카메라'),
    ('카메라/주변기기 > 액션캠'),
    ('카메라/주변기기 > 일반 디지털카메라'),
    ('카메라/주변기기 > 즉석카메라'),
    ('카메라/주변기기 > 카메라렌즈'),
    ('카메라/주변기기 > 필름카메라'),
    ('카페트/러그 > 카페트'),
    ('캐주얼/유니섹스 > 니트/스웨터'),
    ('캐주얼/유니섹스 > 바지/팬츠'),
    ('캐주얼/유니섹스 > 셔츠/남방'),
    ('캐주얼/유니섹스 > 스커트'),
    ('캐주얼/유니섹스 > 원피스'),
    ('캐주얼/유니섹스 > 재킷/점퍼/코트'),
    ('캐주얼/유니섹스 > 청바지'),
    ('캐주얼/유니섹스 > 카디건'),
    ('캐주얼/유니섹스 > 트레이닝복'),
    ('캐주얼/유니섹스 > 티셔츠'),
    ('캠핑 > 텐트'),
    ('캠핑 > 텐트/타프 용품'),
    ('커튼/블라인드 > 커튼'),
    ('커튼/블라인드 > 커튼소품/부자재'),
    ('커피/생수/음료 > 건강/차 음료'),
    ('커피/생수/음료 > 생수'),
    ('커피/생수/음료 > 전통음료'),
    ('커피/생수/음료 > 주스/과즙음료'),
    ('커피/생수/음료 > 커피'),
    ('커피/생수/음료 > 코코아/라떼/기타분말'),
    ('커피/생수/음료 > 탄산수'),
    ('커피/생수/음료 > 탄산음료'),
    ('컴퓨터 주변기기 > PC스피커'),
    ('컴퓨터 주변기기 > 게이밍 주변기기'),
    ('컴퓨터 주변기기 > 마우스'),
    ('컴퓨터 주변기기 > 마우스/키보드 세트'),
    ('컴퓨터 주변기기 > 키보드'),
    ('클렌징/필링 > 립앤아이리무버'),
    ('클렌징/필링 > 클렌징로션'),
    ('클렌징/필링 > 클렌징비누'),
    ('클렌징/필링 > 클렌징오일'),
    ('클렌징/필링 > 클렌징워터'),
    ('클렌징/필링 > 클렌징젤'),
    ('클렌징/필링 > 클렌징크림'),
    ('클렌징/필링 > 클렌징티슈'),
    ('클렌징/필링 > 클렌징파우더'),
    ('클렌징/필링 > 클렌징폼'),
    ('클렌징/필링 > 필링젤/스크럽'),
    ('키즈의류 (3~8세) > 남녀공용의류'),
    ('키즈의류 (3~8세) > 여아의류'),
    ('태블릿 > iOS'),
    ('태블릿 > 기타'),
    ('태블릿 > 듀얼OS'),
    ('태블릿 > 리퍼/전시/중고'),
    ('태블릿 > 안드로이드'),
    ('태블릿 > 태블릿 액세서리'),
    ('패션잡화 > 가발'),
    ('패션잡화 > 귀마개'),
    ('패션잡화 > 넥워머'),
    ('패션잡화 > 넥타이'),
    ('패션잡화 > 넥타이핀'),
    ('패션잡화 > 레깅스/타이즈'),
    ('패션잡화 > 마스크'),
    ('패션잡화 > 머플러'),
    ('패션잡화 > 모자'),
    ('패션잡화 > 부토니에'),
    ('패션잡화 > 브로치'),
    ('패션잡화 > 선글라스'),
    ('패션잡화 > 손수건'),
    ('패션잡화 > 숄'),
    ('패션잡화 > 스카프'),
    ('패션잡화 > 스타킹'),
    ('패션잡화 > 안경테'),
    ('패션잡화 > 암워머/토시'),
    ('패션잡화 > 양말'),
    ('패션잡화 > 양산'),
    ('패션잡화 > 와펜'),
    ('패션잡화 > 우산'),
    ('패션잡화 > 장갑'),
    ('패션잡화 > 커프스'),
    ('패션잡화 > 코사지'),
    ('패션잡화 > 키홀더'),
    ('패션잡화 > 파티용 소품'),
    ('패션잡화 > 행커치프'),
    ('패션잡화 > 헤어액세서리'),
    ('프린터/복합기 > 3D프린터'),
    ('프린터/복합기 > 도트프린터'),
    ('프린터/복합기 > 레이저 복합기'),
    ('프린터/복합기 > 레이저 프린터'),
    ('프린터/복합기 > 사무용 복합기/프린터'),
    ('프린터/복합기 > 스캐너'),
    ('프린터/복합기 > 잉크'),
    ('프린터/복합기 > 잉크젯 프린터'),
    ('프린터/복합기 > 토너'),
    ('프린터/복합기 > 포토프린터'),
    ('학생/사무용가구 > 책상'),
    ('해외직구 > 의류/언더웨어'),
    ('향수 > 남녀공용향수'),
    ('향수 > 남성향수'),
    ('향수 > 드레스퍼퓸'),
    ('향수 > 여성향수'),
    ('향수 > 향수세트'),
    ('헤어케어 > 린스'),
    ('헤어케어 > 샴푸'),
    ('헤어케어 > 염색소품'),
    ('헤어케어 > 염색약'),
    ('헤어케어 > 탈모/두피관리제'),
    ('헤어케어 > 트리트먼트'),
    ('헤어케어 > 파마약'),
    ('헤어케어 > 헤어글레이즈'),
    ('헤어케어 > 헤어무스'),
    ('헤어케어 > 헤어미스트'),
    ('헤어케어 > 헤어스타일링'),
    ('헤어케어 > 헤어스프레이'),
    ('헤어케어 > 헤어에센스'),
    ('헤어케어 > 헤어왁스'),
    ('헤어케어 > 헤어젤'),
    ('헤어케어 > 헤어클리닉세트'),
    ('헤어케어 > 헤어팩'),
    ('헬스 > 러닝머신'),
    ('헬스 > 러닝머신 관리용품'),
    ('헬스 > 부위별 운동기구'),
    ('헬스 > 스텝퍼'),
    ('헬스 > 워킹머신'),
    ('헬스 > 웨이트기구'),
    ('헬스 > 일립티컬'),
    ('헬스 > 헬스사이클'),
    ('홈패브릭/수예 > 방석'),
    ('홈패브릭/수예 > 쿠션'),
    ('화방용품 > 붓'),
    ('화방용품 > 스케치북'),
    ('화방용품 > 스케치연필'),
    ('화방용품 > 캔버스/판넬'),
    ('화장지 > 갑티슈')
ON CONFLICT (category) DO NOTHING;

INSERT INTO categories (category) VALUES
    ('화장지 > 냅킨'),
    ('화장지 > 롤화장지'),
    ('화장지 > 여행용티슈'),
    ('화장지 > 키친타월'),
    ('휴대폰 액세서리 > 케이스'),
    ('휴대폰 액세서리 > 휴대폰 보호필름'),
    ('휴대폰 액세서리 > 휴대폰 충전기')
ON CONFLICT (category) DO NOTHING;
