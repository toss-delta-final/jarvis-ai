...
### Fixed
- **이슈 #76 — I-17 소비자 복구·데이터 최소화 정합** — Spring의 `400 INVALID_CURSOR`를 일반 장애와 구분해 저장 커서가 무효면 `since="0"` 임시 스토어 전체 재구축으로 자동 복구하고, 재구축 실패 시 기존 artifact·커서를 보존한다. I-17 원본 입력은 enrichment와 `search_doc` 생성에만 사용하도록 `CatalogArtifact`·pg-catalog·sample loader의 독립 `name`/`category` 사본을 제거하고 기존 볼륨용 idempotent migration을 추가했다. (api-spec §4.8)
