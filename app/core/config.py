...
class Settings(BaseSettings):
    ...
    # [#386] **찜 조회 표지 목록은 두지 않는다.** 처음에는 "조회 표지가 보이면 해제 자동 선택을
    # 막는다"로 짰다가 뺐다 — 그 설계는 조회 표현을 **전수로** 유지해야 성립하는데, 실제로
    # `"내 찜 뭐야"`·`"찜 리스트 좀"` 같은 목록 밖 표현에서 그대로 뚫렸다(PR #441 리뷰).
    # 지금은 `wishlist.py::_resolve_wishlist_remove_target` 이 반대로 **해제 의도의 적극적 근거**
    # 를 요구한다 — 근거가 없으면 되물음이라, 판정이 불완전해도 실패 방향이 안전하다.
    #
    # 그 근거 판정은 아래 두 축의 **결합**으로 한다(`wishlist_remove_markers` 목록 매칭이 아니다).
    # 처음엔 그 목록을 그대로 근거로 썼는데, 목록의 역할이 "가드를 걸지 않을 예외"(누락돼도 무해)
    # 에서 "해제를 허용할 근거"(누락이 곧 기능 손실)로 **정반대가 됐는데도** 목록을 그대로 둬서
    # `"찜 목록에서 빼줘"` 같은 정상 해제 발화가 되물음으로 떨어졌다(`evals/combo_matrix` 의
    # observed 드리프트 가드(#424)가 잡았다). 두 축으로 나누면 목록 전수 유지 없이 표현 변형
    # ("찜 목록에서"·"위시리스트에서"·"찜 리스트에서")을 흡수한다.
    #
    # ⚠️ 여기의 **동사** 표지는 짧아도 된다 — 명사 축과 **함께** 있어야만 근거가 서기 때문이다.
    # 명사 축 없이 `"빼줘"` 하나로는 아무 근거도 만들지 못하므로 `docs/lessons.md` 의
    # "명사 하나짜리 표지 금지"가 겨냥한 위험(표지 하나가 단독으로 동작을 발동)이 생기지 않는다.
    wishlist_scope_markers: list[str] = ["찜", "위시리스트"]
    wishlist_removal_verb_markers: list[str] = ["빼", "해제", "취소", "지워", "삭제", "제거"]
