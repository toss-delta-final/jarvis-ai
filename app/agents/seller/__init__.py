"""판매자 챗봇 그래프 (api-spec v0.7.0 §3.2 — v0.4.0 Batch 1 확대 범위).
...
MVP 범위 2가지 (결정 20 개정):
  ...
  (2) 상세 수정 draft 흐름 — spring_client.get_product_detail(I-7, §4.5) → LLM 개정안 →
      SSE draft {productId, changes:[{field,before,after}]} → FE diff 카드 →
      FE 가 판매자 JWT 로 Spring S-3 PATCH 실행(AI 표면 밖). 채팅 발화는 동의가 아니다.
