"""장바구니 서브그래프 (결정 7 / api-spec v0.7.0 §4.1·§4.9).
...
흐름:
  ...
  조회   : "장바구니에 뭐 있어?" → spring_client.get_cart(I-9) → token 텍스트 답변.
...
TODO: intent 추출 노드(상품/옵션/수량) + 되물음 상태 관리 + add_to_cart/get_cart 호출
      + 응답 분기 (PRODUCT_NOT_FOUND/CART_ERROR/OUT_OF_STOCK(🔴), api-spec §4.1, C-3).
"""
