...
def _row_to_artifact(row: tuple) -> CatalogArtifact:
    product_id, search_doc, embedding, extras, name, category = row
...
_SELECT_COLS = "product_id, search_doc, embedding, extras, name, category"
...
class PgCatalogArtifactStore:
    ...
    def upsert(self, artifact: CatalogArtifact) -> None:
        with self._pool.connection() as conn:
            conn.execute(
                """
                INSERT INTO products (product_id, search_doc, embedding, extras, name, category, updated_at)
